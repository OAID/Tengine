...
class bad_any_cast : public std::bad_cast {
 public:
  bad_any_cast(const std::type_info& expected, const std::type_info& real) {
    ...
    message =
        std::string("Bad any cast:  Expected: ") + GetRealName(real.name());
...
class any final {
 public:
  ...
  /// is ill-formed. This is because an `any` may be copy constructed into
  /// another `any` at any time, so a copy should always be allowed.
  ...
  /// Has the same effect as any(std::forward<ValueType>(value)).swap(*this). No
  /// effect if a exception is thrown.
  ///
  /// T shall satisfy the CopyConstructible requirements, otherwise the program
  /// is ill-formed. This is because an `any` may be copy constructed into
  /// another `any` at any time, so a copy should always be allowed.
 ...
 private:  // Storage and Virtual Method Table
  union storage_union {
    using stack_storage_t =
        typename std::aligned_storage<2 * sizeof(void*),
  ...
  struct vtable_stack {
    ...
    static void swap(storage_union& lhs, storage_union& rhs) noexcept {
      std::swap(reinterpret_cast<T&>(lhs.stack),
                reinterpret_cast<T&>(rhs.stack));
    }
  };
  ...
  static vtable_type* vtable_for_type() {
    using VTableType =
        typename std::conditional<requires_allocation<T>::value,
...
///     *any_cast<remove_reference_t<ValueType>>(&operand). Throws bad_any_cast
///     on failure.
///
template <typename ValueType>
inline ValueType any_cast(any&& operand) {
