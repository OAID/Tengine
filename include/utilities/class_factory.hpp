...
class ClassFactory : NonCopyable {
 public:
  ...
  typename std::enable_if<std::is_class<T>::value, void>::type RegisterType(
      T* creator(Args...), const std::string& name = "") {
    // using funt_t = T*(Args...);
  ...
  typename std::enable_if<std::is_base_of<T, U>::value, T*>::type Create(
      Args... args) {
    any constructor = GetDerivedFunction<T, U, Args...>();
    std::function<T*(Args && ...)> function =
        any_cast<std::function<T*(Args && ...)>>(constructor);
  ...
  template <class T, typename... Args>
  std::shared_ptr<T> CreateSharedWithName(const std::string& name,
                                          Args... args) {
  ...
  template <class T, typename... Args>
  std::unique_ptr<T> CreateUniqueWithName(const std::string& name,
                                          Args... args) {
    T* t = CreateWithName<T, Args...>(name, std::forward<Args>(args)...);
    return std::unique_ptr<T>(t);
  }
 ...
 private:
  ...
  RegisterType_Helper(const std::string& name = "") {
    ...
    std::function<T*(U && u, Args && ...)> function = [](U&& u,
                                                         Args&&... args) {
  ...
  // function
  // this function is diferent from simple function, simple function do not use
  // move/forward(&&)
  ...
  RegisterType_Helper(T* creator(Args...), const std::string& name = "") {
    using func_t = T*(Args && ...);
    // std::cout << "RegisterType_Helper<func&&>: " << type_name<func_t>() <<
    // std::endl;
    size_t key = typeid(func_t).hash_code();
    if (name.length() != 0) key += hash_string_fn{}(name);
  ...
  // Function为简单Class创建实例(new T*(Args&&...)),或Function
  // Creator(T*(Args...) 或 T*(Args&&...))
  template <class T, typename... Args>
