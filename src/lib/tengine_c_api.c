...
graph_t DLLEXPORT teng_create_graph(context_t context, const char* model_format, const char* fname, ...)
{
    int priv_context = 0;

    if (context == NULL)
    {
        context = teng_create_context(NULL, 1);
        priv_context = 1;
    }
...
int DLLEXPORT teng_set_graph_input_node(graph_t graph, const char* input_nodes[], int input_number)
{
    struct ir_graph* ir_graph = ( struct ir_graph* )graph;
    int16_t* node_idxs;
...
int DLLEXPORT teng_set_graph_output_node(graph_t graph, const char* output_nodes[], int output_number)
{
    struct ir_graph* ir_graph = ( struct ir_graph* )graph;
...
int DLLEXPORT teng_destroy_graph(graph_t graph)
{
    struct ir_graph* ir_graph = ( struct ir_graph* )graph;

    if (ir_graph->exec_attr->priv_context)
        teng_destroy_context(ir_graph->exec_attr->exec_context);
...
node_t DLLEXPORT teng_get_graph_output_node(graph_t graph, int idx)
{
    struct ir_graph* ir_graph = ( struct ir_graph* )graph;
...
tensor_t DLLEXPORT teng_get_graph_input_tensor(graph_t graph, int input_idx, int tensor_idx)
{
    struct ir_graph* ir_graph = ( struct ir_graph* )graph;
...
tensor_t DLLEXPORT teng_get_graph_output_tensor(graph_t graph, int output_idx, int tensor_idx)
{
    struct ir_graph* ir_graph = ( struct ir_graph* )graph;
...
node_t DLLEXPORT teng_create_graph_node(graph_t graph, const char* node_name, const char* op_name)
{
    struct ir_graph* ir_graph = ( struct ir_graph* )graph;
...
node_t DLLEXPORT teng_get_graph_node(graph_t graph, const char* node_name)
{
    struct ir_graph* ir_graph = ( struct ir_graph* )graph;
...
const_char_t DLLEXPORT teng_get_node_name(node_t node)
{
...
const_char_t DLLEXPORT teng_get_node_op(node_t node)
{
    struct ir_node* ir_node = ( struct ir_node* )node;
...
void DLLEXPORT teng_release_graph_node(node_t node)
{
    ( void )node;
    // NOTHING NEEDS TO DO
}

tensor_t DLLEXPORT teng_get_node_input_tensor(node_t node, int input_idx)
{
    struct ir_node* ir_node = ( struct ir_node* )node;
...
tensor_t DLLEXPORT teng_get_node_output_tensor(node_t node, int output_idx)
{
    struct ir_node* ir_node = ( struct ir_node* )node;
...
int DLLEXPORT teng_set_node_input_tensor(node_t node, int input_idx, tensor_t tensor)
{
    struct ir_node* ir_node = ( struct ir_node* )node;
...
int DLLEXPORT teng_set_node_output_tensor(node_t node, int output_idx, tensor_t tensor, int tensor_type)
{
    struct ir_node* ir_node = ( struct ir_node* )node;
    struct ir_tensor* ir_tensor = ( struct ir_tensor* )tensor;
...
int DLLEXPORT teng_get_node_input_number(node_t node)
{
    struct ir_node* ir_node = ( struct ir_node* )node;
...
int DLLEXPORT teng_set_node_attr_int(node_t node, const char* attr_name, const int* attr_val)
{
    return teng_set_node_attr_generic(node, attr_name, data_type_typeinfo_name(TENGINE_DT_INT32), attr_val, sizeof(int));
}
//
...
int DLLEXPORT teng_set_node_attr_generic(node_t node, const char* attr_name, const char* type_name, const void* buf,
                                    int size)
{
    struct ir_node* ir_node = ( struct ir_node* )node;
...
tensor_t DLLEXPORT teng_create_graph_tensor(graph_t graph, const char* tensor_name, int data_type)
{
    struct ir_graph* ir_graph = ( struct ir_graph* )graph;
...
tensor_t DLLEXPORT teng_get_graph_tensor(graph_t graph, const char* tensor_name)
{
    struct ir_graph* ir_graph = ( struct ir_graph* )graph;
...
void DLLEXPORT teng_release_graph_tensor(tensor_t tensor)
{
    // NOTHING NEEDS TO DO
}

int DLLEXPORT teng_set_tensor_shape(tensor_t tensor, const int dims[], int dim_number)
{
    struct ir_tensor* ir_tensor = ( struct ir_tensor* )tensor;
...
int DLLEXPORT teng_get_tensor_buffer_size(tensor_t tensor)
{
    struct ir_tensor* ir_tensor = ( struct ir_tensor* )tensor;
...
void_ptr_t DLLEXPORT teng_get_tensor_buffer(tensor_t tensor)
{
    struct ir_tensor* ir_tensor = ( struct ir_tensor* )tensor;
...
int DLLEXPORT teng_set_tensor_buffer(tensor_t tensor, void* buffer, int buffer_size)
{
    struct ir_tensor* ir_tensor = ( struct ir_tensor* )tensor;
    int tensor_size = teng_get_tensor_buffer_size(tensor);
...
//int DLLEXPORT teng_get_tensor_data(tensor_t tensor, void* output_data, int data_size)
//{
//    struct ir_tensor* ir_tensor = ( struct ir_tensor* )tensor;
...
//int DLLEXPORT teng_set_tensor_data(tensor_t tensor, const void* input_data, int data_size)
//{
//    struct ir_tensor* ir_tensor = ( struct ir_tensor* )tensor;
//    int tensor_size = get_tensor_buffer_size(tensor);
//
...
int DLLEXPORT teng_prerun_graph(graph_t graph)
{
    struct ir_graph* ir_graph = ( struct ir_graph* )graph;
...
int DLLEXPORT teng_prerun_graph_multithread(graph_t graph, int cluster, int threads)
{
    check_cpu();
    size_t mask = get_cluster_mask(cluster);
    int count = get_mask_count(mask);
...
int DLLEXPORT teng_run_graph(graph_t graph, int block)
{
    struct ir_graph* ir_graph = ( struct ir_graph* )graph;
    struct exec_context* context = get_ir_graph_context(ir_graph);
    struct exec_scheduler* scheduler = context->scheduler;
...
int DLLEXPORT teng_postrun_graph(graph_t graph)
{
    struct ir_graph* ir_graph = ( struct ir_graph* )graph;
    struct exec_context* context = get_ir_graph_context(ir_graph);
    struct exec_scheduler* scheduler = context->scheduler;
...
void DLLEXPORT teng_dump_graph(graph_t graph)
{
    dump_ir_graph(graph);
}
...
context_t DLLEXPORT teng_create_context(const char* context_name, int empty_context)
{
...
void DLLEXPORT teng_destroy_context(context_t context)
{
...
void DLLEXPORT teng_set_log_level(enum log_level level)
{
    SET_LOG_LEVEL(level);
}

void DLLEXPORT teng_set_log_output(log_print_t func)
{
    SET_LOG_OUTPUT(func);
}
