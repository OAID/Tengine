...
static int tm2_load_concat(struct ir_graph* ir_graph, struct ir_node* ir_node, const TM2_Node* tm_node,
...
static int reg_tm2_ops(void* arg)
{
    ...
    tm2_s->register_op_loader(tm2_s, TM2_OPTYPE_CONCAT, 1, tm2_load_concat, concat_op_map, NULL);
...
static int unreg_tm2_ops(void* arg)
{
    ...
    tm2_s->unregister_op_loader(tm2_s, TM2_OPTYPE_CONCAT, 1, tm2_load_concat);

    return 0;
}

REGISTER_MODULE_INIT(MOD_OP_LEVEL, "reg_concat_ops", reg_tm2_ops);
REGISTER_MODULE_EXIT(MOD_OP_LEVEL, "unreg_concat_ops", unreg_tm2_ops);
