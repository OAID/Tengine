...
static int tm2_load_cast(struct ir_graph* ir_graph, struct ir_node* ir_node, const TM2_Node* tm_node,
                         const TM2_Operator* tm_op)
{
    ...
    fprintf(stderr, "form : %d ,to : %d\n", param->type_from, param->type_to);
...
static int reg_tm2_ops(void* arg)
{
    ...
    tm2_s->register_op_loader(tm2_s, TM2_OPTYPE_CAST, 1, tm2_load_cast, op_map, NULL);
...
static int unreg_tm2_ops(void* arg)
{
    ...
    tm2_s->unregister_op_loader(tm2_s, TM2_OPTYPE_CAST, 1, tm2_load_cast);

    return 0;
}

REGISTER_MODULE_INIT(MOD_OP_LEVEL, "reg_cast_ops", reg_tm2_ops);
REGISTER_MODULE_EXIT(MOD_OP_LEVEL, "unreg_cast_ops", unreg_tm2_ops);
