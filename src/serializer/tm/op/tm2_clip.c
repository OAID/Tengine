...
static int tm2_load_clip(struct ir_graph* ir_graph, struct ir_node* ir_node, const TM2_Node* tm_node,
...
static int reg_tm2_ops(void* arg)
{
    ...
    tm2_s->register_op_loader(tm2_s, TM2_OPTYPE_CLIP, 1, tm2_load_clip, clip_op_map, NULL);
...
static int unreg_tm2_ops(void* arg)
{
    ...
    tm2_s->unregister_op_loader(tm2_s, TM2_OPTYPE_CLIP, 1, tm2_load_clip);

    return 0;
}

REGISTER_MODULE_INIT(MOD_OP_LEVEL, "reg_clip_ops", reg_tm2_ops);
REGISTER_MODULE_EXIT(MOD_OP_LEVEL, "unreg_clip_ops", unreg_tm2_ops);
