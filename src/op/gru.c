...
DEFINE_PARM_PARSE_ENTRY(gru_param, clip, output_len, sequence_len, input_size, hidden_size, has_clip, has_gate_bias,
                        has_candidate_bias, has_init_state);

static int infer_shape(struct ir_node* node)
{
    struct ir_graph* ir_graph = node->graph;
    struct ir_tensor* input = get_ir_graph_tensor(ir_graph, node->input_tensors[0]);
    struct ir_tensor* weight = get_ir_graph_tensor(ir_graph, node->input_tensors[1]);
...
static int register_gru_op(void* arg)
{
    struct op_method m;
...
static int unregister_gru_op(void* arg)
{
    sys_free(GET_PARAM_PARSE_MAP(gru_param));
    return unregister_op(OP_GRU, 1);
}

AUTO_REGISTER_OP(register_gru_op);
AUTO_UNREGISTER_OP(unregister_gru_op);
