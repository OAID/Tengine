...
DEFINE_PARM_PARSE_ENTRY(spacetodepth_param, block_size)
...
static int init_op(struct ir_op* op)
{
    ...
    spacetodepth_param->block_size = 1;

    op->param_mem = spacetodepth_param;
    op->param_size = sizeof(struct spacetodepth_param);
    op->same_shape = 0;
...
static int register_spacetodepth_op(void* arg)
{
    struct op_method m;
...
static int unregister_spacetodepth_op(void* arg)
{
    return unregister_op(OP_SPACETODEPTH, 1);
}

AUTO_REGISTER_OP(register_spacetodepth_op);
AUTO_UNREGISTER_OP(unregister_spacetodepth_op);
