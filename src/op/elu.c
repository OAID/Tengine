...
static int register_elu_op(void* arg)
{
    struct op_method m;
...
static int unregister_elu_op(void* arg)
{
    sys_free(GET_PARAM_PARSE_MAP(elu_param));
    return unregister_op(OP_ELU, 1);
}

AUTO_REGISTER_OP(register_elu_op);
AUTO_UNREGISTER_OP(unregister_elu_op);
