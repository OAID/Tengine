...
static int register_reverse_op(void* arg)
{
    struct op_method m;
...
static int unregister_reverse_op(void* arg)
{
    return unregister_op(OP_REVERSE, 1);
}

AUTO_REGISTER_OP(register_reverse_op);
AUTO_UNREGISTER_OP(unregister_reverse_op);
