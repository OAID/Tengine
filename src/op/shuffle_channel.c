...
DEFINE_PARM_PARSE_ENTRY(shuffle_channel_param, group);
...
static int register_shuffle_channel_op(void* arg)
{
    struct op_method m;
...
static int unregister_shuffle_channel_op(void* arg)
{
    return unregister_op(OP_SHUFFLECHANNEL, 1);
}

AUTO_REGISTER_OP(register_shuffle_channel_op);
AUTO_UNREGISTER_OP(unregister_shuffle_channel_op);
