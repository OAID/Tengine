...
static int register_tile_op(void* arg)
{
    struct op_method m;
    m.op_version = 1;
    m.init_op = init_op;
...
static int unregister_tile_op(void* arg)
{
    sys_free(GET_PARAM_PARSE_MAP(tile_param));
    return unregister_op(OP_TILE,1);
}

AUTO_REGISTER_OP(register_tile_op);
AUTO_UNREGISTER_OP(unregister_tile_op);