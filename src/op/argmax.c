...
DEFINE_PARM_PARSE_ENTRY(argmax_param, axis, keepdims);
...
static int register_argmax_op(void* arg)
{
    struct op_method m;
    m.op_version = 1;
    m.init_op = init_op;
...
static int unregister_argmax_op(void* arg)
{
    return unregister_op(OP_ARGMAX, 1);
}

AUTO_REGISTER_OP(register_argmax_op);
AUTO_UNREGISTER_OP(unregister_argmax_op);
