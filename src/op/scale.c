...
DEFINE_PARM_PARSE_ENTRY(scale_param, axis, num_axes, bias_term);
...
static int register_scale_op(void* arg)
{
    struct op_method m;
...
static int unregister_scale_op(void* arg)
{
    sys_free(GET_PARAM_PARSE_MAP(scale_param));
    return unregister_op(OP_SCALE, 1);
}

AUTO_REGISTER_OP(register_scale_op);
AUTO_UNREGISTER_OP(unregister_scale_op);
