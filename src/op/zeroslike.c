...
static int register_zeroslike_op(void* arg)
{
    struct op_method m;
...
static int unregister_zeroslike_op(void* arg)
{
    return unregister_op(OP_ZEROSLIKE, 1);
}

AUTO_REGISTER_OP(register_zeroslike_op);
AUTO_UNREGISTER_OP(unregister_zeroslike_op);
