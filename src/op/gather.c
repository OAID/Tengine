...
DEFINE_PARM_PARSE_ENTRY(gather_param, axis, indices_num);
...
static int register_gather_op(void* arg)
{
    struct op_method m;
...
static int unregister_gather_op(void* arg)
{
    sys_free(GET_PARAM_PARSE_MAP(gather_param));
    return unregister_op(OP_GATHER, 1);
}

AUTO_REGISTER_OP(register_gather_op);
AUTO_UNREGISTER_OP(unregister_gather_op);
