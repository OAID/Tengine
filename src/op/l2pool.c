...
DEFINE_PARM_PARSE_ENTRY(l2pool_param, paddingType, kernel_h, kernel_w, stride_h, stride_w);
...
static int register_l2pool_op(void* arg)
{
    struct op_method m;
    m.op_version = 1;
    m.init_op = init_op;
...
static int unregister_l2pool_op(void* arg)
{
    sys_free(GET_PARAM_PARSE_MAP(l2pool_param));
    return unregister_op(OP_L2POOL,1);
}

AUTO_REGISTER_OP(register_l2pool_op);
AUTO_UNREGISTER_OP(unregister_l2pool_op);