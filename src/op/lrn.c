...
DEFINE_PARM_PARSE_ENTRY(lrn_param, local_size, alpha, beta, norm_region, k);
...
static int register_lrn_op(void* arg)
{
    struct op_method m;
...
static int unregister_lrn_op(void* arg)
{
    sys_free(GET_PARAM_PARSE_MAP(lrn_param));
    return unregister_op(OP_LRN, 1);
}

AUTO_REGISTER_OP(register_lrn_op);
AUTO_UNREGISTER_OP(unregister_lrn_op);
