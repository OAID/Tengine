...
DEFINE_PARM_PARSE_ENTRY(reorg_param, stride);
...
static int register_reorg_op(void* arg)
{
    struct op_method m;
...
static int unregister_reorg_op(void* arg)
{
    sys_free(GET_PARAM_PARSE_MAP(reorg_param));
    return unregister_op(OP_REORG, 1);
}

AUTO_REGISTER_OP(register_reorg_op);
AUTO_UNREGISTER_OP(unregister_reorg_op);
