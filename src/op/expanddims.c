...
DEFINE_PARM_PARSE_ENTRY(expanddims_param, axis);
...
static int register_expanddims_op(void* arg)
{
    struct op_method m;
...
static int unregister_expanddims_op(void* arg)
{
    sys_free(GET_PARAM_PARSE_MAP(expanddims_param));
    return unregister_op(OP_EXPANDDIMS, 1);
}

AUTO_REGISTER_OP(register_expanddims_op);
AUTO_UNREGISTER_OP(unregister_expanddims_op);
