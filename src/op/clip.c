...
DEFINE_PARM_PARSE_ENTRY(clip_param, max, min);
...
static int register_clip_op(void* arg)
{
    struct op_method m;
...
static int unregister_clip_op(void* arg)
{
    sys_free(GET_PARAM_PARSE_MAP(clip_param));
    return unregister_op(OP_CLIP, 1);
}

AUTO_REGISTER_OP(register_clip_op);
AUTO_UNREGISTER_OP(unregister_clip_op);
