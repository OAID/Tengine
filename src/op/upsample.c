...
DEFINE_PARM_PARSE_ENTRY(upsample_param, scale);
...
static int infer_shape(struct ir_node* node)
{
    ...
    out_dim[2] = input->dims[2] * scale;
    out_dim[3] = input->dims[3] * scale;
...
static int register_upsample_op(void* arg)
{
    struct op_method m;
...
static int unregister_upsample_op(void* arg)
{
    sys_free(GET_PARAM_PARSE_MAP(upsample_param));
    return unregister_op(OP_UPSAMPLE, 1);
}

AUTO_REGISTER_OP(register_upsample_op);
AUTO_UNREGISTER_OP(unregister_upsample_op);
