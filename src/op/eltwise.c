...
DEFINE_PARM_PARSE_ENTRY(eltwise_param, type, caffe_flavor);
...
static int register_eltwise_op(void* arg)
{
    struct op_method m;
...
static int unregister_eltwise_op(void* arg)
{
    sys_free(GET_PARAM_PARSE_MAP(eltwise_param));
    return unregister_op(OP_ELTWISE, 1);
}

AUTO_REGISTER_OP(register_eltwise_op);
AUTO_UNREGISTER_OP(unregister_eltwise_op);
