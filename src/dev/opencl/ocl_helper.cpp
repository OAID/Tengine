...
bool CHECK_ENQUEUE_BUFFER_STATUS(cl_int status)
{
    ...
    {
        fprintf(stderr,"Log: clEnqueue****Buffer status %d\n",status);
        if (status == CL_INVALID_COMMAND_QUEUE  )
            fprintf(stderr,"Log: CL_INVALID_COMMAND_QUEUE   \n");
        else if (status == CL_INVALID_CONTEXT   )
            fprintf(stderr,"Log: CL_INVALID_CONTEXT    \n");
        else if (status == CL_INVALID_MEM_OBJECT  )
            fprintf(stderr,"Log: CL_INVALID_MEM_OBJECT   \n");
        else if (status == CL_INVALID_VALUE  )
            fprintf(stderr,"Log: CL_INVALID_VALUE   \n");
        else if (status == CL_INVALID_EVENT_WAIT_LIST  )
            fprintf(stderr,"Log: CL_INVALID_EVENT_WAIT_LIST   \n");
        else if (status == CL_MISALIGNED_SUB_BUFFER_OFFSET   )
            fprintf(stderr,"Log: CL_MISALIGNED_SUB_BUFFER_OFFSET    \n");
        else if (status == CL_EXEC_STATUS_ERROR_FOR_EVENTS_IN_WAIT_LIST  )
            fprintf(stderr,"Log: CL_EXEC_STATUS_ERROR_FOR_EVENTS_IN_WAIT_LIST   \n");
        else if (status == CL_MEM_OBJECT_ALLOCATION_FAILURE   )
            fprintf(stderr,"Log: CL_MEM_OBJECT_ALLOCATION_FAILURE     \n");
        else if (status == CL_INVALID_OPERATION    )
            fprintf(stderr,"Log: CL_INVALID_OPERATION    \n");
        else if (status == CL_OUT_OF_RESOURCES    )
            fprintf(stderr,"Log: CL_OUT_OF_RESOURCES      \n");
        else if (status == CL_OUT_OF_HOST_MEMORY     )
            fprintf(stderr,"Log: CL_OUT_OF_HOST_MEMORY     \n");
        return false;
    }
//    else
//        fprintf(stderr,"Log: clEnqueue****Buffer SUCCESS\n");
    return true;
}
...
bool CHECK_ENQUEUE_KERNEL_STATUS(cl_int status)
{
    if (status != CL_SUCCESS)
    {
        fprintf(stderr,"Log: clEnqueueNDRangeKernel status %d\n",status);
        if (status == CL_INVALID_PROGRAM_EXECUTABLE   )
            fprintf(stderr,"Log: CL_INVALID_PROGRAM_EXECUTABLE    \n");
        else if (status == CL_INVALID_COMMAND_QUEUE    )
            fprintf(stderr,"Log: CL_INVALID_COMMAND_QUEUE     \n");
        else if (status == CL_INVALID_KERNEL   )
            fprintf(stderr,"Log: CL_INVALID_KERNEL    \n");
        else if (status == CL_INVALID_CONTEXT   )
            fprintf(stderr,"Log: CL_INVALID_CONTEXT    \n");
        else if (status == CL_INVALID_KERNEL_ARGS   )
            fprintf(stderr,"Log: CL_INVALID_KERNEL_ARGS    \n");
        else if (status == CL_INVALID_WORK_DIMENSION    )
            fprintf(stderr,"Log: CL_INVALID_WORK_DIMENSION     \n");
        else if (status == CL_INVALID_GLOBAL_WORK_SIZE   )
            fprintf(stderr,"Log: CL_INVALID_GLOBAL_WORK_SIZE    \n");
        else if (status == CL_INVALID_GLOBAL_OFFSET    )
            fprintf(stderr,"Log: CL_INVALID_GLOBAL_OFFSET      \n");
        else if (status == CL_INVALID_WORK_GROUP_SIZE     )
            fprintf(stderr,"Log: CL_INVALID_WORK_GROUP_SIZE     \n");
        else if (status == CL_INVALID_WORK_ITEM_SIZE     )
            fprintf(stderr,"Log: CL_INVALID_WORK_ITEM_SIZE       \n");
        else if (status == CL_MISALIGNED_SUB_BUFFER_OFFSET      )
            fprintf(stderr,"Log: CL_MISALIGNED_SUB_BUFFER_OFFSET      \n");
        else if (status == CL_INVALID_IMAGE_SIZE    )
            fprintf(stderr,"Log: CL_INVALID_IMAGE_SIZE     \n");
        else if (status == CL_OUT_OF_RESOURCES     )
            fprintf(stderr,"Log: CL_OUT_OF_RESOURCES       \n");
        else if (status == CL_MEM_OBJECT_ALLOCATION_FAILURE     )
            fprintf(stderr,"Log: CL_MEM_OBJECT_ALLOCATION_FAILURE     \n");
...
//    else
//        fprintf(stderr,"Log: clEnqueueNDRangeKernel SUCCESS\n");
    return true;
}
...
bool CHECK_SET_KERNEL_STATUS(cl_int status)
{
    if (status != CL_SUCCESS)
    {
        fprintf(stderr,"Log: clSetKernelArg status %d\n",status);
        if (status == CL_INVALID_KERNEL )
            fprintf(stderr,"Log: CL_INVALID_KERNEL  \n");
        else if (status == CL_INVALID_ARG_INDEX  )
            fprintf(stderr,"Log: CL_INVALID_ARG_INDEX   \n");
        else if (status == CL_INVALID_ARG_VALUE )
            fprintf(stderr,"Log: CL_INVALID_ARG_VALUE  \n");
        else if (status == CL_INVALID_MEM_OBJECT )
            fprintf(stderr,"Log: CL_INVALID_MEM_OBJECT  \n");
        else if (status == CL_INVALID_SAMPLER )
            fprintf(stderr,"Log: CL_INVALID_SAMPLER  \n");
        else if (status == CL_INVALID_ARG_SIZE  )
            fprintf(stderr,"Log: CL_INVALID_ARG_SIZE   \n");
        else if (status == CL_INVALID_ARG_VALUE )
            fprintf(stderr,"Log: CL_INVALID_ARG_VALUE  \n");
        else if (status == CL_OUT_OF_RESOURCES   )
            fprintf(stderr,"Log: CL_OUT_OF_RESOURCES    \n");
...
//    else
//    {
//        fprintf(stderr,"Log: clSetKernelArg SUCCESS   \n");
//    }
    return true;
}
