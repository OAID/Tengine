...
bool OCLEngine::AddConcatNode(struct ir_node* ir_node)
{
    ...
    if (ir_node->input_num == 1)
    {
        ...
        strcat(cl_kernel_path, "/src/dev/opencl/cl/reshape.cl");
//    fprintf(stderr, "Log cl kernel path: %s\n", cl_kernel_path);
        ...
        CHECK_SET_KERNEL_STATUS(clSetKernelArg(kernel, ++arg_idx, sizeof(cl_mem), (void *)&this->ocl_tensor_map[input_tensor0->idx]) );
    ...
    else if (ir_node->input_num == 2)
    {
        ...
        char cl_kernel_path[500] = "";
        strcat(cl_kernel_path, cl_env);
        strcat(cl_kernel_path, "/src/dev/opencl/cl/concat.cl");
        ...
        CHECK_SET_KERNEL_STATUS(clSetKernelArg(kernel,   arg_idx, sizeof(cl_mem), (void *)&this->ocl_tensor_map[output_tensor->idx]) );
        CHECK_SET_KERNEL_STATUS(clSetKernelArg(kernel, ++arg_idx, sizeof(cl_mem), (void *)&this->ocl_tensor_map[input_tensor0->idx]) );
        CHECK_SET_KERNEL_STATUS(clSetKernelArg(kernel, ++arg_idx, sizeof(cl_mem), (void *)&this->ocl_tensor_map[input_tensor1->idx]) );
        CHECK_SET_KERNEL_STATUS(clSetKernelArg(kernel, ++arg_idx, sizeof(int), &output_tensor->elem_num) );
        ...
        fprintf(stderr,"Log : Not Support up till now concat%d!",ir_node->input_num);
