...
void relu6_gpu_kernel(struct ir_graph* ir_graph, struct ir_node* ir_node, dict_uint2voidx  gpu_addr_map)
{
    ...
    relu6<<<grid, bs>>>((float*)gpu_addr_map[output_tensor->idx], (float*)gpu_addr_map[input_tensor->idx], output_tensor->elem_num);
}

void CUDAEngine::AddClipNode(struct ir_graph* ir_graph, struct ir_node* ir_node)
{
