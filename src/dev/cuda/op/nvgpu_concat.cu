...
void concat_gpu_kernel(struct ir_graph* ir_graph, struct ir_node* ir_node, dict_uint2voidx  gpu_addr_map)
{
    ...
    for (int i = 0; i < ir_node->input_num; i++)
    {
        ...
        {
            cudaMemcpy((float*)gpu_addr_map[output_tensor->idx] + sum_size + p * elem_num_axis_sum, (float*)gpu_addr_map[input_tensor->idx] + p * elem_num_axis,
...
void CUDAEngine::AddConcatNode(struct ir_graph* ir_graph, struct ir_node* ir_node)
{
