...
void pooling_gpu_kernel(cudnnHandle_t& handle, struct ir_graph* ir_graph, struct ir_node* ir_node, dict_uint2voidx  gpu_addr_map)
{
    struct ir_tensor* pool_input_data = get_ir_graph_tensor(ir_graph, ir_node->input_tensors[0]);
    struct ir_tensor* pool_output_data = get_ir_graph_tensor(ir_graph, ir_node->output_tensors[0]);
    ...
    auto ret = cudnnPoolingForward(handle, pool_descriptor,
                                   &alpha, input_descriptor, gpu_addr_map[pool_input_data->idx],
                                   &beta, output_descriptor, gpu_addr_map[pool_output_data->idx]
    );
...
void CUDAEngine::AddPoolingNode(struct ir_graph* ir_graph, struct ir_node* ir_node)
{
