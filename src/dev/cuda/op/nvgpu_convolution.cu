...
void conv_gpu_kernel(cudnnHandle_t& handle, struct ir_graph* ir_graph, struct ir_node* ir_node, dict_uint2voidx  gpu_addr_map,
                     cudnnConvolutionFwdAlgo_t& algo1, int setalgo)
{
    struct ir_tensor* conv_input_data = get_ir_graph_tensor(ir_graph, ir_node->input_tensors[0]);
    struct ir_tensor* conv_weight = get_ir_graph_tensor(ir_graph, ir_node->input_tensors[1]);
    struct ir_tensor* conv_output_data = get_ir_graph_tensor(ir_graph, ir_node->output_tensors[0]);
...
//                                                           input_descriptor, gpu_addr_map[conv_input_data->idx],
//                                                           kernel_descriptor, gpu_addr_map[conv_weight->idx],
//                                                           conv_descriptor,
//                                                           output_descriptor, gpu_addr_map[conv_output_data->idx],
//                                                           1,
//                                                           &returnedAlgoCount,
//                                                           &algo,
//                                                           workspace,
...
//    }
    ...
    auto ret2 = cudnnConvolutionForward(handle,
                                        &alpha, input_descriptor, gpu_addr_map[conv_input_data->idx],
                                        kernel_descriptor, gpu_addr_map[conv_weight->idx],
                                        conv_descriptor, algo1,
                                        workspace, workspace_size,
                                        &beta, output_descriptor, gpu_addr_map[conv_output_data->idx]);
    ...
    {
        struct ir_tensor* conv_bias = get_ir_graph_tensor(ir_graph, ir_node->input_tensors[2]);
        ...
        if (conv_param->activation == 0)
            bias_add_relu<<<grid, bs>>>((float*)gpu_addr_map[conv_output_data->idx], (float*)gpu_addr_map[conv_bias->idx], elem_123, elem_23, conv_output_data->elem_num);
        else
            bias_add<<<grid, bs>>>((float*)gpu_addr_map[conv_output_data->idx], (float*)gpu_addr_map[conv_bias->idx], elem_123, elem_23, conv_output_data->elem_num);
    }
...
void CUDAEngine::AddConvolutionNode(struct ir_graph* ir_graph, struct ir_node* ir_node)
{
