...
void softmax_gpu_kernel(cudnnHandle_t& handle, struct ir_graph* ir_graph, struct ir_node* ir_node, dict_uint2voidx  gpu_addr_map, int ag)
{
    struct ir_tensor* soft_input_data = get_ir_graph_tensor(ir_graph, ir_node->input_tensors[0]);
    struct ir_tensor* soft_output_data = get_ir_graph_tensor(ir_graph, ir_node->output_tensors[0]);
    ...
    auto ret = cudnnSoftmaxForward(handle, algo, mode,
                                   &alpha, input_descriptor, gpu_addr_map[soft_input_data->idx],
                                   &beta, output_descriptor, gpu_addr_map[soft_output_data->idx]
    );
}
void softmax_gpu_kernel_2(struct ir_graph* ir_graph, struct ir_node* ir_node, dict_uint2voidx  gpu_addr_map)
{
//    double th = get_time();
    ...
    softmax_k_upload<<<grid, bs>>>(k, (float*)gpu_addr_map[input_tensor->idx], elem_perchannel_match * channels, elem_perchannel_match, elem_perchannel);
    ...
    softmax_k_download<<<grid, bs>>>( k, (float*)gpu_addr_map[output_tensor->idx], elem_perchannel_match * channels, elem_perchannel_match, elem_perchannel );
...
void CUDAEngine::AddSoftmaxNode(struct ir_graph* ir_graph, struct ir_node* ir_node)
{
