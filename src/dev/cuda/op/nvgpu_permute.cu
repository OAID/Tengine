...
void permute_gpu_kernel(struct ir_graph* ir_graph, struct ir_node* ir_node, dict_uint2voidx  gpu_addr_map)
{
    ...
    if (param->order0 == 0 && param->order1 == 2 && param->order2 == 3 && param->order3 == 1)
        permute<<<grid, bs>>>((float*)gpu_addr_map[output_tensor->idx], (float*)gpu_addr_map[input_tensor->idx], output_tensor->elem_num,
                              input_tensor->dims[0], input_tensor->dims[1], input_tensor->dims[2] * input_tensor->dims[3]);
}

void CUDAEngine::AddPermuteNode(struct ir_graph* ir_graph, struct ir_node* ir_node)
{
