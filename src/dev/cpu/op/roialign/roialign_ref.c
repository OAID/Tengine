...
static int ref_roialign_fp32(struct ir_tensor* input_tensor, struct ir_tensor* roi_tensor,
                             struct ir_tensor* output_tensor, struct roialign_param* param, int num_thread)
{
...
static int reg_roialign_hcl_ops(void* arg)
{
    return register_builtin_node_ops(OP_ROIALIGN, &hcl_node_ops);
}

static int unreg_roialign_hcl_ops(void* arg)
{
    return unregister_builtin_node_ops(OP_ROIALIGN, &hcl_node_ops);
}

AUTO_REGISTER_OPS(reg_roialign_hcl_ops);
AUTO_UNREGISTER_OPS(unreg_roialign_hcl_ops);
