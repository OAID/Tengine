...
static int ret_priorbox_node_ops(void* arg)
{
    return register_builtin_node_ops(OP_PRIORBOX, &priorbox_node_ops);
}

static int unret_priorbox_node_ops(void* arg)
{
    return unregister_builtin_node_ops(OP_PRIORBOX, &priorbox_node_ops);
}

AUTO_REGISTER_OPS(ret_priorbox_node_ops);
AUTO_UNREGISTER_OPS(unret_priorbox_node_ops);
