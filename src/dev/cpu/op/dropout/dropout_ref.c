...
static int reg_dropout_hcl_ops(void* arg)
{
    return register_builtin_node_ops(OP_DROPOUT, &hcl_node_ops);
}

static int unreg_dropout_hcl_ops(void* arg)
{
    return unregister_builtin_node_ops(OP_DROPOUT, &hcl_node_ops);
}

AUTO_REGISTER_OPS(reg_dropout_hcl_ops);
AUTO_UNREGISTER_OPS(unreg_dropout_hcl_ops);
