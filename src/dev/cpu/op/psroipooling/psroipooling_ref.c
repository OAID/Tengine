...
static int ref_psroipooling_fp32(struct ir_tensor* featmap_tensor, struct ir_tensor* roi_tensor,
                                 struct ir_tensor* output_tensor, struct psroipooling_param* param, int num_thread)
{
...
static int run(struct node_ops* node_ops, struct exec_node* exec_node, struct exec_graph* exec_graph)
{
    ...
    struct ir_tensor* featmap_tensor;
...
static int reg_psroipooling_hcl_ops(void* arg)
{
    return register_builtin_node_ops(OP_PSROIPOOLING, &hcl_node_ops);
}

static int unreg_psroipooling_hcl_ops(void* arg)
{
    return unregister_builtin_node_ops(OP_PSROIPOOLING, &hcl_node_ops);
}

AUTO_REGISTER_OPS(reg_psroipooling_hcl_ops);
AUTO_UNREGISTER_OPS(unreg_psroipooling_hcl_ops);
