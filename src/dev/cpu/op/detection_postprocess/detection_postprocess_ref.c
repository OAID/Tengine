...
static int run(struct node_ops* node_ops, struct exec_node* exec_node, struct exec_graph* exec_graph)
{
    ...
    struct ir_tensor* score = get_ir_graph_tensor(ir_graph, ir_node->input_tensors[1]);
    void* score_data = score->data;
    struct ir_tensor* anchor = get_ir_graph_tensor(ir_graph, ir_node->input_tensors[2]);
    void* anchor_data = anchor->data;

    struct ir_tensor* detect_boxes = get_ir_graph_tensor(ir_graph, ir_node->output_tensors[0]);
    float* detect_boxes_data = detect_boxes->data;
    struct ir_tensor* detect_classes = get_ir_graph_tensor(ir_graph, ir_node->output_tensors[1]);
    float* detect_classes_data = detect_classes->data;
    struct ir_tensor* detect_scores = get_ir_graph_tensor(ir_graph, ir_node->output_tensors[2]);
    float* detect_scores_data = detect_scores->data;
    struct ir_tensor* detect_num = get_ir_graph_tensor(ir_graph, ir_node->output_tensors[3]);
...
static int reg_detection_postprocess_ops(void* arg)
{
    return register_builtin_node_ops(OP_DETECTION_POSTPROCESS, &detection_postprocess_node_ops);
}

static int unreg_detection_postprocess_ops(void* arg)
{
    unregister_builtin_node_ops(OP_DETECTION_POSTPROCESS, &detection_postprocess_node_ops);
    return 0;
}

AUTO_REGISTER_OPS(reg_detection_postprocess_ops);
AUTO_UNREGISTER_OPS(unreg_detection_postprocess_ops);
