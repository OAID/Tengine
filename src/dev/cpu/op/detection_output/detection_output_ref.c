...
static int ret_detection_output_node_ops(void* arg)
{
    return register_builtin_node_ops(OP_DETECTION_OUTPUT, &detection_output_node_ops);
}

static int unret_detection_output_node_ops(void* arg)
{
    return unregister_builtin_node_ops(OP_DETECTION_OUTPUT, &detection_output_node_ops);
}

AUTO_REGISTER_OPS(ret_detection_output_node_ops);
AUTO_UNREGISTER_OPS(unret_detection_output_node_ops);
