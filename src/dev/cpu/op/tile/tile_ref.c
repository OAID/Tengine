...
static int run(struct node_ops* node_ops, struct exec_node* exec_node, struct exec_graph* exec_graph)
{
    ...
    struct ir_tensor* reps_tensor = get_ir_graph_tensor(ir_graph, ir_node->input_tensors[1]);
...
static int reg_tile_hcl_ops(void* arg)
{
    return register_builtin_node_ops(OP_TILE, &hcl_node_ops);
}

static int unreg_tile_hcl_ops(void* arg)
{
    return unregister_builtin_node_ops(OP_TILE, &hcl_node_ops);
}

AUTO_REGISTER_OPS(reg_tile_hcl_ops);
AUTO_UNREGISTER_OPS(unreg_tile_hcl_ops);
