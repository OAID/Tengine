...
int ref_sparsetodense_fp32(struct ir_tensor* input_tensor, struct ir_tensor* output_shape_tensor,
                           struct ir_tensor* sparse_values_tensor, struct ir_tensor* output_tensor,
...
static int run(struct node_ops* node_ops, struct exec_node* exec_node, struct exec_graph* exec_graph)
{
    ...
    struct ir_tensor* output_shape_tensor;
    struct ir_tensor* sparse_values_tensor;
...
static int reg_sparsetodense_hcl_ops(void* arg)
{
    return register_builtin_node_ops(OP_SPARSETODENSE, &hcl_node_ops);
}

static int unreg_sparsetodense_hcl_ops(void* arg)
{
    return unregister_builtin_node_ops(OP_SPARSETODENSE, &hcl_node_ops);
}

AUTO_REGISTER_OPS(reg_sparsetodense_hcl_ops);
AUTO_UNREGISTER_OPS(unreg_sparsetodense_hcl_ops);
