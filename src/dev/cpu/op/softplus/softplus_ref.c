...
int ref_softplus_fp32(struct ir_tensor* input_tensor, struct ir_tensor* output_tensor, int num_thread)
{
...
static int reg_softplus_hcl_ops(void* arg)
{
    return register_builtin_node_ops(OP_SOFTPLUS, &hcl_node_ops);
}

static int unreg_softplus_hcl_ops(void* arg)
{
    return unregister_builtin_node_ops(OP_SOFTPLUS, &hcl_node_ops);
}

AUTO_REGISTER_OPS(reg_softplus_hcl_ops);
AUTO_UNREGISTER_OPS(unreg_softplus_hcl_ops);
