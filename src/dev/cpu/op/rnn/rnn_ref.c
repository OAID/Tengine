...
static struct ir_tensor* bias_tensor;
...
static int prerun(struct node_ops* node_ops, struct exec_node* exec_node, struct exec_graph* exec_graph)
{
    ...
    struct ir_tensor* init_h_tensor;
...
static int run(struct node_ops* node_ops, struct exec_node* exec_node, struct exec_graph* exec_graph)
{
    ...
    struct ir_graph* ir_graph = ir_node->graph;
    struct ir_tensor* input_tensor;
    struct ir_tensor* kernel_tensor;
    struct ir_tensor* output_tensor;
...
static int reg_rnn_hcl_ops(void* arg)
{
    return register_builtin_node_ops(OP_RNN, &hcl_node_ops);
}

static int unreg_rnn_hcl_ops(void* arg)
{
    return unregister_builtin_node_ops(OP_RNN, &hcl_node_ops);
}

AUTO_REGISTER_OPS(reg_rnn_hcl_ops);
AUTO_UNREGISTER_OPS(unreg_rnn_hcl_ops);
