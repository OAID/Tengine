...
static int reg_l2pool_hcl_ops(void* arg)
{
    return register_builtin_node_ops(OP_L2POOL, &hcl_node_ops);
}

static int unreg_l2pool_hcl_ops(void* arg)
{
    return unregister_builtin_node_ops(OP_L2POOL, &hcl_node_ops);
}

AUTO_REGISTER_OPS(reg_l2pool_hcl_ops);
AUTO_UNREGISTER_OPS(unreg_l2pool_hcl_ops);
