...
#include "./cortex-a/interp_kernel_arm.h"
...
static int reg_interp_hcl_ops(void* arg)
{
    return register_builtin_node_ops(OP_INTERP, &hcl_node_ops);
}

static int unreg_interp_hcl_ops(void* arg)
{
    return unregister_builtin_node_ops(OP_INTERP, &hcl_node_ops);
}

AUTO_REGISTER_OPS(reg_interp_hcl_ops);
AUTO_UNREGISTER_OPS(unreg_interp_hcl_ops);
