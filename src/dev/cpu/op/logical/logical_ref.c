...
static int run(struct node_ops* node_ops, struct exec_node* exec_node, struct exec_graph* exec_graph)
{
    ...
    struct ir_tensor* input_tensor1;
    struct ir_tensor* output_tensor;
    int layout = ir_graph->graph_layout;

    if (ir_node->input_num != 2)
    {
        fprintf(stderr, "logical op need 2 input tensor!\n");
        return -1;
    }
...
static int reg_logical_hcl_ops(void* arg)
{
    return register_builtin_node_ops(OP_LOGICAL, &hcl_node_ops);
}

static int unreg_logical_hcl_ops(void* arg)
{
    return unregister_builtin_node_ops(OP_LOGICAL, &hcl_node_ops);
}

AUTO_REGISTER_OPS(reg_logical_hcl_ops);
AUTO_UNREGISTER_OPS(unreg_logical_hcl_ops);
