...
static int ret_normalize_node_ops(void* arg)
{
    return register_builtin_node_ops(OP_NORMALIZE, &normalize_node_ops);
}

static int unret_normalize_node_ops(void* arg)
{
    return unregister_builtin_node_ops(OP_NORMALIZE, &normalize_node_ops);
}

AUTO_REGISTER_OPS(ret_normalize_node_ops);
AUTO_UNREGISTER_OPS(unret_normalize_node_ops);
