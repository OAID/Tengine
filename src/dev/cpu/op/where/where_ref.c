...
static int run(struct node_ops* node_ops, struct exec_node* exec_node, struct exec_graph* exec_graph)
{
    ...
    struct ir_tensor* input_tensor_a = get_ir_graph_tensor(ir_graph, ir_node->input_tensors[1]);
    struct ir_tensor* input_tensor_b = get_ir_graph_tensor(ir_graph, ir_node->input_tensors[2]);
    ...
    if(ret < -1){
        printf("where operator execution error\n");
        return -1;
    }
...
static int reg_where_hcl_ops(void* arg)
{
    return register_builtin_node_ops(OP_WHERE, &hcl_node_ops);
}

static int unreg_where_hcl_ops(void* arg)
{
    return unregister_builtin_node_ops(OP_WHERE, &hcl_node_ops);
}

AUTO_REGISTER_OPS(reg_where_hcl_ops);
AUTO_UNREGISTER_OPS(unreg_where_hcl_ops);
