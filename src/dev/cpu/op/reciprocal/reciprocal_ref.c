...
int ref_reciprocal_fp32(struct ir_tensor* input_tensor, struct ir_tensor* output_tensor, int num_thread)
{
...
static int reg_reciprocal_hcl_ops(void* arg)
{
    return register_builtin_node_ops(OP_RECIPROCAL, &hcl_node_ops);
}

static int unreg_reciprocal_hcl_ops(void* arg)
{
    return unregister_builtin_node_ops(OP_RECIPROCAL, &hcl_node_ops);
}

AUTO_REGISTER_OPS(reg_reciprocal_hcl_ops);
AUTO_UNREGISTER_OPS(unreg_reciprocal_hcl_ops);
