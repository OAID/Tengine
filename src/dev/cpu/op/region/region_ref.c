...
static int ref_region_fp32(struct ir_tensor* input_tensor, struct ir_tensor* output_tensor, struct region_param* param,
                           int num_thread)
{
...
static int reg_region_hcl_ops(void* arg)
{
    return register_builtin_node_ops(OP_REGION, &hcl_node_ops);
}

static int unreg_region_hcl_ops(void* arg)
{
    return unregister_builtin_node_ops(OP_REGION, &hcl_node_ops);
}

AUTO_REGISTER_OPS(reg_region_hcl_ops);
AUTO_UNREGISTER_OPS(unreg_region_hcl_ops);
