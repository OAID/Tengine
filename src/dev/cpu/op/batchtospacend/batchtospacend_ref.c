...
static int ref_batchtospacend_fp32(struct ir_tensor* input_tensor, struct ir_tensor* output_tensor,
                                   struct batchtospacend_param* param, int num_thread)
{
    ...
    {
        const int out_batch = round(in_batch % out_dims[0]);
        const int spatial_offset = round(in_batch / out_dims[0]);
        for (int in_h = 0; in_h < in_dims[1]; ++in_h)
        {
            const int out_h =
                round(in_h * (param->dilation_y) + spatial_offset / (param->dilation_x) - param->crop_top);
            ...
            {
                const int out_w =
                    round(in_w * param->dilation_x + spatial_offset % param->dilation_x - param->crop_left);

                if (out_w < 0 || out_w >= out_dims[2])
                    continue;

                int outOffset = round(out_batch * out_dims[1] * out_dims[2] * out_dims[3] +
                                      out_h * out_dims[2] * out_dims[3] + out_w * in_dims[3]);
                float* out = out_data + outOffset;
                int inOffset = round(in_batch * in_dims[1] * in_dims[2] * in_dims[3] + in_h * in_dims[2] * in_dims[3] +
                                     in_w * in_dims[3]);
...
static int reg_batchtospacend_hcl_ops(void* arg)
{
    return register_builtin_node_ops(OP_BATCHTOSPACEND, &hcl_node_ops);
}

static int unreg_batchtospacend_hcl_ops(void* arg)
{
    return unregister_builtin_node_ops(OP_BATCHTOSPACEND, &hcl_node_ops);
}

AUTO_REGISTER_OPS(reg_batchtospacend_hcl_ops);
AUTO_UNREGISTER_OPS(unreg_batchtospacend_hcl_ops);
