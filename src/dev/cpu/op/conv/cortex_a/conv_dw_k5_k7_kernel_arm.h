...
void depthwise_conv_k5s1(float* input, float* weight, float* bias, float* output, int input_h, int input_w, int channel,
                         int output_h, int output_w, int pad0, int pad1, int activation, int num_thread)
{
...
// #pragma omp parallel for num_threads(num_thread)
        ...
        {
            /* pad */
            float* input_buf = ( float* )malloc(sizeof(float) * input_h_pad * input_w_pad + 128);
            ...
            if (bias)
                ...
                dw_k5s1(input_buf, weight_cur, NULL, output_cur, output_h, output_w, activation);

            /* free input temp buffer */
            free(input_buf);
        }
    }
    else
...
// #pragma omp parallel for num_threads(num_thread)
        for (int c = 0; c < channel; c++)
        {
            ...
            float* weight_cur = weight + c * 25;    // kernel_w * kernel_h is 5 * 5 = 25
            float* output_cur = output + c * output_h * output_w;
            dw_k5s1(input_cur, weight_cur, bias + c, output_cur, output_h, output_w, activation);
        }
    }
}
