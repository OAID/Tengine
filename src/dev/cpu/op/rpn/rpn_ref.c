...
static int run(struct node_ops* node_ops, struct exec_node* exec_node, struct exec_graph* exec_graph)
{
    ...
    struct ir_tensor* score_tensor;
    struct ir_tensor* featmap_tensor;
    struct ir_tensor* info_tensor;
...
static int reg_rpn_ops(void* arg)
{
    return register_builtin_node_ops(OP_RPN, &rpn_node_ops);
}

static int unreg_rpn_ops(void* arg)
{
    return unregister_builtin_node_ops(OP_RPN, &rpn_node_ops);
}

AUTO_REGISTER_OPS(reg_rpn_ops);
AUTO_UNREGISTER_OPS(unreg_rpn_ops);
