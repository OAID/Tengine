...
bool CLGraph::AddCropLayer(struct ir_node* node)
{
    ...
    struct ir_tensor* crop_ref = get_ir_graph_tensor(graph, node->input_tensors[1]);
    ...
    tensors_map_[std::to_string(node->idx+0.1).c_str()] = boxes;
    ...
    tensors_map_[std::to_string(node->idx+0.2).c_str()] = box_ind;
    int boxes_idx[1] = {0};
    ...
    if (data_type_ == DataType::F16)
    {
        ...
        tensors_map_[std::to_string(node->idx+0.1).c_str()] = cast_in_tensor;
        ...
        tensors_map_[std::to_string(node->idx+0.2).c_str()] = cast_out_tensor;

        /* fp16 to fp32 */
        CLCast* cast_in = new CLCast();
        CLCast* cast_out = new CLCast();
