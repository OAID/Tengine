...
bool TensorRTEngine::AddUpsampleNode(struct ir_graph* ir_graph, struct ir_node* node)
{
    struct ir_tensor* upsample_input = get_ir_graph_tensor(ir_graph, node->input_tensors[0]);
    struct ir_tensor* upsample_output = get_ir_graph_tensor(ir_graph, node->output_tensors[0]);
    ...
    if (!check_if_input_in_map(upsample_input->idx, this->tensor_swap_map))
    {
        fprintf(stderr, "Tengine: Query input for Interp(id: %d, name: %s) layer failed.\n", node->idx, node->name);
        return false;
    }

    nvinfer1::ITensor* upsample_input_tensor = tensor_real_map[tensor_swap_map[upsample_input->idx]];
