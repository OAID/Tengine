...
bool TensorRTEngine::AddConcatNode(struct ir_graph* ir_graph, struct ir_node* node)
{
    ...
    {
        struct ir_tensor* ir_tensor = get_ir_graph_tensor(ir_graph, node->input_tensors[i]);
        if (nullptr != ir_tensor && !check_if_input_in_map(ir_tensor->idx, this->tensor_swap_map))
        {
    ...
    this->tensor_real_map[output_tensor->idx] = concat_tensor;
    this->tensor_swap_map[output_tensor->idx] = output_tensor->idx;
