...
bool TensorRTEngine::AddConvolutionNode(struct ir_graph* ir_graph, struct ir_node *node)
{
    nvinfer1::Weights weight{nvinfer1::DataType::kFLOAT, nullptr, 0};
    nvinfer1::Weights bias{nvinfer1::DataType::kFLOAT, nullptr, 0};

    struct ir_tensor* conv_data = get_ir_graph_tensor(ir_graph, node->input_tensors[0]);
    struct ir_tensor* conv_weight = get_ir_graph_tensor(ir_graph, node->input_tensors[1]);

    if (nullptr == conv_data || nullptr == conv_weight)
    {
        fprintf(stderr, "Tengine: Get input data & weight for conv(id: %d, name: %s).\n", conv_weight->idx, conv_weight->name);
        return false;
    }

    if (!check_if_input_in_map(conv_data->idx, this->tensor_swap_map))
    {
        fprintf(stderr, "Tengine: Query input for Concat(id: %d, name: %s) layer failed.\n", node->idx, node->name);
        return false;
    }

    nvinfer1::ITensor* input = this->tensor_real_map[this->tensor_swap_map[conv_data->idx]];
    ...
    switch (conv_weight->data_type)
    {
        ...
        default:
            fprintf(stderr, "Tengine: Unsupported weight quant data type(%d) of conv(id: %d, name: %s).\n", conv_weight->data_type, node->idx, node->name);
            return false;
    }

    if(2 < node->input_num)
    {
        struct ir_tensor* conv_bias = get_ir_graph_tensor(ir_graph, node->input_tensors[2]);
        ...
        switch (conv_bias->data_type)
        {
            ...
            default:
                fprintf(stderr, "Tengine: Unsupported weight quant data type(%d) of conv(id: %d, name: %s).\n", conv_bias->data_type, node->idx, node->name);
                return false;
        }
    }
