...
bool TensorRTEngine::addReLUNode(struct ir_graph *ir_graph, struct ir_node *node)
{
    int op_type = OP_RELU;

    struct ir_tensor* relu_input = nullptr;
    ...
    nvinfer1::ITensor* trt_tensor = tensor_real_map[relu_input->idx];
    ...
    /*if (need_change_name)
    {
        struct ir_tensor* node_output = get_ir_graph_tensor(ir_graph, node->output_tensors[0]);

        std::string tensor_name = std::string(node_output->name) + std::to_string(node_output->idx);
    ...
    if (OP_CONV == node->op.op_type)
    {
        this->tensor_real_map[relu_input->idx + tensor_swap_count] = trt_tensor;
        this->tensor_swap_map[relu_input->idx] = relu_input->idx + tensor_swap_count;
    }
    else
    {
        struct ir_tensor* relu_output = get_ir_graph_tensor(ir_graph, node->output_tensors[0]);

        this->tensor_real_map[relu_output->idx] = trt_tensor;
        this->tensor_swap_map[relu_output->idx] = relu_output->idx;
    }
