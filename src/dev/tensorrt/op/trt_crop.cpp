...
bool TensorRTEngine::AddCropNode(struct ir_graph* ir_graph, struct ir_node* node)
{
    struct ir_tensor* crop_input = get_ir_graph_tensor(ir_graph, node->input_tensors[0]);
    struct ir_tensor* crop_ref = get_ir_graph_tensor(ir_graph, node->input_tensors[1]);
    struct ir_tensor* crop_output = get_ir_graph_tensor(ir_graph, node->output_tensors[0]);
    ...
    if (!check_if_input_in_map(crop_input->idx, this->tensor_swap_map))
    {
    ...
    nvinfer1::ITensor* crop_input_tensor = tensor_real_map[tensor_swap_map[crop_input->idx]];
