...
bool TensorRTEngine::AddDeConvolutionNode(struct ir_graph* ir_graph, struct ir_node *node)
{
    nvinfer1::Weights weight{nvinfer1::DataType::kFLOAT, nullptr, 0};
    nvinfer1::Weights bias{nvinfer1::DataType::kFLOAT, nullptr, 0};

    struct ir_tensor* deconv_data = get_ir_graph_tensor(ir_graph, node->input_tensors[0]);
    struct ir_tensor* deconv_weight = get_ir_graph_tensor(ir_graph, node->input_tensors[1]);

    if (nullptr == deconv_data || nullptr == deconv_weight)
    {
        fprintf(stderr, "Tengine: Get input data & weight for deconv(id: %d, name: %s).\n", deconv_weight->idx, deconv_weight->name);
        return false;
    }

    if (!check_if_input_in_map(deconv_data->idx, this->tensor_swap_map))
    {
        fprintf(stderr, "Tengine: Query input for Deconv(id: %d, name: %s) layer failed.\n", node->idx, node->name);
        return false;
    }

    nvinfer1::ITensor* input = this->tensor_real_map[this->tensor_swap_map[deconv_data->idx]];
    ...
    switch (deconv_weight->data_type)
    {
        ...
        default:
            fprintf(stderr, "Tengine: Unsupported weight quant data type(%d) of conv(id: %d, name: %s).\n", deconv_weight->data_type, node->idx, node->name);
            return false;
    }

    if(2 < node->input_num)
    {
        struct ir_tensor* deconv_bias = get_ir_graph_tensor(ir_graph, node->input_tensors[2]);
        ...
        switch (deconv_bias->data_type)
        {
            ...
            default:
                fprintf(stderr, "Tengine: Unsupported weight quant data type(%d) of conv(id: %d, name: %s).\n",
                        deconv_bias->data_type, node->idx, node->name);
                return false;
        }
    }
