...
bool TensorRTEngine::AddSoftmaxNode(struct ir_graph* ir_graph, struct ir_node* node)
{
    struct ir_tensor* softmax_input = get_ir_graph_tensor(ir_graph, node->input_tensors[0]);
    if (nullptr != softmax_input && !check_if_input_in_map(softmax_input->idx, this->tensor_swap_map))
    {
        fprintf(stderr, "Tengine: Query input for Softmax(id: %d, name: %s) layer failed.\n", node->idx, node->name);
        return false;
    }

    nvinfer1::ITensor * trt_tensor = tensor_real_map[tensor_swap_map[softmax_input->idx]];
    ...
    struct ir_tensor* softmax_output  = get_ir_graph_tensor(ir_graph, node->output_tensors[0]);

    this->SetRange(softmax_output, trt_tensor);

    this->tensor_real_map[softmax_output->idx] = trt_tensor;
    this->tensor_swap_map[softmax_output->idx] = softmax_output->idx;
