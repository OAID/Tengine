...
bool ACLGraphExecutor::DevGetPerf(Subgraph* graph, int policy,
                                  GraphPerf& perf) {
...
bool ACLGraphExecutor::DevSetConfig(const char* config_name, const void* buffer,
                                    int size) {
  return backend_dev_->SetDevConfig(config_name, buffer, size);
}

bool ACLGraphExecutor::DevGetConfig(const char* config_name, void* buffer,
                                    int size) {
...
bool ACLGraphExecutor::DevRun(void* graph_handle) {
  auto f = std::bind(&ACLGraphExecutor::OnSubgraphDone, this,
...
bool ACLGraphExecutor::Init(void) { return true; }

bool ACLGraphExecutor::Release(void) { return true; }

void ACLGraphExecutor::UnbindDevice(void) { backend_dev_ = nullptr; }
...
bool ACLGraphExecutor::DevStart(void) { return backend_dev_->Start(); }

bool ACLGraphExecutor::DevStop(void) { return backend_dev_->Stop(); }
