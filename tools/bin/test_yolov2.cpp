...
const char *image_list =
    "/home/firefly/my_tengine/tengine/tools/data/2007_test.txt";
...
void get_region_box(Box &b, float *x, std::vector<float> &biases, int n,
...
void correct_region_boxes(std::vector<Box> &boxes, int n, int w, int h,
                          int netw, int neth) {
  int i;
  int new_w = 0;
  int new_h = 0;
...
void get_region_boxes(float *output, std::vector<float> &biases, int neth,
                      int netw, int h, int w, int img_w, int img_h, int num_box,
                      int num_classes, float thresh, float **probs,
  ...
  for (i = 0; i < hw; ++i) {
    ...
    for (n = 0; n < num_box; ++n) {
      ...
      get_region_box(boxes[index], predictions, biases, n, box_index, col, row,
                     w, h, hw);
...
void do_nms_sort(std::vector<Box> &boxes, float **probs, int total, int classes,
                 float thresh) {
  int i, j, k;
...
void string_replace(std::string &str, const std::string &srcstr,
...
void record_tp(std::string &image_file, int num, float thresh,
               std::vector<Box> &boxes, float **probs, int classes,
    ...
    for (j = 0; j < classes; ++j) {
      if (probs[i][j] > thresh) {
        ...
        for (int g = 0; g < ground_num; g++) {
          if (ground_truth[g].class_index == class_id &&
              ground_truth[g].b_find == false) {
...
void cal_recall_prob(MAP *map, int class_num) {
  for (int i = 0; i < class_num; i++) {
    ...
    // std::cout<<"class:   "<<class_names[i+1]<<":"<<"  recall:   "<<recall<<"
    // ,precesion:   "<<prob<<"\n";
  }
}
void preprocess_yolov2(std::string &image_file, float *input_data, int img_h,
                       int img_w, int *raw_h, int *raw_w) {
  ...
  *raw_w = img0.cols;
  ...
  cv::copyMakeBorder(resize_img, dst_img, delta_h, delta_h, delta_w, delta_w,
...
int main(int argc, char **argv) {
  ...
  for (int i = 0; i < REPEAT_COUNT; i++) {
    ...
    if (get_node_param_int(node, "num_box", &num_box) < 0) {
    ...
    if (get_node_param_generic(node, "biases", &typeid(std::vector<float>),
                               &param_biases) < 0) {
    ...
    get_region_boxes(output, param_biases, img_h, img_w, out_dim[2], out_dim[3],
                     raw_w, raw_h, num_box, num_class, thresh, probs, boxes);

    float nms_thresh = 0.3;
    do_nms_sort(boxes, probs, total, num_class, nms_thresh);

    // char *ground_truth_file =
    // "/home/firefly/VOCdevkit/VOC2007/labels/000001.txt";
    ...
    GROUND_TRUTH *ground_truth =
        read_boxes((char *)ground_truth_file.c_str(), &ground_num);

    std::cout << "ground num is :" << ground_num << "\n";

    record_tp(image_file, total, thresh, boxes, probs, num_class, ground_truth,
              ground_num, global_map);
