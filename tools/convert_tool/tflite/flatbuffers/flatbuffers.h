...
#if defined(FLATBUFFERS_NAN_DEFAULTS) && (!defined(_MSC_VER) || _MSC_VER >= 1800)
...
public:
    ...
    template<typename T>
    bool Verify(const uint8_t* base, voffset_t elem_off)
        const
    {
    ...
    {
        size_t end;
        return !str || (VerifyVectorOrString(reinterpret_cast<const uint8_t*>(str), 1, &end) && Verify(end, 1) && // Must have terminator
    ...
    bool VerifyTableStart(const uint8_t* table)
    {
        ...
        return VerifyComplexity() && Verify<voffset_t>(vtableo) && VerifyAlignment<voffset_t>(ReadScalar<voffset_t>(buf_ + vtableo)) && Verify(vtableo, ReadScalar<voffset_t>(buf_ + vtableo));
    }

    template<typename T>
    bool VerifyBufferFromStart(const char* identifier, size_t start)
    {
        if (identifier && (size_ < 2 * sizeof(flatbuffers::uoffset_t) || !BufferHasIdentifier(buf_ + start, identifier)))
        {
            return false;
        }
    ...
    {
        return Verify<uoffset_t>(0U) && ReadScalar<uoffset_t>(buf_) == size_ - sizeof(uoffset_t) && VerifyBufferFromStart<T>(identifier, sizeof(uoffset_t));
    }
...
template<typename T>
void FlatBufferBuilder::Required(Offset<T> table,
                                 voffset_t field)
{
