...
{
    return (t >= FBT_VECTOR_INT && t <= FBT_VECTOR_STRING) || t == FBT_VECTOR_BOOL;
}
...
private:
    ...
    struct Value
    {
        ...
        Value()
            : i_(0), type_(FBT_NULL), min_bit_width_(BIT_WIDTH_8)
        {
        }
        ...
        Value(float f)
            : f_(f), type_(FBT_FLOAT), min_bit_width_(BIT_WIDTH_32)
        {
        }
        Value(double f)
            : f_(f), type_(FBT_FLOAT), min_bit_width_(WidthF(f))
        {
        }
        ...
        BitWidth ElemWidth(size_t buf_size, size_t elem_index) const
        {
            ...
            else
            {
                     ...
                     byte_width *= 2)
                {
                    ...
                    auto offset_loc = buf_size + flatbuffers::PaddingBytes(buf_size, byte_width) + elem_index * byte_width;
    ...
    {
        explicit KeyOffsetCompare(const std::vector<uint8_t>& buf)
            : buf_(&buf)
        {
        }
