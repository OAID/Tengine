...
const char* model_file = "./models/psroipooling_mx.tmfile";
...
float result[4] = {0.440000, 0.373333, 0.578333, 0.675000};
...
int main(int argc, char* argv[])
{
    ...
    set_tensor_shape(input_tensor2, dims2, 4);  
    float* input_data = ( float* )malloc(sizeof(float) * img_h * img_w );

    for(int i = 0; i < img_h*img_w*1; i++){
    ...
    if(set_tensor_buffer(input_tensor2, input2, 4 * 4) < 0)
    {
        std::printf("Set buffer for tensor failed\n");
        return -1;
    }
