...
int QuantTool::activation_quant_tool()
{
    ...
    for (int nums = 0; nums < img_num; nums++)
    {
        ...
        get_input_data_cv(imgs_list[nums].c_str(), input_data.data(), img_c, img_h, img_w, mean, scale, sw_RGB,
                          center_crop, letterbox_rows, letterbox_cols, focus);
            ...
            for (int i = 0; i < ir_graph->tensor_num; i++)
            {
                ...
                if (ir_tensor->tensor_type == TENSOR_TYPE_VAR || ir_tensor->tensor_type == TENSOR_TYPE_INPUT)
                {
                    ...
                    if (nums == imgs_list.size() - 1)
                    {
                        ...
                        hist_gram.push_back(histCount((float*)ir_tensor->data, ir_tensor->elem_num, max_activation[i],
                                                      min_activation[i]));
                    }
                    else
                    {
                        std::vector<uint32_t> hist_tmp;
                        hist_tmp = histCount((float*)ir_tensor->data, ir_tensor->elem_num, max_activation[i],
                                             min_activation[i]);
                        for (int j = 0; j < 2048; j++)
                        {
...
int main(int argc, char* argv[])
{
    ...
    save_graph_u8_perlayer(quant_tool.model_file.c_str(), quant_tool.scale_file.c_str(), quant_tool.output_file,
                           quant_tool.inplace, false);
