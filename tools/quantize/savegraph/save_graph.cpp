...
tm_uoffset_t SaveTmTensor(void* const start_ptr, tm_uoffset_t* cur_pos, struct tensor* tensor, unsigned int tensor_id,
...
tm_uoffset_t SaveTmSubgraph(void* const start_ptr, tm_uoffset_t* cur_pos, struct graph* graph)
{
    ...
    std::vector<struct tensor*> tensor_ptrs;
...
bool SaveModelIntoMem(void* start_ptr, struct graph* graph, uint32_t* tm_model_size)
{
    bool tm_with_string = IsSaveString();
...
int save_model(std::vector<void*>& addr_list, std::vector<int>& size_list, struct graph* graph)
{
