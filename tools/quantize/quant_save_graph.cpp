                            ...
                            std::tr1::unordered_map<std::string, bool>&  layer_pass)
{
    if (layer_pass[t->name] == false && layer_used[t->name] < 2)
    {
        ...
        if (op_name == "Flatten" || op_name == "Reshape" || op_name == "Squeeze" || op_name == "Clip" || poolTrue
            || reluTrue)
        {
            ...
            if (layer_scale[t->name] != 0)
            {
                ...
                {
                    recursion_pass_through(ir_graph, t->name, t_in_tensor, layer_used, layer_scale, layer_zeropoint,
                                           layer_pass);
                }
            }
        }
...
int save_graph_u8_perlayer(const char* model_file, const char* scale_file, const std::string& output_file, int inplace,
                           bool internal)
{
                ...
                if (layer_pass[ir_tensor->name] == false)
                {
                        ...
                        || op_name == "Slice" || poolTrue || reluTrue)
                    {
                        ...
                        if (layer_scale[ir_tensor->name] != 0)
                        {
                            ...
                            if (t_in_tensor->tensor_type == TENSOR_TYPE_VAR
                                || t_in_tensor->tensor_type == TENSOR_TYPE_INPUT)
                            {
                                recursion_pass_through(ir_graph, ir_tensor->name, t_in_tensor, layer_used, layer_scale,
...
int save_graph_i8_perchannel(const char* model_file, const char* scale_file, const std::string& output_file,
                             int inplace, bool internal)
{
    fprintf(stderr, "[Quant Tools Info]: Step 3, load FP32 tmfile once again\n");
                ...
                for (int ch = 0; ch < channel_num; ch++)
                {
                    for (int bi = 0; bi < bstep; bi++)
                    {
                        if (bias_data[ch * bstep + bi] == 0 || bias_scale_list[ch] == 0)
                            ...
                            int32_bias_data[ch * bstep + bi] =
                                int(round(bias_data[ch * bstep + bi] / bias_scale_list[ch]));
                    }
                }
