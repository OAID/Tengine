...
void TEnginePlugin::SetPluginManager(void) {
  ...
  for (unsigned int i = 0; i < keys.size(); i++) {
    value = "";
    if (!TEngineConfig::Get<std::string>(keys.at(i), value)) continue;
    // Set the plugin manager
    SetPlugin(keys.at(i), value);
  }
}

void TEnginePlugin::SetPlugin(const std::string& key,
...
bool TEnginePlugin::LoadAll(void) {
  ...
  while (ir != pm->end()) {
    ...
    if (!LoadPlugin(key)) return false;

    if (!InitPlugin(key)) return false;
...
bool TEnginePlugin::LoadPlugin(const std::string& name) {
  ...
  if (!GetFullName(name, fullname)) return false;
...
bool TEnginePlugin::InitPlugin(const std::string& name) {
  ...
  if (!GetInitName(name, initname)) return false;

  if (!initname.size() || !GetHandler(name, p)) return false;

  p.ExecuteFunc<int()>(initname);
  return true;
}

bool TEnginePlugin::GetFullName(const std::string& name,
                                std::string& fullname) {
  NameManager* nm = GetNameManager();
  ...
  if (!fullname.size()) {
    LOG_ERROR()
        << "The fullname of the plugin is not set in the config file!\n";
    return false;
  }
  return true;
}

bool TEnginePlugin::GetInitName(const std::string& name,
                                std::string& initname) {
  InitManager* im = GetInitManager();
  ...
  if (!initname.size()) {
    LOG_ERROR()
        << "The initname of the plugin is not set in the config file!\n";
    return false;
  }
  return true;
}

bool TEnginePlugin::GetHandler(const std::string& name,
                               ShareLibParser& handler) {
  HandlerManager* hm = GetHandlerManager();
...
void TEnginePlugin::DumpPlugin(void) {
  ...
  for (unsigned int i = 0; i < names.size(); i++) {
    ...
    std::cout << names.at(i) << " : " << fullname << " : " << initname
              << std::endl;
  }
}
...
void TEnginePlugin::RegisterModuleInit(int priority,
...
std::unordered_map<int, module_release_func_t>* TEnginePlugin::GetReleaseTable(
    void) {
...
void TEnginePlugin::RegisterModuleRelease(int priority,
...
int load_tengine_plugin(const char* plugin_name, const char* fname,
                        const char* init_func_name) {
  if (plugin_name == nullptr || fname == nullptr) return -1;

  ShareLibParserPtr handle(new ShareLibParser());

  if (handle->Load(fname) < 0) return -1;

  if (init_func_name) {
    if (handle->ExecuteFunc<int()>(init_func_name) < 0) return -1;
  }
...
int unload_tengine_plugin(const char* plugin_name, const char* rel_func_name) {
  if (plugin_name == nullptr) return -1;
  ...
  while (ir != ir_end) {
    ...
    if (ptr->plugin_name == std::string(plugin_name)) break;
  }

  if (ir == ir_end) return -1;

  int ret = 0;

  if (rel_func_name) ret = (*ir)->handle->ExecuteFunc<int()>(rel_func_name);
...
int get_tengine_plugin_number(void) { return g_plugin_table.size(); }

const char* get_tengine_plugin_name(int idx) {
  int plugin_number = g_plugin_table.size();

  if (idx >= plugin_number || idx < 0) return nullptr;
