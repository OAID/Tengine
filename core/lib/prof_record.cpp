...
int ProfTime::GetRecordNum(void) const { return record.size(); }
...
bool ProfTime::Stop(int idx) {
  ...
  if (used_time > r.max_time) r.max_time = used_time;
  if (used_time < r.min_time) r.min_time = used_time;
...
void ProfTime::Dump(int method) {
  if (method == PROF_DUMP_DECREASE) {
    std::sort(record.begin(), record.end(),
              [](const TimeRecord& a, const TimeRecord& b) {
