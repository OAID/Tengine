...
void Graph::DumpGraph(void) {
  std::cout << "Graph: " << name_ << " total nodes: " << seq_nodes.size()
            << std::endl;
...
Node* Graph::FindNode(const std::string& node_name) {
  ...
  for (unsigned int i = 0; i < seq_nodes.size(); i++) {
    ...
    if (target_name == node_name) return node;
  }
...
bool Graph::AddInputNode(const std::string& node_name) {
  ...
  for (unsigned int i = 0; i < input_nodes.size(); i++) {
    if (node == input_nodes[i]) return true;
  }
...
bool Graph::AddOutputNode(const std::string& node_name) {
  ...
  for (unsigned int i = 0; i < output_nodes.size(); i++) {
    if (node == output_nodes[i]) return true;
  }
...
Tensor* Graph::FindTensor(const std::string& tensor_name) {
  if (tensor_map_.count(tensor_name)) return tensor_map_[tensor_name];

  return nullptr;
}

bool Graph::CreateNodeFromStatic(Node* node, const StaticGraph* static_graph,
                                 const StaticNode* static_node) {
  ...
  for (unsigned int i = 0; i < static_node->output_tensor_list.size(); i++) {
    ...
    if (static_tensor->type == kConstTensor) {
      StaticConstTensor* const_tensor =
          dynamic_cast<StaticConstTensor*>(static_tensor);
...
bool Graph::SetupConnection(Tensor* tensor, const StaticGraph* static_graph,
                            const StaticTensor* static_tensor) {
  ...
  for (unsigned int i = 0; i < static_tensor->consumer.size(); i++) {
    ...
    const StaticNode* static_node =
        static_graph->node_list[p_synapse->node_index].get();
...
bool Graph::RealCreateFromStatic(const StaticGraphPtr& static_graph) {
  ...
  for (int i = 0; i < node_number; i++) {
    ...
    if (!CreateNodeFromStatic(node, static_graph.get(), node_ptr)) return false;
  }
  ...
  /* re-order the seq_nodes_, removing un-used node, according to node
   * dependency*/
...
Graph* Graph::CreateFromStatic(const StaticGraphPtr& static_graph) {
  ...
  if (new_graph->RealCreateFromStatic(static_graph)) return new_graph;
...
bool Graph::RemoveTensor(Tensor* tensor) {
  ...
  if (ir != tensor_map_.end()) tensor_map_.erase(ir);

  /* if it is my tensor */
  if (RemoveTensorOwner(tensor)) delete tensor;
...
bool Graph::RemoveNode(Node* node) {
  ...
  if (ir != seq_nodes.end()) seq_nodes.erase(ir);
  ...
  if (RemoveNodeOwner(node)) delete node;

  return true;
}

void Graph::PopulateDynamicShape(void) {
  /* assume all the input nodes  of a node are before the node in seq_nodes
   * array */
...
void Graph::StripGraph(void) {
  ...
  for (int i = 0; i < node_number; i++) seq_nodes[i]->SetNodeIndex(i);
...
bool Graph::IsOutputNode(Node* node) {
  for (unsigned int i = 0; i < output_nodes.size(); i++) {
    if (output_nodes[i] == node) return true;
  }
...
bool Graph::IsInputNode(Node* node) {
  for (unsigned int i = 0; i < output_nodes.size(); i++) {
    if (input_nodes[i] == node) return true;
  }
...
void Graph::RemoveNoChildTensor(void) {
  ...
  for (unsigned int i = 0; i < tensor_list.size(); i++) {
    ...
    if (!IsOutputNode(tensor->producer->owner)) RemoveTensor(tensor);
  }
}

static bool AllChildVisited(Graph* graph, Node* node,
  ...
  for (unsigned int i = 0; i < node->GetOutputNum(); i++) {
    ...
    for (unsigned int k = 0; k < tensor->consumer.size(); k++) {
      NodePort* in_port = tensor->consumer[k];
      Node* child = in_port->owner;
      if (!visited[child->GetNodeIndex()]) return false;
    }
  }
  return true;
}

static bool AllInputVisited(Graph* graph, Node* node,
                            std::vector<int>& visited) {
  for (unsigned int i = 0; i < node->GetInputNum(); i++) {
    ...
    if (!visited[parent->GetNodeIndex()]) return false;
  }
  return true;
}

void Graph::BFSVisit(Graph* graph, std::vector<Node*>& starts,
                     Graph::graph_visit_t func, bool backward,
                     bool input_ready) {
  if (backward)
...
void Graph::ForwardBFS(Graph* graph, std::vector<Node*>& starts,
  ...
  for (int i = 0; i < node_number; i++) in_graph.insert(graph->seq_nodes[i]);
...
void Graph::BackwardBFS(Graph* graph, std::vector<Node*>& starts,
                        graph_visit_t func, bool input_ready) {
  int node_number = graph->seq_nodes.size();
  std::vector<int> visited(node_number, 0);
  std::queue<Node*> visit_queue;
...
void Graph::SetNodeOwner(Node* node) { owned_nodes_.push_back(node); }
...
Tensor* Graph::GetInputTensor(const std::string& name) {
  for (unsigned int i = 0; i < input_nodes.size(); i++) {
    ...
    for (unsigned int j = 0; j < node->GetInputNum(); j++) {
      ...
      if (tensor->GetName() == name) return tensor;
    }
  }
...
bool Graph::Replace(Subgraph* orig_sub, Subgraph* new_sub) {
  ...
  for (unsigned int i = 0; i < orig_input.size(); i++) {
    ...
    for (unsigned int j = 0; j < input_node->GetInputNum(); j++) {
      ...
      if (tensor->GetType() == kConstTensor) continue;

      if (new_sub->GetInputTensor(tensor->GetName()) == nullptr) return false;
    }
  }
  ...
  for (unsigned int i = 0; i < orig_output.size(); i++) {
    ...
    for (unsigned int j = 0; j < output_node->GetOutputNum(); j++) {
      ...
      if (new_sub->GetOutputTensor(tensor->GetName()) == nullptr) return false;
    }
  }
  ...
  for (unsigned int i = 0; i < new_sub->seq_nodes.size(); i++) {
    ...
    if (new_sub->RemoveNodeOwner(node)) {
      ...
      if (node->GetInputNum() == 0) {
        ...
        if (op->GetName() != "Const") graph_input_node = true;
      } else {
        for (unsigned int i = 0; i < node->GetInputNum(); i++) {
          Tensor* input_tensor = node->GetInputTensor(i);

          if (input_tensor->GetType() != kVarTensor) continue;
      ...
      if (graph_input_node) input_nodes.push_back(node);

      if (graph_output_node) output_nodes.push_back(node);
    } else {
...
bool Graph::NodeInGraph(Node* node) {
  for (unsigned int i = 0; i < seq_nodes.size(); i++) {
    if (node == seq_nodes[i]) return true;
  }
