...
std::vector<std::string> TEngineConfig::ParseKey(const std::string& key) {
  ...
  if (subkey.size() > 0) result.push_back(subkey);
...
bool GetSyncRunMode(void) {
  ...
  if (inited) return sync_run_mode;
  ...
  if (TEngineConfig::Get("exec.sync_run", value)) {
    if (value == "true" || value == "TRUE") sync_run_mode = true;
  }
...
template <>
bool ConvertSpecialAny<float>(float& entry, const std::type_info& info,
                              any& data) {
  if (info == typeid(double)) {
    entry = any_cast<double>(data);
    return true;
...
template <>
bool ConvertSpecialAny<std::string>(std::string& entry,
                                    const std::type_info& info, any& data) {
