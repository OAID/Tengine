...
bool OpManager::AddOpAttr(const std::string& op_name,
                          const std::string& attr_name, const any& val) {
  Operator* op;

  if (!SafeGet(op_name, op)) return false;
...
bool OpManager::RemoveOpAttr(const std::string& op_name,
                             const std::string& attr_name) {
  Operator* op;

  if (!SafeGet(op_name, op)) return false;

  if (!op->ExistAttr(attr_name)) return false;
...
bool OpManager::GetOpAttr(const std::string& op_name,
...
Operator* OpManager::CreateOp(const std::string& op_name) {
  ...
  if (!SafeGet(op_name, op)) return nullptr;
