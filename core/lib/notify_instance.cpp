...
NotifyBus::NotifyBus(const std::string& name, int max_event_type_num,
                     int max_event_pending_num) {
  bus_name_ = name;
  ...
  for (int i = 0; i < max_event_type_; i++) event_array_[i].event = -1;
}
...
int NotifyBus::CreateEvent(const std::string& name) {
  ...
  for (i = 0; i < max_event_type_; i++)
    if (event_array_[i].event < 0) break;
...
int NotifyBus::EventNum(const std::string& name) {
  ...
  if (ir != name_map_.end()) id = ir->second;
...
std::string NotifyBus::BusName() { return bus_name_; }
...
int NotifyBus::DropPendingEvent(void) {
  ...
  for (int i = 0; i < max_event_type_; i++) count += DropPendingEvent(i);
...
bool NotifyBus::Send(const std::string& event_name, EventData* data,
                     int priority) {
...
void NotifyBus::HandleSingleEvent(EventNode* node, EventData* data) {
  for (unsigned int i = 0; i < node->handle_list.size(); i++) {
    ...
    if (!handle.func(node->event, data, &handle.argument)) break;
  }
...
int NotifyBus::RealHandlePendingEvent(EventNode* node) {
  ...
  while (ir != node->event_tree.end()) {
    ...
    while (!event_queue.empty()) {
      ...
      if (batch_count_ > batch_budget_) return count;
    }
...
int NotifyBus::HandlePendingEvent(EventNode* node) {
  ...
  while (node->event_tree.size()) {
    ...
    if (batch_count_ >= batch_budget_) break;
  }
...
void NotifyBus::SetBatchNum(int budget) { batch_budget_ = budget; }

int NotifyBus::GetBatchNum(void) { return batch_budget_; }
...
void NotifyBus::LockNameMap() { name_map_mutex.lock(); }

void NotifyBus::UnLockNameMap() { name_map_mutex.unlock(); }

bool NotifyBus::GetNode(NotifyBus::EventNode* node) {
  node->event_mutex.lock();
  /* if it is a valid event node */

  if (node->event >= 0) return true;
...
void NotifyBus::PutNode(EventNode* node) { node->event_mutex.unlock(); }
