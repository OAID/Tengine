...
template <>
A DataType::Convert<A>(const std::string& str ) const
{
        A a(str);
        return a;
}
...
int main(void)
{
   ...
   /* Layout */
    ...
    const DataLayout * p_layout;

    p_layout=DataLayout::GetLayout("NCHW");

    std::cout<<"Layout Name: "<<p_layout->GetName()<<std::endl;
  
    std::cout<<"H is: "<<p_layout->GetH()<<std::endl;
