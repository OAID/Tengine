...
struct TEngineConfig {
  ...
  template <typename T>
  static bool Get(const std::string& key, T& value, bool show_warning = false);
...
template <typename T>
bool TEngineConfig::Set(const std::string& key, const T& value,
                        const bool create) {
  ConfManager* manager = GetConfManager();
...
#else
static inline bool GetTEngineMTMode(void) { return false; }
#endif
