...
class Operator : public BaseObject {
 public:
  ...
  using infer_shape_t =
      std::function<bool(const std::vector<TShape>&, std::vector<TShape>&)>;
  ...
  virtual bool GetParamItem(const char* param_name,
                            const std::type_info* type_info, void* val) {
    return false;
  }
  virtual bool SetParamItem(const char* param_name,
                            const std::type_info* type_info, const void* val) {
    return false;
  }

  virtual any GetDefParam(void) { return any(); }

  virtual bool InferShape(const std::vector<TShape>& ishape,
                          std::vector<TShape>& oshape) {
    oshape = ishape;
    return true;
  }

  virtual float GetFops(const std::vector<TShape>& ishape,
                        const std::vector<TShape>& oshape) {
    return 0.0f;
  }
  ...
  const std::string& GetInputDtype(int idx) const {
  ...
  const std::string& GetOutputDtype(int idx) const {
...
class OperatorWithParam : public Operator {
 public:
  ...
  P* GetParam(void) { return &param_; }

  void ParseDefParam(void) { ParseParam(param_, this); }
  ...
  bool GetParamItem(const char* param_name, const std::type_info* type_info,
                    void* val) override {
    return param_.GetItemVal(param_name, type_info, val);
  }

  bool SetParamItem(const char* param_name, const std::type_info* type_info,
