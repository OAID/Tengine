...
using exec_cb_t =
    std::function<void(GraphExecutor *, ExecEngine *, int event, int status)>;
...
struct ExecEngine {
  ...
  virtual bool SetTensorBuffer(Tensor *, void *, int,
                               exec_handle_t = nullptr) = 0;
  ...
  virtual bool SetCallback(exec_handle_t, exec_event_t &, int event,
                           exec_cb_t) = 0;
...
class ExecEngineManager
    : public SimpleObjectManagerWithLock<ExecEngineManager, ExecEnginePtr> {};
