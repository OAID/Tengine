...
struct EventBusInterface {
  ...
  struct EventStats {
    ...
    void Dump(std::ostream& os) {
      os << "event: " << event << " name: " << name
         << " booked handler: " << book_count << std::endl;
      os << "\tsend " << send_count << " sync_send " << sync_send_count
         << " send fail " << send_fail_count << "\n";
      os << "\tpending " << pending_count << " process " << process_count
         << " drop " << drop_count << "\n";
    }
  };
  ...
  virtual bool Send(const std::string& event_name, EventData* data,
                    int priority = 10) = 0;
  virtual bool SyncSend(const std::string& event_name, EventData* data) = 0;
  virtual bool Book(const std::string& event_name,
                    const EventHandle& handle) = 0;
