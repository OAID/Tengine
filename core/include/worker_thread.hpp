...
class WorkerThread {
 public:
  ...
  void PushTask(const std::vector<T>& task_list) {
    ...
    for (auto task : task_list) task_queue_->push(task);

    if (inc_req_) inc_req_(task_list.size());
  ...
  void SetQueue(std::queue<T>* task_queue, std::mutex* worker_lock,
 ...
 private:
  void DoWork(void) {
    ...
    if (bind_cpu_ >= 0) {
      ...
      if (sched_setaffinity(0, sizeof(mask), &mask) == 0) bind_done_ = true;
    }
  ...
  void GetTask(T& task) {
    ...
    if (task_queue_->empty() && !quit_work_)
      worker_cv_->wait(cv_lock,
                       [this] { return !task_queue_->empty() || quit_work_; });
