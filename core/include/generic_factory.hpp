...
class BaseFactory {
 public:
  ...
  using creator_type_t =
      std::unordered_multimap<std::string,
                              std::string>;  // saved the creator type info
  ...
  bool RegisterCreator(const std::string& name,
  ...
  T* Create(const std::string& name, Args&&... args) {
    ...
    if (it == creator_map_.end()) {
      LOG_ERROR() << "failed to find " << name
                  << " with func type: " << GetTypeName(typeid(func_t).name())
                  << "\n";
      return nullptr;
    }
 ...
 protected:
  bool RegisterCreator(const std::string& key, any creator,
                       bool replace = false) {
...
class SpecificFactory : public GenericFactory {
 public:
  ...
  T* Create(const std::string& name, Args&&... args) {
    ...
    return GenericFactory::Create<T, Args...>(name,
  ...
  RegisterInterface(const std::string& name, bool replace = false) {
    return GenericFactory::RegisterInterface<T, Derived, Args...>(name,
                                                                  replace);
  }
