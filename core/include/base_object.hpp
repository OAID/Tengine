...
class BaseObject : public Attribute {
 public:
  ...
  bool ExistAttr(const std::string& name) const override {
    if (dict_map_.count(name)) return true;

    if (ExistClassAttr(name)) return true;
  ...
  const any& GetAttr(const std::string& name) const override {
    ...
    if (ExistClassAttr(name)) return GetClassAttr(name);
...
#define REGISTER_CLASS_ATTR_OPS(parent)                                  \
  bool RealExistClassAttr(const std::string& name) const {               \
    if (ClassAttr()->ExistAttr(name)) return true;                       \
                                                                         \
    if (parent::RealExistClassAttr(name)) return true;                   \
                                                                         \
    return false;                                                        \
  }                                                                      \
  any& RealGetClassAttr(const std::string& name) {                       \
    if (ClassAttr()->ExistAttr(name)) return ClassAttr()->GetAttr(name); \
                                                                         \
