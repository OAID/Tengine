...
#if __cplusplus <= 199711L && (!defined(_MSC_VER) || _MSC_VER < 1600) && (!defined(__GNUC__) || (__GNUC__ * 10000 + __GNUC_MINOR__ * 100 + __GNUC_PATCHLEVEL__ < 40400))
#error A C++11 compatible compiler with support for the auto typing is \
         required for FlatBuffers.
#error __cplusplus _MSC_VER __GNUC__  __GNUC_MINOR__  __GNUC_PATCHLEVEL__
#endif

#if !defined(__clang__) && defined(__GNUC__) && (__GNUC__ * 10000 + __GNUC_MINOR__ * 100 + __GNUC_PATCHLEVEL__ < 40600)
...
#if (!defined(_MSC_VER) || _MSC_VER > 1600) && (!defined(__GNUC__) || (__GNUC__ * 100 + __GNUC_MINOR__ >= 407))
#define FLATBUFFERS_FINAL_CLASS final
#else
#define FLATBUFFERS_FINAL_CLASS
#endif

#if (!defined(_MSC_VER) || _MSC_VER >= 1900) && (!defined(__GNUC__) || (__GNUC__ * 100 + __GNUC_MINOR__ >= 406))
...
public:
    explicit vector_downward(size_t initial_size,
        ...
        : reserved_((initial_size + sizeof(largest_scalar_t) - 1) & ~(sizeof(largest_scalar_t) - 1)),
...
template<typename T>
const T* data(const std::vector<T>& v)
{
...
public:
    ...
    {
        TableKeyComparator(vector_downward& buf)
            : buf_(buf)
        {
        }
...
private:
    ...
    void Finish(uoffset_t root, const char* file_identifier, bool size_prefix)
    {
        ...
        PreAlign((size_prefix ? sizeof(uoffset_t) : 0) + sizeof(uoffset_t) + (file_identifier ? kFileIdentifierLength : 0),
                 minalign_);
        if (file_identifier)
        {
...
/// the pointer!
template<typename T>
T* GetMutableTemporaryPointer(FlatBufferBuilder& fbb,
                              Offset<T> offset)
{
...
inline bool BufferHasIdentifier(const void* buf, const char* identifier)
{
    return strncmp(reinterpret_cast<const char*>(buf) + sizeof(uoffset_t),
                   identifier, FlatBufferBuilder::kFileIdentifierLength)
           == 0;
}
...
public:
    ...
    template<typename T>
    bool Verify(const Vector<const T*>* vec) const
    {
    ...
    {
        const uint8_t* end;
        return !str || (VerifyVector(reinterpret_cast<const uint8_t*>(str), 1, &end) && Verify(end, 1) && // Must have terminator
    ...
    bool VerifyBufferFromStart(const char* identifier,
                               const uint8_t* start)
    {
        if (identifier && (size_t(end_ - start) < 2 * sizeof(flatbuffers::uoffset_t) || !BufferHasIdentifier(start, identifier)))
        {
            return false;
        }

        // Call T::Verify, which must be in the generated code for this type.
        return Verify<uoffset_t>(start) && reinterpret_cast<const T*>(start + ReadScalar<uoffset_t>(start))->Verify(*this)
#ifdef FLATBUFFERS_TRACK_VERIFIER_BUFFER_SIZE
               && GetComputedSize()
#endif
            ;
    ...
    {
        return Verify<uoffset_t>(buf_) && ReadScalar<uoffset_t>(buf_) == end_ - buf_ - sizeof(uoffset_t) && VerifyBufferFromStart<T>(identifier, buf_ + sizeof(uoffset_t));
    }
...
public:
    ...
    bool VerifyTableStart(Verifier& verifier) const
    {
        ...
        return verifier.VerifyComplexity() && verifier.Verify<voffset_t>(vtable) && (ReadScalar<voffset_t>(vtable) & (sizeof(voffset_t) - 1)) == 0 && verifier.Verify(vtable, ReadScalar<voffset_t>(vtable));
    }
...
inline const uint8_t* GetBufferStartFromRootPointer(const void* root)
{
         ...
         possible_roots--)
    {
        ...
        if (ReadScalar<uoffset_t>(start) + start == reinterpret_cast<const uint8_t*>(root)) return start;
    }
...
// used).
template<typename T>
bool IsFieldPresent(const T* table, voffset_t field)
{
...
volatile __attribute__((weak)) const char* flatbuffer_version_string = "FlatBuffers " FLATBUFFERS_STRING(FLATBUFFERS_VERSION_MAJOR) "." FLATBUFFERS_STRING(FLATBUFFERS_VERSION_MINOR) "." FLATBUFFERS_STRING(FLATBUFFERS_VERSION_REVISION);
