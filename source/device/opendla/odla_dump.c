...
const char* get_tensor_data_type_string_odla(int data_type)
{
    switch (data_type)
    {
    case TENGINE_DT_FP32:
...
void print_tensor_data_to_file_odla(FILE* file, const struct tensor* tensor)
{
    ...
    case 5:
    {
        ...
        else
        {
            ...
            {
                const char* type_name = get_tensor_data_type_string_odla(tensor->data_type);
                ...
                for (int ch = 0; ch < channel; ch++)
                {
                    ...
                    for (int h = 0; h < height; h++)
                    {
                        ...
                        for (int w = 0; w < width; w++)
                        {
                            ...
                            print_tensor_data_value_odla(file, tensor, offset);
                        }
                        fprintf(file, "\n");
                    }
                    fprintf(file, "\n");
    ...
    case 1:
    {
        ...
        for (int w = 0; w < width; w++)
        {
            print_tensor_data_value_odla(file, tensor, w);
        }
...
char* replace_string_character_odla(char* src_str, char* dst_str, char* target_char, char* replaced_char)
{
    char* p;
    char* _out = dst_str;
    char* _str = src_str;
...
//        }
    ...
    replace_string_character_odla(layer_short_name, layer_legal_name, "/", "-");
    ...
    print_tensor_data_to_file_odla(file, tensor);
