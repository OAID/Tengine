...
int ref_gru_default_fp32(struct tensor* input_tensor, struct tensor* w, struct tensor* r, struct tensor* output_tensor,
                         struct gru_param* param)
{
    ...
    for (int seq = 0; seq < input_tensor->dims[0]; seq++)
    {
        for (int t = 0; t < T; t++)
        {
            for (int q = 0; q < hidden_size; q++)
            {
                ...
                {
                    float x_i =
                        x_data[seq * input_tensor->dims[1] * input_tensor->dims[2] + t * input_tensor->dims[2] + m];
                ...
                for (int k = 0; k < hidden_size; k++)
                {
                    if (seq == 0)
                    {
                        r_H += r_tmp * initial_h_data[t * hidden_size + k]
                               * r_data[(hidden_size * 2 + q) * hidden_size + k];
                    }
                    else
                    {
                        r_H += r_tmp * output_h_data[t * hidden_size + k]
                               * r_data[(hidden_size * 2 + q) * hidden_size + k];
                    }
                }
...
int ref_gru_with_bias_fp32(struct tensor* input_tensor, struct tensor* w, struct tensor* r, struct tensor* b,
                           struct tensor* output_tensor, struct gru_param* param)
{
    int batch_size     = input_tensor->dims[1];
    int size           = input_tensor->dims[2];
    int hidden_size    = param->hidden_size;
...
int ref_gru_case1_fp32(struct tensor* input_tensor, struct tensor* w, struct tensor* r, struct tensor* b,
                       struct tensor* output_tensor, struct gru_param* param)
{
    int    batch_size  = input_tensor->dims[1];
    int    hidden_size = param->hidden_size;
