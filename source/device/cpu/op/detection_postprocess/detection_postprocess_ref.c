...
static int run(struct node_ops* node_ops, struct exec_node* exec_node, struct exec_graph* exec_graph)
{
    ...
    if (input_tensor->data_type == TENGINE_DT_FP32)
        ref_dpp_fp32(input_data, score_data, anchor_data, detect_num_data, detect_classes_data, detect_scores_data,
                     detect_boxes_data, &param);
    else
        ref_dpp_uint8(input_data, score_data, anchor_data, detect_num_data, detect_classes_data, detect_scores_data,
                      detect_boxes_data, &param);
