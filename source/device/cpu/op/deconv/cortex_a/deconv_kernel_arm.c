...
static void col2im(float* col, float* im, float* bias, int output_ch, int output_x, int output_y, int kernel_x,
                   int kernel_y, int stride_x, int stride_y, int dilation_x, int dilation_y, int pad_x, int pad_y,
...
static void sgemm_set(float* input, float* kernel, float* col, int in_ch, int in_hw, int kernel_size, int kernel_start,
                      int kernel_end, int num_thread, int cpu_affinity)
{
...
static void sgemm4x4(float* input, float* kernel, float* col, int in_ch, int in_hw, int kernel_size, int kernel_start,
                     int kernel_end, int num_thread, int cpu_affinity)
{
    float result[16];
...
int deconv_hcl_prerun(struct tensor* input_tensor, struct tensor* filter_tensor, struct tensor* output_tensor,
                      struct deconv_priv_info* priv_info, struct deconv_param* param)
{
    int group = param->group;
...
int deconv_hcl_run(struct tensor* input_tensor, struct tensor* filter_tensor, struct tensor* bias_tensor,
                   struct tensor* output_tensor, struct deconv_priv_info* priv_info, struct deconv_param* param,
                   int num_thread, int cpu_affinity)
{
    ...
    for (int n = 0; n < batch; n++) // batch size
    {
        for (int g = 0; g < group; g++)
        {
            ...
            /* gemm */
            sgemm_set(trans_input_buf, cur_kernel, col_buf, in_c, in_hw, kernel_size, 0, sgemm_set_num, num_thread,
                      cpu_affinity);
            if (sgemm_set_remain)
                sgemm4x4(trans_input_buf, cur_kernel, col_buf, in_c, in_hw, kernel_size, sgemm_set_num, kernel_size,
                         num_thread, cpu_affinity);
            float* cur_bias = biases_buf ? (biases_buf + g * out_c) : NULL;
            col2im(col_buf, cur_output, cur_bias, out_c, out_w, out_h, ksize, ksize, stride, stride, dilation, dilation,
                   pad, pad, in_w, in_h);
        }
    }
