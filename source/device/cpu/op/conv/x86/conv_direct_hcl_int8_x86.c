...
static void pad_int8(int8_t* input, int8_t* output, int in_h, int in_w, int out_h, int out_w, int top, int left,
                     int8_t v)
{
    int8_t* ptr    = input;
...
#pragma omp parallel for num_threads(num_thread)
    for (int i = 0; i < outch; i++)
    {
        for (int j = 0; j < outh * outw; j++)
        {
            ...
            if (bias_tensor)
                output_fp32[output_off] =
                    (float)(output_int32[output_off] + bias_int32[i]) * input_scale * kernel_scales[i];
            else
...
static int score(struct node_ops* node_ops, struct exec_graph* exec_graph, struct node* exec_node)
{
    ...
    if (group == 1 && pad_h0 == pad_h1 && pad_w0 == pad_w1 && dilation_h == 1 && dilation_w == 1 && kernel_h == 3
        && kernel_w == 3 && ((stride_h == 1 && stride_w == 1) || (stride_h == 2 && stride_w == 2)))
...
int register_conv_direct_hcl_int8_x86_op()
{
    return register_builtin_node_ops(OP_CONV, &hcl_node_ops);
}

int unregister_conv_direct_hcl_int8_x86_op()
{
