...
static int run(struct node_ops* node_ops, struct exec_node* exec_node, struct exec_graph* exec_graph)
{
    ...
    {
        if (conv_dw_run(input_tensor, weight_tensor, bias_tensor, output_tensor, conv_priv_info, conv_param, num_thread,
                        cpu_affinity)
            < 0)
        {
    ...
    {
        if (conv_dw_fp16_run(input_tensor, weight_tensor, bias_tensor, output_tensor, conv_param, num_thread,
                             cpu_affinity)
            < 0)
        {
    ...
    {
        if (conv_dw_int8_run(input_tensor, weight_tensor, bias_tensor, output_tensor, conv_priv_info, conv_param,
...
static int score(struct node_ops* node_ops, struct exec_graph* exec_graph, struct node* exec_node)
{
    ...
    if (param->group > 1 && in_c == 1 && out_c == 1 && pad_h0 == pad_h1
        && pad_w0 == pad_w1)    // caution this, todo fix.
