...
void depthwise_k3s2p1_int8_a72(int8_t* input, int8_t* kernel, int8_t* out, int* bias, long out_h, long out_w,
                               long multi, long shift, long input_w, long act_min, long act_max);
#else
void depthwise_k3s1_int8(int8_t* input, int8_t* kernel, int8_t* out, int* bias, int out_h, int out_w, int multi,
                         int shift, int input_w, int act_min, int act_max);
void depthwise_k3s2_int8(int8_t* input, int8_t* kernel, int8_t* out, int* bias, int out_h, int out_w, int multi,
                         int shift, int input_w, int act_min, int act_max);
#endif
...
#pragma omp parallel for num_threads(num_thread)
    for (int i = 0; i < channel_num; i++)
    {
        ...
        {
#ifdef __aarch64__
            depthwise_k3s1p1_int8_a72(input_tmp, weight_buf + 9 * i, output_buf + i * output_h * output_w, bias_tmp,
                                      output_h, output_w, p_multi[i], p_shift[i], input_w, activation_min,
                                      activation_max);
#else
            depthwise_k3s1_int8(input_tmp, weight_buf + 9 * i, output_buf + i * output_h * output_w, bias_tmp, output_h,
                                output_w, p_multi[i], p_shift[i], input_w_pad, activation_min, activation_max);
#endif
        }
        else if (2 == stride)
        {
#ifdef __aarch64__
            depthwise_k3s2p1_int8_a72(input_tmp, weight_buf + 9 * i, output_buf + i * output_h * output_w, bias_tmp,
                                      output_h, output_w, p_multi[i], p_shift[i], input_w, activation_min,
                                      activation_max);
#else
            depthwise_k3s2_int8(input_tmp, weight_buf + 9 * i, output_buf + i * output_h * output_w, bias_tmp, output_h,
                                output_w, p_multi[i], p_shift[i], input_w_pad, activation_min, activation_max);
#endif
        }
    }
                     ...
                     int num_thread, int cpu_affinity)
{
    ...
    for (int n = 0; n < batch; n++) // batch size
    {
        ...
        conv_dw_int8_direct(input, kernel, output, biases_buf, in_h, in_w, out_h, out_w, in_c * group, stride_h, pads,
                            multi, q_shift, activation_min, activation_max, num_thread, cpu_affinity);
    }
    return 0;
}
