...
void hgemm_4x16_a76(__fp16* biases, __fp16* input, __fp16* kernel, long kernel_size, __fp16* output, long output_xy,
                    long fused_relu);
void hgemm_4x4_a76(__fp16* biases, __fp16* input, __fp16* kernel, long kernel_size, __fp16* output, long output_xy,
                   long fused_relu);
...
static void hgemm_set(__fp16* col, __fp16* kernel, __fp16* biases, __fp16* output, int kernel_size, int ch_start,
                      int ch_end, int output_xy, int relu_fused, int num_thread, int cpu_affinity)
{
...
static void hgemm4x4(__fp16* col, __fp16* kernel, __fp16* biases, __fp16* output, int kernel_size, int ch_start,
                     int ch_end, int output_xy, int relu_fused, int num_thread, int cpu_affinity)
{
    __fp16  result[16];
...
int fp16_conv_hcl_get_shared_mem_size(struct tensor* input, struct tensor* output, struct conv_param* param)
{
    int group       = param->group;
...
int fp16_conv_hcl_prerun(struct tensor* input_tensor, struct tensor* filter_tensor, struct tensor* output_tensor,
...
int fp16_conv_hcl_run(struct tensor* input_tensor, struct tensor* filter_tensor, struct tensor* bias_tensor,
    ...
    for (int n = 0; n < batch; n++)    // batch size
    {
        for (int g = 0; g < group; g++)
        {
            ...
            im2col(cur_input, col_buf, in_c, in_w, in_h, kernel_w, kernel_h, stride_w, stride_h, dilation_w, dilation_h,
                   pad_w0, pad_w1, pad_h0, pad_h1, out_w, out_h, 0, out_hw);
            ...
            hgemm_set(col_buf, cur_kernel, cur_bias, cur_output, kernel_size, 0, sgemm_set_chan, out_hw, fused_relu,
                      num_thread, cpu_affinity);
            if (sgemm_set_remain)
            {
                hgemm4x4(col_buf, cur_kernel, cur_bias, cur_output, kernel_size, sgemm_set_chan, out_c, out_hw,
                         fused_relu, num_thread, cpu_affinity);
            }
        }
    }
