...
int ref_interp_fp32(struct tensor* input_tensor, struct tensor* output_tensor, struct interp_param* param)
{
    ...
    else if (param->resize_type == 2)
    {
        ...
        {
            resize_bilinear_image(input + in_channel_size * q, output + out_channel_size * q, alpha, xofs, beta, yofs,
...
int ref_interp_uint8(struct tensor* input_tensor, struct tensor* output_tensor, struct interp_param* param)
{
    ...
    else if (param->resize_type == 2)
    {
        ...
        for (int q = 0; q < channel; ++q)
        {
            resize_bilinear_image(input_fp32 + in_channel_size * q, output_fp32 + out_channel_size * q, alpha, xofs,
                                  beta, yofs, out_h, out_w, in_h, in_w);
        }
