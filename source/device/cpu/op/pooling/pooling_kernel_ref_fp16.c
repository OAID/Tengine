        ...
        for (int c = 0; c < channel; c++)
        {
            for (int ph = 0; ph < out_h; ph++)
            {
                for (int pw = 0; pw < out_w; pw++)
                {
                    ...
                    {
                        fp16_t max;
                        calc_max_fp16(input_cur, &max, layout, channel, in_h, in_w, c, h_start, w_start, h_end, w_end);
                        output[offset] = max;
                    }
                    else if (method == 1)
                    {
                        fp16_t sum;
                        calc_sum_fp16(input_cur, &sum, layout, channel, in_h, in_w, c, h_start, w_start, h_end, w_end);
