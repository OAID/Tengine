...
static int innerproduct(int inn, int inc, int inh, int inw, int outc, const float* weight, const float* input,
                        float* output, const float* _bias, int num_thread, int cpu_affinity)
{
...
static int run(struct node_ops* node_ops, struct exec_node* exec_node, struct exec_graph* exec_graph)
{
    ...
    if (innerproduct(batch_number, inc, inh, inw, outc, weight_data, input_data, output_data, bias_data, num_thread,
                     cpu_affinity)
        < 0)
        return -1;
