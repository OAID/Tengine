...
static int run(struct node_ops* node_ops, struct exec_node* exec_node, struct exec_graph* exec_graph)
{
    ...
    {
        if (fp16_fc_kernel_run(input_tensor, weight_tensor, bias_tensor, output_tensor, priv_info, fc_param, num_thread,
                               cpu_affinity)
            < 0)
        {
    ...
    {
        if (int8_fc_kernel_run(input_tensor, weight_tensor, bias_tensor, output_tensor, priv_info, fc_param, num_thread,
                               cpu_affinity)
            < 0)
        {
            TLOG_ERR("hcl fc run failed\n");
