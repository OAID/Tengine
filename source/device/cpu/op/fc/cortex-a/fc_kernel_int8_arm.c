...
void gemv_1x8_int8(int32_t* biases, const float* scales, int8_t* inp, int8_t* kernel, long kernel_size, int8_t* output)
{
    int8x8_t input;
                   ...
                   int8_t* output)
{
    ...
    for (int i = 0; i < remainw; i = i + 8)
    {
        ...
        sum0 += vgetq_lane_s32(out_32_0, 0) + vgetq_lane_s32(out_32_0, 1) + vgetq_lane_s32(out_32_0, 2)
                + vgetq_lane_s32(out_32_0, 3);
        sum1 += vgetq_lane_s32(out_32_1, 0) + vgetq_lane_s32(out_32_1, 1) + vgetq_lane_s32(out_32_1, 2)
                + vgetq_lane_s32(out_32_1, 3);
        weight_ptr += 16;
        input_ptr += 8;
    }
...
void gemv1x8(const int8_t* input, const int8_t* output, int8_t* weight_interleaved, const int32_t* biases,
             const float* scales, int kernel_size, int start_channel, int end_channel, int num_thread, int cpu_affinity)
{
    ...
    for (ch = start_channel; ch < end_channel; ch += 8)
    {
        ...
        gemv_1x8_int8(cur_biases, cur_scales, (int8_t*)input, cur_kernel, kernel_size, cur_result);
    }
}

// start channel must be 2 aligned
void gemv1x2(const int8_t* input, int8_t* output, int8_t* weight_interleaved, const int32_t* biases,
             const float* scales, int kernel_size, int start_channel, int end_channel, int num_thread, int cpu_affinity)
{
    int32_t sum;
    int ch = 0;
...
int int8_fc_kernel_prerun(struct tensor* input_tensor, struct tensor* filter_tensor, struct tensor* output_tensor,
                          struct fc_priv_info* priv_info, struct fc_param* param)
{
    ...
    interleave_kernel(filter_data, (int8_t*)priv_info->interleave_buffer, num_output, kernel_size);

    return 0;
}

int int8_fc_kernel_run(struct tensor* input_tensor, struct tensor* filter_tensor, struct tensor* bias_tensor,
                       ...
                       int num_thread, int cpu_affinity)
{
    ...
    for (int i = 0; i < input_tensor->dims[0]; i++)
    {
        ...
        gemv1x8(cur_input, cur_output, weight, biases, requant_scales, kernel_size, 0, out_num_8, num_thread,
                cpu_affinity);
        if (out_num & 0x7)
            gemv1x2(cur_input, cur_output, weight, biases, requant_scales, kernel_size, out_num_8, out_num, num_thread,
                    cpu_affinity);
    }
