...
static int ref_logsoftmax_fp32(float* input_data, float* output_data, float* max_array, float* sum_array,
                               struct ref_logsoftmax_param* op_param)
{
    for (int i = 0; i < op_param->out_size; i++)
    {
        ...
        GetOutResult(input_data + img_base, output_data + img_base, max_array, sum_array, op_param->in_size,
                     op_param->on_size);
    }
