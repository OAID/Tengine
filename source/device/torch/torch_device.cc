...
int torch_describe(struct device* device, struct vector* allowed_ops, struct vector* blocked_ops, struct vector* precision)
{
    (void)device;

    for (int op_type : torch_supported_ops)
    {
    ...
    for (int i = 0; i < OP_BUILTIN_LAST; i++)
    {
        ...
        for (const auto& type : torch_supported_ops)
        {
            if (type == i)
            {
                in_list = true;
...
int torch_evaluation(struct device* device, struct subgraph* sub_graph, struct vector* evolution_tensors, struct vector* evolution_nodes)
{
...
int torch_allocate(struct device* device, struct subgraph* sub_graph)
{
    if (nullptr == device)
    {
        return -1;
...
int torch_release(struct device* device, struct subgraph* sub_graph)
{
    (void)sub_graph;
...
int torch_split_graph(struct graph* ir_graph)
{
    struct device* cur_dev = ir_graph->attribute->context->device;

    if (0 != strcmp(TORCH_DEV_NAME, cur_dev->name))
    {
        return -1;
    }
...
extern "C"
{
static struct interface torch_interface = {
        .init           = torch_dev_init,
        .pre_run        = torch_dev_prerun,
        .run            = torch_dev_run,
        .post_run       = torch_dev_postrun,
        .async_run      = nullptr,
        .async_wait     = nullptr,
        .release_graph  = nullptr,
        .release_device = torch_dev_release,
};


static struct allocator torch_allocator = {
        .describe       = torch_describe,
...
static struct optimizer torch_optimizer = {
        .split_graph    = torch_split_graph,
        .optimize_graph = nullptr,
};



static struct torch_device torch_dev = {
    .base = {
            .name       = TORCH_DEV_NAME,
            .interface  = &torch_interface,
            .allocator  = &torch_allocator,
...
int register_torch_device(void)
{
    int ret = register_device(&torch_dev.base);
    if (0 != ret)
    {
...
int unregister_torch_device(void)
{
    int ret = unregister_device(&torch_dev.base);
    if (0 != ret)
    {
