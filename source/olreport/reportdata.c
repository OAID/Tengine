...
void init_report_data(const char* tengine_key, const char* tengine_key_token, const char* appid, const char* app_key,
                      const char* tengine_version, const char* hcl_version, const char* api_version,
...
const char* general_report_data(int* len, int action)
{
    ...
    offset = snprintf(app_data, MAX_REPORT_DATA_LEN, "%s=%s&%s=%s&%s=%s&%s=%u%s", APPID, gsData.data_.APPID_,
                      METHOD_NAME, gsData.REQ_URL, PARAM, json_data, TIMESTAMP, tm, gsData.app_key_);
    ...
    *len = snprintf(gsData.report_dat_, 1024 * 10, "{\"%s\":\"%s\",\"%s\":\"%s\",\"%s\":%s,\"%s\":%u,\"%s\":\"%s\"}",
                    APPID, gsData.data_.APPID_, METHOD_NAME, gsData.REQ_URL, PARAM, json_data, TIMESTAMP, tm, SIGN,
                    tengine_md5_str);
    if (*len > 1024 * 10)
    {
        *len = 1024 * 10;
