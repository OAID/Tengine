    ...
    for (int ii = 0; ii < nums; ++ii, ++if_req)
    {
        ...
        if (if_req->ifr_flags == AF_INET || if_req->ifr_flags == AF_INET6)
        {
            ...
            if (ioctl(fd, SIOCGIFHWADDR, &ifr_mac) == 0)
            {
                ...
                sprintf(mac, "%02x-%02x-%02x-%02x-%02x-%02x", temp_str[0] & 0xff, temp_str[1] & 0xff,
                        temp_str[2] & 0xff, temp_str[3] & 0xff, temp_str[4] & 0xff, temp_str[5] & 0xff);
            }
    ...
    for (ifa = if_addr_struct; ifa != NULL; ifa = ifa->ifa_next)
    {
        ...
        if (ifa->ifa_addr->sa_family == AF_INET)
        {
            ...
            if( ioctl(fd,SIOCGIFHWADDR,&ifr) == 0 )
            {
                ...
                sprintf(mac, "%02x-%02x-%02x-%02x-%02x-%02x", temp_str[0]&0xff, temp_str[1]&0xff, temp_str[2]&0xff,
