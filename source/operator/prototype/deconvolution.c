...
static int infer_shape(struct node* node)
{
    ...
    if (deconv_param->stride_w == 1)
        kernel_extent_w = deconv_param->dilation_w * (deconv_param->kernel_w - 1);
    else
        kernel_extent_w = deconv_param->dilation_w * (deconv_param->kernel_w - 1) + 1;

    if (deconv_param->stride_h == 1)
        kernel_extent_h = deconv_param->dilation_h * (deconv_param->kernel_h - 1);
    else
