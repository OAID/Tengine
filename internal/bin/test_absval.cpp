...
int create_input_node(graph_t graph, const char* node_name, int n, int c, int h, int w, int data_type)
{
...
{
    node_t test_node = create_graph_node(graph, node_name, "Absval");
...
graph_t create_test_graph(const char* test_node_name, int n, int c, int h, int w, int layout, int data_type)
{
    ...
    if(create_input_node(graph, input_name, n, c, h, w, data_type) < 0)
    {
...
void dump_output_data(node_t test_node)
{
    ...
    printf("output shape: %d %d %d %d ", dims[0],dims[1],dims[2],dims[3]);
...
int main(int argc, char* argv[])
{
    ...
    init_tengine();
    graph_t graph = create_test_graph(test_node_name,n, c, h, w, layout, data_type);
    ...
    FILE *infp;  
    infp=fopen("./data/absval_in.bin","rb");
    ...
    infp=fopen("./data/absval_out.bin","rb");
