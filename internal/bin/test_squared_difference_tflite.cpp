...
int main(int argc, char* argv[])
{
    ...
    const char* model_file = "./models/test_squared_difference.tflite";
    ...
    set_tensor_shape(input_tensor2, dims, 4);
    ...
    float* input_data2 = ( float* )malloc(sizeof(float) * 3 * 3 * 3 * 1) ;
    for(int i =0; i < 3 * 3 * 3; i ++)
    {
        input_data2[i] = 5;
    }
    ...
    if(set_tensor_buffer(input_tensor2, input_data2, 3 * 3 * 3 * 4) < 0)
    {
        std::printf("Set buffer for tensor failed\n");
    }
