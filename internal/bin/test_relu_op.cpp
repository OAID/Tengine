...
graph_t create_test_graph(const char* test_node_name, int h, int w, int layout, int data_type, float negative_slope)
{
...
int main(int argc, char* argv[])
{
    ...
    while((res = getopt(argc, argv, "h:w:l:t:s")) != -1)
    {
        switch(res)
        {
            ...
            case 't':
                ...
                break;
            case 's':
                negative_slope = strtoul(optarg, NULL, 0.0);
                break;
    ...
    graph_t graph = create_test_graph(test_node_name, h, w, layout, data_type, negative_slope);
