...
float biases[18] = {10, 10, 20, 21, 39, 29, 28, 55, 66, 48, 49, 109, 96, 75, 246, 37, 132, 11};
...
void preprocess_yolov2(std::string& image_file, float* input_data, int img_h, int img_w, int* raw_h, int* raw_w)
{
    ...
    *raw_w = img.w;
    ...
    int new_w = img.w;
    int new_h = img.h;
    if((( float )img_w / img.w) < (( float )img_h / img.h))
    {
        new_w = img_w;
        new_h = (img.h * img_w) / img.w;
    }
    else
    {
        new_h = img_h;
        new_w = (img.w * img_h) / img.h;
    }

    for(int i = 0; i < 3 * img.h * img.w; i++)
        img.data[i] = img.data[i] * 0.00392156862745098f;

    image resImg = resize_image(img, new_w, new_h);

    int delta_h = (img_h - new_h) * 0.5f;
    int delta_w = (img_w - new_w) * 0.5f;
    image dst_img = copyMaker(resImg, delta_h, delta_h, delta_w, delta_w, 0.5);
...
int main(int argc, char* argv[])
{
    ...
    std::string image_file = "./tests/images/liandao.jpg";

    int net_w = 416;
    int net_h = 416;

    float* input_data = ( float* )malloc(sizeof(float) * net_w * net_h * 3);
    // preprocess_yolov2(image_file, input_data, net_h, net_w, &img_h, &img_w);

    int size = 3 * net_w * net_h;
    image sized;
    // im = load_image_color(( char* )image_file.c_str(), 0, 0);
    image im = imread(image_file.c_str());
    sized = letterbox_image(im, net_w, net_h);
    ...
    for(int i = 0; i < output_node_num; ++i)
    {
        ...
        int out_w = out_dim[3];
        int out_h = out_dim[2];
        l_params = make_yolo_layer(1, out_w, out_h, net_w, net_h, numBBoxes, yolov3_numAnchors, classes);
        layers_params.push_back(l_params);
        float* out_data = ( float* )get_tensor_buffer(out_tensor);
        forward_yolo_layer_cpu(out_data, l_params);
    }
    int nboxes = 0;
    // get network boxes
    detection* dets =
    ...
    save_image(img, "caffe-yolo3_test.jpg");
