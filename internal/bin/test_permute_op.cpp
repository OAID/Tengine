...
int create_test_node(graph_t graph, const char* node_name, int order, const char* inputs[], int input_counts)
{
    std::cout << "create_ggraph_node--" << order << "\n";
    node_t test_node = create_graph_node(graph, node_name, "Permute");
    ...
    std::cout << "orders:" << order0 << "-" << order1 << "-" << order2 << "-" << order3 << "\n";
    ...
    set_node_attr_int(test_node, "order0", &order0);
    set_node_attr_int(test_node, "order1", &order1);
    set_node_attr_int(test_node, "order2", &order2);
    set_node_attr_int(test_node, "order3", &order3);
...
graph_t create_test_graph(const char* test_node_name, const std::vector<ConcatInputParam>& inputParam, int layout,
                          int data_type)
{
    ...
    create_test_node(graph, test_node_name, layout, gInputName, ( int )inputParam.size());
...
int comp_res(node_t out_1, node_t out_2, int data_type)
{
    ...
    else if(data_type == TENGINE_DT_UINT8)
    {
        ...
        for(int ii = 0; ii < num; ++ii)
        {
            if(!comp_uint8(pOut1[ii], pOut2[ii], 1.0f, 0))
            {
                std::cout << "---------------------uint8 not match--------------------------\n";
                return 1;
            }
...
int test_concat(const char* test_node_name, const std::vector<ConcatInputParam>& inputParam, int layout, int data_type)
{
    graph_t graph = create_test_graph(test_node_name, inputParam, layout, data_type);
    graph_t graph1 = create_test_graph(test_node_name, inputParam, layout, 0);
...
void test(int layout, int data_type, CVSResOut& rResFile)
{
    const char* test_node_name = "test_permute_op";
    std::vector<ConcatInputParam> vInput;
    printf("test Permute layout : %d. dataType : %d\n", layout, data_type);
    __PUSH_PARAM__(2, 3, 2, 2);
    if(0 == test_concat(test_node_name, vInput, layout, data_type))
    {
        printf("Layout[%d]-[%s]--------------------match-------------------\n", layout, gTypeName[data_type]);
        rResFile.dump_res(gTypeName[data_type], layout, "match");
    }
    else
    {
        printf("Layout[%d]-[%s]--------------------match-------------------\n", layout, gTypeName[data_type]);
        rResFile.dump_res(gTypeName[data_type], layout, "not match");
    }
}
...
int main(int argc, char* argv[])
{
    ...
    const char* pTitle[] = {"DatType", "Layout", "Res"};
    CVSResOut ResOut("permute_ref_test.cvs", pTitle, 3);

    test(layout, data_type, ResOut);
