...
int create_test_node(graph_t graph, const char* node_name, const char* input_name, int data_type, float alpha, float beta)
{
    node_t test_node = create_graph_node(graph, node_name, "Hardswish");
    ...
    set_node_attr_float(test_node, "beta", &beta);
    release_graph_node(test_node);

    return 0;
}

graph_t create_test_graph(const char* test_node_name, int c, int h, int w, int layout, int data_type, float alpha, float beta)
{
    ...
    if(create_test_node(graph, test_node_name, input_name, data_type, alpha, beta) < 0)
    {
...
int test_op(int c, int h, int w, const char* test_node_name, int layout, int data_type, float alpha, float beta)
{
    graph_t graph = create_test_graph(test_node_name, c, h, w, layout, data_type, alpha, beta);
    ...
    FILE *infp;  
    infp=fopen("./data/hardswish_in.bin","rb");
    ...
    infp=fopen("./data/hardswish_out.bin","rb");
...
{
    int c = 4, h = 200, w = 200;
    const char* test_node_name = "hardswish";
    int data_type = TENGINE_DT_FP32;
    int layout = TENGINE_LAYOUT_NCHW;
    float alpha = 0.2, beta = 0.5;

    init_tengine();
    
    test_op(c, h, w, test_node_name, layout, data_type, alpha, beta);
