...
int create_inputname_vect(const char** inputnamesvec)
{
    //int count = 0;
    ...
    for(int i = 0; i < 24; i++)
    {
        if(inputIfenable[i])
        {
	    vTmp.push_back(inputTensorNames[i]);
            //inputnamesvec[count] = inputTensorNames[i];
            //count += 1;
        }
    }
    int nums = (int)vTmp.size();
    *inputnamesvec = (const char*)malloc(nums);
    for(int ii=0 ;ii<nums;++ii)
    {
	    inputnamesvec[ii] = vTmp[ii];
    }
    return nums;
}
                          ...
                          int cell_size, int output_size, int sequence_size, int layout, int data_type)
{
    ...
    //const char* inputs=inputTensorNames;

    const char* outputs[] = {test_node_name};
    
    int counts = create_inputname_vect(&inputs);
    ...
    if(set_graph_input_node(graph, &inputs, counts) < 0)
    {
