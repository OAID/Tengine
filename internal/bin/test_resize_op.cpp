...
{
    node_t test_node = create_graph_node(graph, node_name, "Resize");

    const int type = 1;
    set_node_attr_int(test_node, "type", &type);
    const float scale_w = 0.4;
    const float scale_h = 0.4;
    set_node_attr_float(test_node, "scale_w", &scale_w);
    set_node_attr_float(test_node, "scale_h", &scale_h);
...
// {
//     node_t test_node = get_graph_node(graph, "test");
