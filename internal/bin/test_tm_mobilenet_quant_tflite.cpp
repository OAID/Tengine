...
#define DEF_MODEL "models/mobilenet_quant_tflite.tmfile"
...
void get_input_data(const char* image_file, uint8_t* input_data, int img_h, int img_w)
{
    ...
    for(int c = 0; c < 3; c++)
    {
        for(int h = 0; h < img_h; h++)
        {
            ...
            {
                input_data[index] = ( uint8_t )resImg.data[h * img_w * 3 + w * 3 + c];
                index++;
            }
        }
    }
...
{
    uint8_t* predictions = ( uint8_t* )get_tensor_buffer(tensor);
    float scale = 0;
    int zero_point = 0;
    get_tensor_quant_param(tensor, &scale, &zero_point, 1);
    std::cout << "output scale: " << scale << " , zero: " << zero_point << "\n";
    ...
    for(int i = 0; i < 1001; i++)
        result.push_back((predictions[i] - zero_point) * scale);
