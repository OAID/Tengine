...
int create_test_node(graph_t graph, const char* node_name, int axis, int dim0, int dim1, int dim2, int dim3,
                     float const_pad, const char* input_name)
{
    node_t test_node = create_graph_node(graph, node_name, "Pad");

    const int c_mode = axis;
    set_node_attr_int(test_node, "mode", &c_mode);

    const int c_no_pad = 0;

    set_node_attr_int(test_node, "pad_0_h", &c_no_pad);
    set_node_attr_int(test_node, "pad_0_w", &c_no_pad);
    set_node_attr_int(test_node, "pad_3_h", &c_no_pad);
    set_node_attr_int(test_node, "pad_3_w", &c_no_pad);

    const int c_h1_pad = dim0;
    const int c_h2_pad = dim1;
    const int c_w1_pad = dim2;
    const int c_w2_pad = dim3;
    set_node_attr_int(test_node, "pad_1_h", &c_h1_pad);
    set_node_attr_int(test_node, "pad_1_w", &c_h2_pad);
    set_node_attr_int(test_node, "pad_2_h", &c_w1_pad);
    set_node_attr_int(test_node, "pad_2_w", &c_w2_pad);

    if(c_mode == 0)
    {
        const float c_v = const_pad;
        set_node_attr_float(test_node, "value", &c_v);
    }
...
graph_t create_test_graph(const char* test_node_name, int axis, int dim0, int dim1, int dim2, int dim3, float const_pad,
                          int c, int h, int w, int layout, int data_type)
{
    ...
    if(create_test_node(graph, test_node_name, axis, dim0, dim1, dim2, dim3, const_pad, input_name) < 0)
    {
...
int test(const char* test_node_name, int c, int h, int w, int layout, int axis, int dim0, int dim1, int dim2, int dim3,
         float const_pad, int data_type)
{
    std::cout << "\n\n-----------------------------------------mode : " << axis << "  data_type: " << data_type
              << " pads_conf: " << dim0 << "," << dim1 << "," << dim2 << "," << dim3
              << "----------------------------------\n\n";
    graph_t graph =
        create_test_graph(test_node_name, axis, dim0, dim1, dim2, dim3, const_pad, c, h, w, layout, data_type);
...
int main(int argc, char* argv[])
{
    ...
    test(test_node_name, 2, 2, 3, TENGINE_LAYOUT_NHWC, 0, 1, 1, 1, 1, 0, 0);
    test(test_node_name, 2, 2, 3, TENGINE_LAYOUT_NHWC, 0, 1, 1, 1, 1, 0, 1);
    test(test_node_name, 2, 2, 3, TENGINE_LAYOUT_NHWC, 0, 1, 1, 1, 1, 0, 2);
    test(test_node_name, 2, 2, 3, TENGINE_LAYOUT_NHWC, 0, 1, 1, 1, 1, 0, 3);

    test(test_node_name, 2, 2, 3, TENGINE_LAYOUT_NHWC, 1, 1, 1, 1, 1, 0, 0);
    test(test_node_name, 2, 2, 3, TENGINE_LAYOUT_NHWC, 1, 1, 1, 1, 1, 0, 1);
    test(test_node_name, 2, 2, 3, TENGINE_LAYOUT_NHWC, 1, 1, 1, 1, 1, 0, 2);
    test(test_node_name, 2, 2, 3, TENGINE_LAYOUT_NHWC, 1, 1, 1, 1, 1, 0, 3);

    test(test_node_name, 2, 2, 3, TENGINE_LAYOUT_NHWC, 2, 1, 1, 1, 1, 0, 3);
    test(test_node_name, 2, 2, 3, TENGINE_LAYOUT_NHWC, 2, 1, 1, 1, 1, 0, 1);
    test(test_node_name, 2, 2, 3, TENGINE_LAYOUT_NHWC, 2, 1, 1, 1, 1, 0, 2);
    test(test_node_name, 2, 2, 3, TENGINE_LAYOUT_NHWC, 2, 1, 1, 1, 1, 0, 0);
