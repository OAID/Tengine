...
static bool LoadCaffeReshape(StaticGraph* graph, StaticNode* node, const te_caffe::LayerParameter& layer_param)
{
    ...
    // dims
    if(caffe_param.shape().dim_size() == 4)
    {
        param.dim_0 = caffe_param.shape().dim(0);
        param.dim_1 = caffe_param.shape().dim(1);
        param.dim_2 = caffe_param.shape().dim(2);
        param.dim_3 = caffe_param.shape().dim(3);
    }
