...
bool TmSerializer::SaveModel(const std::vector<std::string>& file_list, Graph *graph)
{
    ...
    /* Mmap the file */
    void * const start_ptr = mmap(NULL, TM_FILE_MAX_SIZE, PROT_WRITE, MAP_SHARED, fd, 0);
...
bool TmSerializer::LoadNode(StaticGraph *graph, StaticNode *node, const TM_Node *tm_node)
{
    if(tm_node->offset_vi_input_tensors != NOT_SET)
    {
        const TM_Vector_indices *v_input_tensors = GetTmPtr<TM_Vector_indices>(mmap_buf_, tm_node->offset_vi_input_tensors);
    ...
    {
        const TM_Vector_indices *v_output_tensors = GetTmPtr<TM_Vector_indices>(mmap_buf_, tm_node->offset_vi_output_tensors);
...
bool TmSerializer::LoadTensor(StaticGraph *graph, const TM_Tensor *tm_tensor, const TM_Buffer *tm_buf)
{
    /* Set the tensor name */
    ...
    {
        const TM_Vector_dims *v_dims = GetTmPtr<TM_Vector_dims>(mmap_buf_, tm_tensor->offset_vd_dims);
...
{
    const TM_Vector_offsets *v_graphs = GetTmPtr<TM_Vector_offsets>(mmap_buf_, tm_model->offset_vo_subgraphs);
    const TM_Subgraph *tm_graph = GetTmPtr<TM_Subgraph>(mmap_buf_, v_graphs->offsets[0]);

    const TM_Vector_offsets *v_nodes = GetTmPtr<TM_Vector_offsets>(mmap_buf_, tm_graph->offset_vo_seq_nodes);
    const TM_Vector_offsets *v_tensors = GetTmPtr<TM_Vector_offsets>(mmap_buf_, tm_graph->offset_vo_tensors);
    const TM_Vector_offsets *v_buffers = GetTmPtr<TM_Vector_offsets>(mmap_buf_, tm_graph->offset_vo_buffers);

    /* Load const tensors */
    for(unsigned int i=0; i < v_tensors->v_num; i++)
    {
        const TM_Tensor *tm_tensor = GetTmPtr<TM_Tensor>(mmap_buf_, v_tensors->offsets[i]);
        const TM_Buffer *tm_buf;
        if(tm_tensor->type == kConstTensor)
            tm_buf = GetTmPtr<TM_Buffer>(mmap_buf_, v_buffers->offsets[tm_tensor->buffer_id]);
        else
            tm_buf = nullptr;
    ...
    for(i=0; i < v_nodes->v_num; i++)
    {
        ...
        if(!LoadNode(graph, node, tm_node))
              break;

        op_load_t op_func = any_cast<op_load_t>(GetOpLoadMethod(tm_op_name));

        if(!op_func(graph, node, mmap_buf_, tm_operator))
            break;
    ...
    const TM_Vector_indices *v_input_nodes = GetTmPtr<TM_Vector_indices>(mmap_buf_, tm_graph->offset_vi_input_indices);
    const TM_Vector_indices *v_output_nodes = GetTmPtr<TM_Vector_indices>(mmap_buf_, tm_graph->offset_vi_output_indices);
