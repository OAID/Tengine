...
bool TmSerializer::LoadNode(StaticGraph *graph, StaticNode *node, const TM_Node *tm_node, void * mmap_buf)
{
	...
	{
		const TM_Vector_indices *v_input_tensors = GetTmPtr<TM_Vector_indices>(mmap_buf, tm_node->offset_vi_input_tensors);
	...
	{
		const TM_Vector_indices *v_output_tensors = GetTmPtr<TM_Vector_indices>(mmap_buf, tm_node->offset_vi_output_tensors);
...
bool TmSerializer::LoadModel(const std::vector<const void *>&addr_list, 
		const std::vector<int>& size_list, StaticGraph * graph)
{
