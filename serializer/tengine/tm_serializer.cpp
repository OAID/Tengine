...
tm_uoffset_t TmSerializer::SaveTmTensor(void *const start_ptr,
                                        tm_uoffset_t *cur_pos, Tensor *tensor,
  ...
  if (tm_with_string_) {
    ...
    tensor_name.offset_data =
        WriteTmFileAlign1(start_ptr, cur_pos, name.c_str(), tensor_name.size);
    tm_tensor.offset_s_tname =
        WriteTmObject(start_ptr, cur_pos, &tensor_name, sizeof(TM_String));
  } else
  ...
  if (dim.size()) {
    ...
    tm_tensor.offset_vd_dims =
        WriteTmObject(start_ptr, cur_pos, v_dims, vector_size);
    free(v_dims);
  } else
...
tm_uoffset_t TmSerializer::SaveTmNode(void *const start_ptr,
                                      tm_uoffset_t *cur_pos, Node *node,
  ...
  if (tm_with_string_) {
    ...
    node_name.offset_data =
        WriteTmFileAlign1(start_ptr, cur_pos, name.c_str(), node_name.size);
    tm_node.offset_s_nname =
        WriteTmObject(start_ptr, cur_pos, &node_name, sizeof(TM_String));
  } else
  ...
  if (input_num) {
    ...
    TM_Vector_indices *v_input_indices =
        (TM_Vector_indices *)malloc(vector_size);
    ...
    tm_node.offset_vi_input_tensors =
        WriteTmObject(start_ptr, cur_pos, v_input_indices, vector_size);
  ...
  if (output_num) {
    ...
    TM_Vector_indices *v_output_indices =
        (TM_Vector_indices *)malloc(vector_size);
    ...
    tm_node.offset_vi_output_tensors =
        WriteTmObject(start_ptr, cur_pos, v_output_indices, vector_size);
...
tm_uoffset_t TmSerializer::SaveTmSubgraph(void *const start_ptr,
                                          tm_uoffset_t *cur_pos, Graph *graph) {
  ...
  /* Write the nodes */
  size_t vector_size =
      sizeof(tm_size_t) + sizeof(tm_uoffset_t) * graph->seq_nodes.size();
  ...
  for (unsigned int i = 0; i < graph->seq_nodes.size(); i++) {
    ...
    v_nodes->offsets[i] =
        SaveTmNode(start_ptr, cur_pos, p_node, tensor_name_map);
  }
  /* Write the vector of nodes */
  tm_subgraph.offset_vo_seq_nodes =
      WriteTmObject(start_ptr, cur_pos, v_nodes, vector_size);
  ...
  for (unsigned int i = 0; i < tensor_num; i++) {
    ...
    v_tensors->offsets[i] =
        SaveTmTensor(start_ptr, cur_pos, p_tensor, i, buffer_num - 1);
  }
  /* Write the vector of tensors */
  tm_subgraph.offset_vo_tensors =
      WriteTmObject(start_ptr, cur_pos, v_tensors, vector_size);
  ...
  for (unsigned int i = 0; i < buffer_num; i++) {
    ...
    if (tm_no_data_) {
      /* TM_FOR_BENCHMARK environment variable exists. Not write buf data into
       * the tm file */
      tm_buf.offset_data = NOT_SET;
    } else {
      /* TM_FOR_BENCHMARK environment variable does not exist */
      tm_buf.offset_data = WriteTmFileAlign1(
          start_ptr, cur_pos, reinterpret_cast<const uint8_t *>(buf_ptrs[i]),
          tm_buf.size);
    }
    v_buffers->offsets[i] =
        WriteTmObject(start_ptr, cur_pos, &tm_buf, sizeof(TM_Buffer));
  }
  /* Write the vector of buffers */
  tm_subgraph.offset_vo_buffers =
      WriteTmObject(start_ptr, cur_pos, v_buffers, vector_size);

  /* Write the vector of input indices */
  vector_size =
      sizeof(tm_size_t) + sizeof(uint32_t) * graph->input_nodes.size();
  ...
  tm_subgraph.offset_vi_input_indices =
      WriteTmObject(start_ptr, cur_pos, v_input_indices, vector_size);

  /* Write the vector of output indices */
  vector_size =
      sizeof(tm_size_t) + sizeof(uint32_t) * graph->output_nodes.size();
  TM_Vector_indices *v_output_indices =
      (TM_Vector_indices *)malloc(vector_size);
  ...
  tm_subgraph.offset_vi_output_indices =
      WriteTmObject(start_ptr, cur_pos, v_output_indices, vector_size);

  /* Write the subgraph */
  tm_uoffset_t ret =
      WriteTmObject(start_ptr, cur_pos, &tm_subgraph, sizeof(TM_Subgraph));
...
bool TmSerializer::SaveModel(const std::vector<std::string> &file_list,
                             Graph *graph) {
  ...
  if (ftruncate(fd, TM_FILE_MAX_SIZE) != 0) {
    LOG_ERROR() << "Truncate file to max size failed\n";
    return false;
  }

  /* Mmap the file */
  void *const start_ptr =
      mmap(NULL, TM_FILE_MAX_SIZE, PROT_WRITE, MAP_SHARED, fd, 0);
  if (start_ptr == MAP_FAILED) {
    LOG_ERROR() << "Mmap of " << file_list[0] << " failed\n";
    return false;
  }
  memset(start_ptr, 0, TM_FILE_MAX_SIZE);
  ...
  if (tm_with_string_) {
    ...
    model_name.offset_data =
        WriteTmFileAlign1(start_ptr, &cur_pos, fname.c_str(), model_name.size);
    tm_model.offset_s_mname =
        WriteTmObject(start_ptr, &cur_pos, &model_name, sizeof(TM_String));
  } else
  ...
  tm_model.offset_vo_subgraphs =
      WriteTmObject(start_ptr, &cur_pos, v_subgraphs, vector_size);

  /* Write the model */
  header.offset_root =
      WriteTmObject(start_ptr, &cur_pos, &tm_model, sizeof(TM_Model));
  ...
  /* Truncate the file to the actual size and close it */
  if (ftruncate(fd, tm_model_size) == -1) {
    LOG_ERROR() << "Truncate file to actual size failed\n";
  }
  munmap(start_ptr, TM_FILE_MAX_SIZE);
  close(fd);
...
bool TmSerializer::LoadNode(StaticGraph *graph, StaticNode *node,
                            const TM_Node *tm_node) {
  if (tm_node->offset_vi_input_tensors != NOT_SET) {
    const TM_Vector_indices *v_input_tensors = GetTmPtr<TM_Vector_indices>(
        mmap_buf_, tm_node->offset_vi_input_tensors);

    /* Set the input tensors to the node */
    for (unsigned int i = 0; i < v_input_tensors->v_num; i++) {
      StaticTensor *tensor =
          graph->tensor_list[v_input_tensors->indices[i]].get();
      if (!tensor) {
        LOG_ERROR() << "The input tensor not exist: "
                    << v_input_tensors->indices[i] << "\n";
        return false;
      }
  ...
  if (tm_node->offset_vi_output_tensors != NOT_SET) {
    const TM_Vector_indices *v_output_tensors = GetTmPtr<TM_Vector_indices>(
        mmap_buf_, tm_node->offset_vi_output_tensors);

    /* Set the output tensors to the node */
    for (unsigned int i = 0; i < v_output_tensors->v_num; i++) {
      StaticTensor *tensor =
          graph->tensor_list[v_output_tensors->indices[i]].get();
      if (!tensor) {
        LOG_ERROR() << "The output tensor not exist: "
                    << v_output_tensors->indices[i] << "\n";
        return false;
      }
...
bool TmSerializer::LoadTensor(StaticGraph *graph, const TM_Tensor *tm_tensor,
  ...
  if (tm_tensor->offset_s_tname == NOT_SET)
    ...
    const TM_String *tm_string =
        GetTmPtr<TM_String>(mmap_buf_, tm_tensor->offset_s_tname);
    tm_tensor_name.assign(GetTmPtr<char>(mmap_buf_, tm_string->offset_data),
                          tm_string->size);
  }
  ...
  if (tm_tensor->offset_vd_dims != NOT_SET) {
    const TM_Vector_dims *v_dims =
        GetTmPtr<TM_Vector_dims>(mmap_buf_, tm_tensor->offset_vd_dims);
  ...
  if (tm_tensor->data_type == TM_DT_FLOAT32)
    ...
    SetTensorDataType(tensor, "float16");
  else if (tm_tensor->data_type == TM_DT_INT32)
    SetTensorDataType(tensor, "int");
  else if (tm_tensor->data_type == TM_DT_INT8)
    SetTensorDataType(tensor, "int8");
...
bool TmSerializer::LoadGraph(StaticGraph *graph, const TM_Model *tm_model) {
  const TM_Vector_offsets *v_graphs =
      GetTmPtr<TM_Vector_offsets>(mmap_buf_, tm_model->offset_vo_subgraphs);
  const TM_Subgraph *tm_graph =
      GetTmPtr<TM_Subgraph>(mmap_buf_, v_graphs->offsets[0]);

  const TM_Vector_offsets *v_nodes =
      GetTmPtr<TM_Vector_offsets>(mmap_buf_, tm_graph->offset_vo_seq_nodes);
  const TM_Vector_offsets *v_tensors =
      GetTmPtr<TM_Vector_offsets>(mmap_buf_, tm_graph->offset_vo_tensors);
  const TM_Vector_offsets *v_buffers =
      GetTmPtr<TM_Vector_offsets>(mmap_buf_, tm_graph->offset_vo_buffers);

  /* Load const tensors */
  for (unsigned int i = 0; i < v_tensors->v_num; i++) {
    const TM_Tensor *tm_tensor =
        GetTmPtr<TM_Tensor>(mmap_buf_, v_tensors->offsets[i]);
    const TM_Buffer *tm_buf;
    if (tm_tensor->type == kConstTensor)
      tm_buf = GetTmPtr<TM_Buffer>(mmap_buf_,
                                   v_buffers->offsets[tm_tensor->buffer_id]);
    else
      tm_buf = nullptr;
    LoadTensor(graph, tm_tensor, tm_buf);
  }
  ...
  for (i = 0; i < v_nodes->v_num; i++) {
    ...
    if (tm_node->offset_s_nname == NOT_SET)
      ...
      const TM_String *tm_string =
          GetTmPtr<TM_String>(mmap_buf_, tm_node->offset_s_nname);
      tm_node_name.assign(GetTmPtr<char>(mmap_buf_, tm_string->offset_data),
                          tm_string->size);
    }

    const TM_Operator *tm_operator =
        GetTmPtr<TM_Operator>(mmap_buf_, tm_node->offset_t_operator);
    ...
    if (!LoadNode(graph, node, tm_node)) break;

    op_load_t op_func = any_cast<op_load_t>(GetOpLoadMethod(tm_op_name));

    if (!op_func(graph, node, mmap_buf_, tm_operator)) break;

    /* Set the dynamic shape of the operator */
    node->op->dynamic_shape = tm_node->dynamic_shape;
  }

  if (i < v_nodes->v_num) return false;

  const TM_Vector_indices *v_input_nodes =
      GetTmPtr<TM_Vector_indices>(mmap_buf_, tm_graph->offset_vi_input_indices);
  const TM_Vector_indices *v_output_nodes = GetTmPtr<TM_Vector_indices>(
      mmap_buf_, tm_graph->offset_vi_output_indices);

  /* Set the input nodes */
  for (unsigned int i = 0; i < v_input_nodes->v_num; i++) {
    StaticNode *node = graph->node_list[v_input_nodes->indices[i]].get();
    if (!node) {
      LOG_ERROR() << "Input node #" << v_input_nodes->indices[i]
                  << " not exist\n";
      return false;
    }
  ...
  for (unsigned int i = 0; i < v_output_nodes->v_num; i++) {
    ...
    if (!node) {
      LOG_ERROR() << "Output node #" << v_output_nodes->indices[i]
                  << " not exist\n";
      return false;
    }
...
bool TmSerializer::LoadBinaryFile(const char *tm_fname) {
  mmap_fd_ = open(tm_fname, O_RDONLY);
  if (mmap_fd_ == -1) {
  ...
  struct stat sb;
  fstat(mmap_fd_, &sb);
  mmap_buf_size_ = sb.st_size;

  mmap_buf_ = mmap(NULL, mmap_buf_size_, PROT_READ, MAP_SHARED, mmap_fd_, 0);
...
bool TmSerializer::LoadModel(const std::vector<std::string> &file_list,
                             StaticGraph *graph) {
  if (file_list.size() != GetFileNum()) return false;

  if (!LoadBinaryFile(file_list[0].c_str())) return false;
  ...
  if (tm_header->ver_main != TM_FILE_VER_MAIN ||
      tm_header->ver_sub != TM_FILE_VER_SUB ||
      tm_header->ver_compile != TM_FILE_VER_COMPILE) {
    printf("Wrong version of tm file\n");
    return false;
  }

  const TM_Model *tm_model =
      GetTmPtr<TM_Model>(mmap_buf_, tm_header->offset_root);
  if (tm_model->offset_s_mname == NOT_SET) {
    SetGraphIdentity(graph, "tengine", "tengine_model", "0");
  } else {
    std::string tm_model_name;
    const TM_String *tm_string =
        GetTmPtr<TM_String>(mmap_buf_, tm_model->offset_s_mname);
    tm_model_name.assign(GetTmPtr<char>(mmap_buf_, tm_string->offset_data),
                         tm_string->size);
...
bool TmSerializerRegisterOpLoader(void) {
  ...
  if (!SerializerManager::SafeGet("tengine", serializer)) return false;
