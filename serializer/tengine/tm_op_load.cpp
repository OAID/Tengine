...
bool LoadTmAccuracyOp(StaticGraph *graph, StaticNode *node,
                      void *const start_ptr, const TM_Operator *tm_op) {
  StaticOp *op = CreateStaticOp(graph, OP_STR_ACCURACY);
  SetNodeOp(node, op);
  return true;
}

bool LoadTmBatchNormOp(StaticGraph *graph, StaticNode *node,
                       void *const start_ptr, const TM_Operator *tm_op) {
  const std::string &op_str = OP_STR_BATCHNORMALIZATION;

  BatchNormParam param =
      any_cast<BatchNormParam>(OpManager::GetOpDefParam(op_str));
  const TM_BatchNormParam *tm_param =
      GetTmPtr<TM_BatchNormParam>(start_ptr, tm_op->offset_t_param);
...
bool LoadTmResizeOp(StaticGraph *graph, StaticNode *node, void *const start_ptr,
                    const TM_Operator *tm_op) {
  const std::string &op_str = OP_STR_BILINEARRESIZE;

  ResizeParam param = any_cast<ResizeParam>(OpManager::GetOpDefParam(op_str));
  const TM_ResizeParam *tm_param =
      GetTmPtr<TM_ResizeParam>(start_ptr, tm_op->offset_t_param);
...
bool LoadTmConcatOp(StaticGraph *graph, StaticNode *node, void *const start_ptr,
                    const TM_Operator *tm_op) {
  const std::string &op_str = OP_STR_CONCAT;

  ConcatParam param = any_cast<ConcatParam>(OpManager::GetOpDefParam(op_str));
  const TM_ConcatParam *tm_param =
      GetTmPtr<TM_ConcatParam>(start_ptr, tm_op->offset_t_param);
...
bool LoadTmConstOp(StaticGraph *graph, StaticNode *node, void *const start_ptr,
                   const TM_Operator *tm_op) {
...
bool LoadTmConvOp(StaticGraph *graph, StaticNode *node, void *const start_ptr,
                  const TM_Operator *tm_op) {
  const std::string &op_str = OP_STR_CONVOLUTION;

  ConvParam param = any_cast<ConvParam>(OpManager::GetOpDefParam(op_str));
  const TM_ConvParam *tm_param =
      GetTmPtr<TM_ConvParam>(start_ptr, tm_op->offset_t_param);
...
bool LoadTmDeconvOp(StaticGraph *graph, StaticNode *node, void *const start_ptr,
                    const TM_Operator *tm_op) {
  const std::string &op_str = OP_STR_DECONVOLUTION;

  DeconvParam param = any_cast<DeconvParam>(OpManager::GetOpDefParam(op_str));
  const TM_DeconvParam *tm_param =
      GetTmPtr<TM_DeconvParam>(start_ptr, tm_op->offset_t_param);
...
bool LoadTmDetectionOutputOp(StaticGraph *graph, StaticNode *node,
                             void *const start_ptr, const TM_Operator *tm_op) {
  const std::string &op_str = OP_STR_DETECTIONOUTPUT;

  DetectionOutputParam param =
      any_cast<DetectionOutputParam>(OpManager::GetOpDefParam(op_str));
  const TM_DetectionOutputParam *tm_param =
      GetTmPtr<TM_DetectionOutputParam>(start_ptr, tm_op->offset_t_param);
...
bool LoadTmDropoutOp(StaticGraph *graph, StaticNode *node,
                     void *const start_ptr, const TM_Operator *tm_op) {
...
bool LoadTmEltwiseOp(StaticGraph *graph, StaticNode *node,
                     void *const start_ptr, const TM_Operator *tm_op) {
  const std::string &op_str = OP_STR_ELTWISE;

  EltwiseParam param = any_cast<EltwiseParam>(OpManager::GetOpDefParam(op_str));
  const TM_EltwiseParam *tm_param =
      GetTmPtr<TM_EltwiseParam>(start_ptr, tm_op->offset_t_param);
...
bool LoadTmFlattenOp(StaticGraph *graph, StaticNode *node,
                     void *const start_ptr, const TM_Operator *tm_op) {
  const std::string &op_str = OP_STR_FLATTEN;

  FlattenParam param = any_cast<FlattenParam>(OpManager::GetOpDefParam(op_str));
  const TM_FlattenParam *tm_param =
      GetTmPtr<TM_FlattenParam>(start_ptr, tm_op->offset_t_param);
...
bool LoadTmFCOp(StaticGraph *graph, StaticNode *node, void *const start_ptr,
                const TM_Operator *tm_op) {
  const std::string &op_str = OP_STR_FULLYCONNECTED;

  FCParam param = any_cast<FCParam>(OpManager::GetOpDefParam(op_str));
  const TM_FCParam *tm_param =
      GetTmPtr<TM_FCParam>(start_ptr, tm_op->offset_t_param);
...
bool LoadTmInputOp(StaticGraph *graph, StaticNode *node, void *const start_ptr,
                   const TM_Operator *tm_op) {
...
bool LoadTmLRNOp(StaticGraph *graph, StaticNode *node, void *const start_ptr,
                 const TM_Operator *tm_op) {
  const std::string &op_str = OP_STR_LRN;

  LRNParam param = any_cast<LRNParam>(OpManager::GetOpDefParam(op_str));
  const TM_LRNParam *tm_param =
      GetTmPtr<TM_LRNParam>(start_ptr, tm_op->offset_t_param);
...
bool LoadTmNormalizeOp(StaticGraph *graph, StaticNode *node,
                       void *const start_ptr, const TM_Operator *tm_op) {
  const std::string &op_str = OP_STR_NORMALIZE;

  NormalizeParam param =
      any_cast<NormalizeParam>(OpManager::GetOpDefParam(op_str));
  const TM_NormalizeParam *tm_param =
      GetTmPtr<TM_NormalizeParam>(start_ptr, tm_op->offset_t_param);
...
bool LoadTmPermuteOp(StaticGraph *graph, StaticNode *node,
                     void *const start_ptr, const TM_Operator *tm_op) {
  const std::string &op_str = OP_STR_PERMUTE;

  PermuteParam param = any_cast<PermuteParam>(OpManager::GetOpDefParam(op_str));
  const TM_PermuteParam *tm_param =
      GetTmPtr<TM_PermuteParam>(start_ptr, tm_op->offset_t_param);
...
bool LoadTmPoolingOp(StaticGraph *graph, StaticNode *node,
                     void *const start_ptr, const TM_Operator *tm_op) {
  const std::string &op_str = OP_STR_POOLING;

  PoolParam param = any_cast<PoolParam>(OpManager::GetOpDefParam(op_str));
  const TM_PoolParam *tm_param =
      GetTmPtr<TM_PoolParam>(start_ptr, tm_op->offset_t_param);
...
bool LoadTmPreluOp(StaticGraph *graph, StaticNode *node, void *const start_ptr,
                   const TM_Operator *tm_op) {
...
bool LoadTmPriorBoxOp(StaticGraph *graph, StaticNode *node,
                      void *const start_ptr, const TM_Operator *tm_op) {
  const std::string &op_str = OP_STR_PRIORBOX;

  PriorBoxParam param =
      any_cast<PriorBoxParam>(OpManager::GetOpDefParam(op_str));
  const TM_PriorBoxParam *tm_param =
      GetTmPtr<TM_PriorBoxParam>(start_ptr, tm_op->offset_t_param);
  const TM_Vector_floats *v_minsizes =
      GetTmPtr<TM_Vector_floats>(start_ptr, tm_param->offset_vf_max_size);
  const TM_Vector_floats *v_maxsizes =
      GetTmPtr<TM_Vector_floats>(start_ptr, tm_param->offset_vf_min_size);
  const TM_Vector_floats *v_variances =
      GetTmPtr<TM_Vector_floats>(start_ptr, tm_param->offset_vf_variance);
  const TM_Vector_floats *v_ratios =
      GetTmPtr<TM_Vector_floats>(start_ptr, tm_param->offset_vf_aspect_ratio);
...
bool LoadTmRegionOp(StaticGraph *graph, StaticNode *node, void *const start_ptr,
                    const TM_Operator *tm_op) {
  const std::string &op_str = OP_STR_REGION;

  RegionParam param = any_cast<RegionParam>(OpManager::GetOpDefParam(op_str));
  const TM_RegionParam *tm_param =
      GetTmPtr<TM_RegionParam>(start_ptr, tm_op->offset_t_param);
  const TM_Vector_floats *v_biases =
      GetTmPtr<TM_Vector_floats>(start_ptr, tm_param->offset_vf_biases);
...
bool LoadTmReLuOp(StaticGraph *graph, StaticNode *node, void *const start_ptr,
                  const TM_Operator *tm_op) {
  const std::string &op_str = OP_STR_RELU;

  ReLuParam param = any_cast<ReLuParam>(OpManager::GetOpDefParam(op_str));
  const TM_ReLuParam *tm_param =
      GetTmPtr<TM_ReLuParam>(start_ptr, tm_op->offset_t_param);
...
bool LoadTmRelu6Op(StaticGraph *graph, StaticNode *node, void *const start_ptr,
                   const TM_Operator *tm_op) {
...
bool LoadTmReorgOp(StaticGraph *graph, StaticNode *node, void *const start_ptr,
                   const TM_Operator *tm_op) {
  const std::string &op_str = OP_STR_REORG;

  ReorgParam param = any_cast<ReorgParam>(OpManager::GetOpDefParam(op_str));
  const TM_ReorgParam *tm_param =
      GetTmPtr<TM_ReorgParam>(start_ptr, tm_op->offset_t_param);
...
bool LoadTmReshapeOp(StaticGraph *graph, StaticNode *node,
                     void *const start_ptr, const TM_Operator *tm_op) {
  const std::string &op_str = OP_STR_RESHAPE;

  ReshapeParam param = any_cast<ReshapeParam>(OpManager::GetOpDefParam(op_str));
  const TM_ReshapeParam *tm_param =
      GetTmPtr<TM_ReshapeParam>(start_ptr, tm_op->offset_t_param);
  const TM_Vector_dims *v_dims =
      GetTmPtr<TM_Vector_dims>(start_ptr, tm_param->offset_vd_dims);

  param.axis = tm_param->axis;
  for (unsigned int i = 0; i < v_dims->v_num; i++)
    param.dims.push_back(v_dims->dims[i]);
...
bool LoadTmROIPoolingOp(StaticGraph *graph, StaticNode *node,
                        void *const start_ptr, const TM_Operator *tm_op) {
  const std::string &op_str = OP_STR_ROIPOOLING;

  ROIPoolingParam param =
      any_cast<ROIPoolingParam>(OpManager::GetOpDefParam(op_str));
  const TM_ROIPoolingParam *tm_param =
      GetTmPtr<TM_ROIPoolingParam>(start_ptr, tm_op->offset_t_param);
...
bool LoadTmRPNOp(StaticGraph *graph, StaticNode *node, void *const start_ptr,
                 const TM_Operator *tm_op) {
  const std::string &op_str = OP_STR_RPN;

  RPNParam param = any_cast<RPNParam>(OpManager::GetOpDefParam(op_str));
  const TM_RPNParam *tm_param =
      GetTmPtr<TM_RPNParam>(start_ptr, tm_op->offset_t_param);
  const TM_Vector_floats *v_ratios =
      GetTmPtr<TM_Vector_floats>(start_ptr, tm_param->offset_vf_ratios);
  const TM_Vector_floats *v_scales =
      GetTmPtr<TM_Vector_floats>(start_ptr, tm_param->offset_vf_anchor_scales);
...
bool LoadTmScaleOp(StaticGraph *graph, StaticNode *node, void *const start_ptr,
                   const TM_Operator *tm_op) {
  const std::string &op_str = OP_STR_SCALE;

  ScaleParam param = any_cast<ScaleParam>(OpManager::GetOpDefParam(op_str));
  const TM_ScaleParam *tm_param =
      GetTmPtr<TM_ScaleParam>(start_ptr, tm_op->offset_t_param);
...
bool LoadTmSliceOp(StaticGraph *graph, StaticNode *node, void *const start_ptr,
                   const TM_Operator *tm_op) {
  const std::string &op_str = OP_STR_SLICE;

  SliceParam param = any_cast<SliceParam>(OpManager::GetOpDefParam(op_str));
  const TM_SliceParam *tm_param =
      GetTmPtr<TM_SliceParam>(start_ptr, tm_op->offset_t_param);
...
bool LoadTmSoftmaxOp(StaticGraph *graph, StaticNode *node,
                     void *const start_ptr, const TM_Operator *tm_op) {
  const std::string &op_str = OP_STR_SOFTMAX;

  SoftmaxParam param = any_cast<SoftmaxParam>(OpManager::GetOpDefParam(op_str));
  const TM_SoftmaxParam *tm_param =
      GetTmPtr<TM_SoftmaxParam>(start_ptr, tm_op->offset_t_param);
...
bool LoadTmSplitOp(StaticGraph *graph, StaticNode *node, void *const start_ptr,
                   const TM_Operator *tm_op) {
...
op_load_t LoadTmOpFunc(uint32_t op_type) {
  switch (op_type) {
    ...
    default:
      LOG_ERROR() << "Operator #" << op_type
                  << " not supported in tengine model yet\n";
      return nullptr;
  }
}
