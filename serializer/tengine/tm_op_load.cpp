...
bool LoadTmPriorBoxOp(StaticGraph *graph, StaticNode *node, void * const start_ptr, const TM_Operator *tm_op)
{
    ...
    const TM_Vector_floats *v_minsizes = GetTmPtr<TM_Vector_floats>(start_ptr, tm_param->offset_vf_max_size);
    const TM_Vector_floats *v_maxsizes = GetTmPtr<TM_Vector_floats>(start_ptr, tm_param->offset_vf_min_size);
...
bool LoadTmReshapeOp(StaticGraph *graph, StaticNode *node, void * const start_ptr, const TM_Operator *tm_op)
{
    ...
    const TM_Vector_dims *v_dims = GetTmPtr<TM_Vector_dims>(start_ptr, tm_param->offset_vd_dims);
...
op_load_t LoadTmOpFunc(uint32_t op_type)
{
    switch(op_type)
    {
        case TM_OPTYPE_ACCURACY:           return LoadTmAccuracyOp;
        case TM_OPTYPE_BATCHNORMALIZATION: return LoadTmBatchNormOp;
        case TM_OPTYPE_BILINEARRESIZE:     return LoadTmResizeOp;
        case TM_OPTYPE_CONCAT:             return LoadTmConcatOp;
        case TM_OPTYPE_CONST:              return LoadTmConstOp;
        case TM_OPTYPE_CONVOLUTION:        return LoadTmConvOp;
        case TM_OPTYPE_DECONVOLUTION:      return LoadTmDeconvOp;
        case TM_OPTYPE_DETECTIONOUTPUT:    return LoadTmDetectionOutputOp;
        case TM_OPTYPE_DROPOUT:            return LoadTmDropoutOp;
        case TM_OPTYPE_ELTWISE:            return LoadTmEltwiseOp;
        case TM_OPTYPE_FLATTEN:            return LoadTmFlattenOp;
        case TM_OPTYPE_FULLYCONNECTED:     return LoadTmFCOp;
        case TM_OPTYPE_INPUTOP:            return LoadTmInputOp;
        case TM_OPTYPE_LRN:                return LoadTmLRNOp;
        case TM_OPTYPE_NORMALIZE:          return LoadTmNormalizeOp;
        case TM_OPTYPE_PERMUTE:            return LoadTmPermuteOp;
        case TM_OPTYPE_POOLING:            return LoadTmPoolingOp;
        case TM_OPTYPE_PRELU:              return LoadTmPreluOp;
        case TM_OPTYPE_PRIORBOX:           return LoadTmPriorBoxOp;
        case TM_OPTYPE_REGION:             return LoadTmRegionOp;
        case TM_OPTYPE_RELU:               return LoadTmReLuOp;
        case TM_OPTYPE_RELU6:              return LoadTmRelu6Op;
        case TM_OPTYPE_REORG:              return LoadTmReorgOp;
        case TM_OPTYPE_RESHAPE:            return LoadTmReshapeOp;
        case TM_OPTYPE_ROIPOOLING:         return LoadTmROIPoolingOp;
        case TM_OPTYPE_RPN:                return LoadTmRPNOp;
        case TM_OPTYPE_SCALE:              return LoadTmScaleOp;
        case TM_OPTYPE_SLICE:              return LoadTmSliceOp;
        case TM_OPTYPE_SOFTMAX:            return LoadTmSoftmaxOp;
        case TM_OPTYPE_SPLIT:              return LoadTmSplitOp;
        default:
...
std::string GetOpStr(uint32_t op_type)
{
    switch(op_type)
    {
        case TM_OPTYPE_ACCURACY:           return std::string(OP_STR_ACCURACY);
        case TM_OPTYPE_BATCHNORMALIZATION: return std::string(OP_STR_BATCHNORMALIZATION);
        case TM_OPTYPE_BILINEARRESIZE:     return std::string(OP_STR_BILINEARRESIZE);
        case TM_OPTYPE_CONCAT:             return std::string(OP_STR_CONCAT);
        case TM_OPTYPE_CONST:              return std::string(OP_STR_CONST);
        case TM_OPTYPE_CONVOLUTION:        return std::string(OP_STR_CONVOLUTION);
        case TM_OPTYPE_DECONVOLUTION:      return std::string(OP_STR_DECONVOLUTION);
        case TM_OPTYPE_DETECTIONOUTPUT:    return std::string(OP_STR_DETECTIONOUTPUT);
        case TM_OPTYPE_DROPOUT:            return std::string(OP_STR_DROPOUT);
        case TM_OPTYPE_ELTWISE:            return std::string(OP_STR_ELTWISE);
        case TM_OPTYPE_FLATTEN:            return std::string(OP_STR_FLATTEN);
        case TM_OPTYPE_FULLYCONNECTED:     return std::string(OP_STR_FULLYCONNECTED);
        case TM_OPTYPE_INPUTOP:            return std::string(OP_STR_INPUTOP);
        case TM_OPTYPE_LRN:                return std::string(OP_STR_LRN);
        case TM_OPTYPE_NORMALIZE:          return std::string(OP_STR_NORMALIZE);
        case TM_OPTYPE_PERMUTE:            return std::string(OP_STR_PERMUTE);
        case TM_OPTYPE_POOLING:            return std::string(OP_STR_POOLING);
        case TM_OPTYPE_PRELU:              return std::string(OP_STR_PRELU);
        case TM_OPTYPE_PRIORBOX:           return std::string(OP_STR_PRIORBOX);
        case TM_OPTYPE_REGION:             return std::string(OP_STR_REGION);
        case TM_OPTYPE_RELU:               return std::string(OP_STR_RELU);
        case TM_OPTYPE_RELU6:              return std::string(OP_STR_RELU6);
        case TM_OPTYPE_REORG:              return std::string(OP_STR_REORG);
        case TM_OPTYPE_RESHAPE:            return std::string(OP_STR_RESHAPE);
        case TM_OPTYPE_ROIPOOLING:         return std::string(OP_STR_ROIPOOLING);
        case TM_OPTYPE_RPN:                return std::string(OP_STR_RPN);
        case TM_OPTYPE_SCALE:              return std::string(OP_STR_SCALE);
        case TM_OPTYPE_SLICE:              return std::string(OP_STR_SLICE);
        case TM_OPTYPE_SOFTMAX:            return std::string(OP_STR_SOFTMAX);
        case TM_OPTYPE_SPLIT:              return std::string(OP_STR_SPLIT);
        default:
