...
bool SrcSerializer::LoadModel(const std::vector<std::string>& file_list, StaticGraph * static_graph)
{
    ...
    if(patch_list.empty())
	 ...
	 uint32_t offset=patch_list[i].patch_off;
         ...
         memcpy((char *)plain_mem+offset,patch_list[i].addr,
			 patch_list[i].patch_size);
     }
     ...
     bool ret=decrypt(plain_mem,total_patch_size,patch_list[0].vendor_id,patch_list[0].nn_id,
			    &text_addr,&text_len);
     free(plain_mem);
