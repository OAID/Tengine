...
static bool ParseNodeParam(const std::string& str, std::string& param,
                           std::string& value) {
...
bool MxnetSerializer::LoadTextFile(const char* fname,
  ...
  while (!is.eof()) {
    ...
    if ((nest == OUT_DEF_BODY && line == start_def_body) ||
        (nest == IN_DEF_BODY && line == start_nodes_list)) {
      nest++;
      continue;
    } else if (nest == IN_NODES_LIST) {
      if (line == end_nodes_list) {
        nest--;
        if (nest != IN_DEF_BODY) ret = false;
      ...
      if (line == end_node_block1 || line == end_node_block2) {
        ...
        if (param == "op") {
          ...
          if (value == "{}") {
            ...
            Trim(value, "{}");
            if (!value.empty() &&
                ParseNodeParam(value, attr_param, attr_value)) {
...
bool MxnetSerializer::LoadBinaryFile(const char* fname,
...
bool MxnetSerializer::LoadModel(const std::vector<std::string>& file_list,
                                StaticGraph* graph) {
...
bool MxnetSerializer::LoadConstTensor(
    StaticGraph* graph, const std::vector<MxnetParam>& paramlist) {
  ...
  for (int i = 0; i < const_tensor_number; i++) {
    ...
    /* load data */
    for (int k = 0; k < mxnet_tensor.data_len; k++) mem_buf[k] = raw_data[k];
...
static bool GetParam(const std::string name,
...
void MxnetSerializer::CreateInputNode(
    StaticGraph* graph, const std::vector<MxnetNode>& nodelist,
    const std::vector<MxnetParam>& paramlist) {
#ifdef DEBUG
  ...
  for (unsigned int i = 0; i < nodelist.size(); i++) {
    ...
    if (mxnet_node.name == "data") {
      if (FindConstTensor(graph, mxnet_node.name) != nullptr) continue;
...
#ifdef DEBUG
      std::cout << "    create an input node for " << mxnet_node.name
                << std::endl;
#endif
    }
  }
...
void MxnetSerializer::LoadNode(StaticGraph* graph, StaticNode* node,
                               const MxnetNode& mxnet_node,
                               const std::vector<MxnetNode>& nodelist) {
  int input_number = mxnet_node.inputs.size();

#ifdef DEBUG
  std::cout << "name: " << mxnet_node.name << " input_number: " << input_number
            << std::endl;
#endif
  ...
  for (int i = 0; i < input_number; i++) {
    ...
    if (input_node.name == "prob_label") continue;

#ifdef DEBUG
    std::cout << "    input: " << input_idx << " " << input_node.name
              << std::endl;
#endif
...
bool MxnetSerializer::LoadGraph(StaticGraph* graph,
                                const std::vector<MxnetNode>& nodelist,
                                const std::vector<MxnetParam>& paramlist) {
  ...
  for (i = 0; i < nodelist.size(); i++) {
    ...
    if (mxnet_node.op == "null") continue;
    ...
    if (!op_func(graph, node, mxnet_node)) break;
  }

  if (i < nodelist.size()) return false;
...
static bool LoadMxnetSoftmax(StaticGraph* graph, StaticNode* node,
                             const MxnetNode& mxnet_node) {
...
static bool LoadMxnetConcat(StaticGraph* graph, StaticNode* node,
                            const MxnetNode& mxnet_node) {
...
static bool LoadMxnetConvolution(StaticGraph* graph, StaticNode* node,
                                 const MxnetNode& mxnet_node) {
  ConvParam param =
...
#ifdef DEBUG
  std::cout << "ConvParam : " << param.kernel_h << ", " << param.kernel_w
            << ", " << param.stride_h << ", " << param.stride_w << ", "
            << param.pad_h << ", " << param.pad_w << ", " << param.group << ", "
...
static bool LoadMxnetPooling(StaticGraph* graph, StaticNode* node,
                             const MxnetNode& mxnet_node) {
...
#ifdef DEBUG
  std::cout << "PoolParam : " << param.kernel_h << ", " << param.kernel_w
            << ", " << param.stride_h << ", " << param.stride_w << ", "
            << param.pad_h << ", " << param.pad_w << ", " << param.global
            << ", " << param.alg << std::endl;
#endif
...
static bool LoadMxnetBatchNorm(StaticGraph* graph, StaticNode* node,
                               const MxnetNode& mxnet_node) {
...
static bool LoadMxnetDropout(StaticGraph* graph, StaticNode* node,
                             const MxnetNode& mxnet_node) {
...
static bool LoadMxnetRelu(StaticGraph* graph, StaticNode* node,
                          const MxnetNode& mxnet_node) {
...
static bool LoadMxnetEltScalar(StaticGraph* graph, StaticNode* node,
                               const MxnetNode& mxnet_node) {
  EltwiseParam param =
  ...
  StaticTensor* tensor =
      CreateStaticConstTensor(graph, mxnet_node.name + "_scalar");
  std::vector<int> dims;
  dims.push_back(1);
  SetTensorDim(tensor, dims);
  SetTensorDataType(tensor, "float32");
...
static bool LoadMxnetElemwiseAdd(StaticGraph* graph, StaticNode* node,
                                 const MxnetNode& mxnet_node) {
  EltwiseParam param =
      any_cast<EltwiseParam>(OpManager::GetOpDefParam("Eltwise"));
  param.type = ELT_SUM;
...
static bool LoadMxnetLeakyReLU(StaticGraph* graph, StaticNode* node,
                               const MxnetNode& mxnet_node) {
...
static bool LoadMxnetFullyConnected(StaticGraph* graph, StaticNode* node,
                                    const MxnetNode& mxnet_node) {
...
static bool LoadMxnetReshape(StaticGraph* graph, StaticNode* node,
                             const MxnetNode& mxnet_node) {
  ReshapeParam param =
...
bool MxnetSerializerRegisterOpLoader(void) {
  ...
  if (!SerializerManager::SafeGet("mxnet", serializer)) return false;
  ...
  p_mxnet->RegisterOpLoadMethod("elemwise_add",
                                op_load_t(LoadMxnetElemwiseAdd));
  p_mxnet->RegisterOpLoadMethod("LeakyReLU", op_load_t(LoadMxnetLeakyReLU));
  p_mxnet->RegisterOpLoadMethod("FullyConnected",
                                op_load_t(LoadMxnetFullyConnected));
