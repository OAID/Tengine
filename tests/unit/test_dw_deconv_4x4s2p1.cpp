...
graph_t create_conv_graph(int c, int h, int w, int ksize, int stride, int pad, int group)
{
    ...
    if(create_conv_node(graph, conv_name, input_name, ksize, stride, pad, c, group, group) < 0)
    {
...
int main(int argc, char* argv[])
{
    ...
    init_tengine();
    graph_t graph = create_conv_graph(c, h, w, 4,2,1,group);
    graph_t graph1 = create_conv_graph(c, h, w, 4,2,1,group);
    ...
    int out_size = group*h*w*4;
    float err=maxerr(buf1,buf,out_size);
    printf("c h w g = %d %d %d %d\t maxerr = %f\n",c,h,w,group,err);
    if(err < 0.01)
        printf("test pass\n");
    else
        printf("test fail\n");
