...
{
    node_t test_node = create_graph_node(graph, "ZerosLike", "ZerosLike");
    ...
    /* output */
    tensor_t output_tensor = create_graph_tensor(graph, "ZerosLike", TENGINE_DT_FP32);
...
{  
    ...
    dump_graph(graph);
    node_t test_node = get_graph_node(graph, "ZerosLike");
