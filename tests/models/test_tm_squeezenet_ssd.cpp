...
#define DEF_MODEL "models/squeezenet_ssd.tmfile"
...
int main(int argc, char* argv[])
{
    ...
    int res;
    while((res = getopt(argc, argv, "m:i:d:h:")) != -1)
    {
        switch(res)
        {
            case 'm':
            ...
            case 'h':
                std::cout << "[Usage]: " << argv[0] << " [-h]\n"
                          << "[-m model_file] [-i image_file] [-d device]\n";
                return 0;
            default:
                break;
        }
    ...
    FILE *fp;  
    fp=fopen("./data/squeezenet_ssd_out.bin","rb");
    ...
    if(float_mismatch(buf, outdata, num*6) != true)
        return -1;
