...
int main(int argc, char* argv[])
{
    ...
    std::string image_file = root_path + "./images/ssd_dog.jpg";
    int layer_type = 0;
    std::string model_name = "yolov3";
    std::string text_file = root_path + "./models/yolov3_dk.tmfile";
    std::string out_img_name = root_path + "tengine_example_out";
    int numBBoxes = 3;
    ...
    if(model_name == "yolov2")
    {
        ...
        numBBoxes = 5;
        total_numAnchors = 5;
        text_file = root_path + "./models/yolov2_dk.tmfile";
        //out_img_name = root_path + "darknet_yolo2_test";
    }
    else if(model_name == "yolov3_tiny")
    {
        ...
        numBBoxes = 3;
        total_numAnchors = 6;
        text_file = root_path + "./models/yolov3_tiny_dk.tmfile";
        net_w = 416;
        net_h = 416;
        //out_img_name = root_path + "darknet_yolo3_tiny_test";
    }
    else if(model_name == "yolov2_tiny")
    {
        ...
        net_w = 416;
        net_h = 416;
        text_file = root_path + "./models/yolov2_tiny_dk.tmfile";
        //out_img_name = root_path + "darknet_yolov2_tiny_test";
    }
   ...
   int img_h = im.h;
    ...
    for(int i = 0; i < output_node_num; ++i)
    {
        ...
        // int out_size = get_tensor_buffer_size(out_tensor) / sizeof(float);
        // std::string out_file = "./data//" + model_name + "_" + to_string (i) + "_out.bin";
        // FILE *outfp;  
        // outfp=fopen(out_file.c_str(), "wb");
        // fwrite(out_data, sizeof(float), out_size, outfp);
        // fclose(outfp);
    }
    int nboxes = 0;
    // get network boxes
    ...
    save_image(img, out_img_name.c_str());

    // test output data and free tensor
    for(int i = 0; i < output_node_num; ++i)
    {
        tensor_t out_tensor = get_graph_output_tensor(graph, i, 0);    //"detection_out");
        float* out_data = ( float* )get_tensor_buffer(out_tensor);
        ...
        if(float_mismatch(out_data_ref, out_data, out_size) != true)
            return -1;
