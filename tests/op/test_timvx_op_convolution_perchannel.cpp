...
int create_test_convolution_node(graph_t graph, const char* input_name, const char* node_name, int data_type, int layout, int n, int c, int h, int w)
{
    ...
    int weight_dims[4] = {2, 2, 2, 2};  // channel num
 ...
 */
float input_fp32[12] = {3, 1,  -2,
                        4, 2,  -3,
                        2, -1, -3,
                        3, -2, -4};
float input_scale = 0.5;
int input_zero_point = 127;

int8_t weight_int8[16] = {1, 3, 3, 5, 2, 4, 4, 6,
                               4, 3, 2, 1, 4, 3, 2, 1};

int bias_int32[2] = {6, -2,};

float reference_out[4] = {31, -57, 64, -46};
float output_scale = 0.5;
int output_zero_point = 127;
...
//    tensor_t weight_tesnor = get_graph_input_tensor(ir_graph, 1, 0);
    ...
    // set input data
    int8_t input_i8[12] = {0};
    get_int8_data(input_fp32, input_i8, 12, input_scale, input_zero_point);
    set_tensor_buffer(input_tensor, input_i8, 12);

    // set weight data
    float* scale_list = (float*) malloc(sizeof(float) * 2 );
    scale_list[0] = 1;
    scale_list[1] = 2;
    int* zp_list = (int*) malloc(sizeof(int) * 2 );
    zp_list[0] = 0;
    zp_list[1] = 0;
    weight_tensor->scale_list = scale_list;
    weight_tensor->zp_list = zp_list;
    weight_tensor->quant_param_num = 2;

    set_tensor_buffer(weight_tensor, weight_int8, 16);

    // set bias data
    float* scale_list_bias = (float*) malloc(sizeof(float) * 2 );
    scale_list_bias[0] = 0.5;
    scale_list_bias[1] = 1;
    int* zp_list_bias = (int*) malloc(sizeof(int) * 2 );
    zp_list_bias[0] = 0;
    zp_list_bias[1] = 0;
    bias_tensor->scale_list = scale_list_bias;
    bias_tensor->zp_list = zp_list_bias;
    bias_tensor->quant_param_num = 2;

    set_tensor_buffer(bias_tensor, bias_int32, 2 * 4);
