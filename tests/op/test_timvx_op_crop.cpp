...
#include "operator/prototype/crop_param.h"


int create_test_crop_node(graph_t graph, const char* input_name, const char* node_name, int data_type, int layout, int n, int c, int h, int w)
{
    (void)layout; (void)n; (void)c; (void)h; (void)w;

    /* create the test node */
    struct node* test_node = (struct node* )create_graph_node(graph, node_name, "Crop");
    ...
    /* set params */
    struct crop_param* param = ( struct crop_param* )(struct node* )test_node->op.param_mem;
 ...
 */
float input_fp32[32] = {1.0f, 1.0f, 1.0f, 1.0f,
                        1.0f, 2.0f, 2.0f, 1.0f,
                        1.0f, 2.0f, 2.0f, 1.0f,
                        1.0f, 1.0f, 1.0f, 1.0f,
                        1.0f, 1.0f, 1.0f, 1.0f,
...
float reference_out[24] = {1.0f, 1.0f, 1.0f, 1.0f,
                        1.0f, 2.0f, 2.0f, 1.0f,
                        1.0f, 2.0f, 2.0f, 1.0f,
                        1.0f, 1.0f, 1.0f, 1.0f,
                        1.0f, 1.0f, 1.0f, 1.0f,
...
{
    int n = 1, c = 2, h = 4, w = 4;
    const char* test_node_name = "crop";
    ...
    // create
    struct graph* ir_graph = (struct graph* )create_timvx_test_graph(test_node_name, data_type, layout, n, c, h, w, &create_test_crop_node);
    if(NULL == ir_graph)
        return -1;
    ...
    struct tensor* output_tensor = (struct tensor*)get_graph_tensor(ir_graph, "crop");
...
//    tensor_t weight_tesnor = get_graph_input_tensor(ir_graph, 1, 0);
    ...
    // set input data
    uint8_t input_u8[32] = {0};
    get_uint8_data(input_fp32, input_u8, 32, input_scale, input_zero_point);
    set_tensor_buffer(input_tensor, input_u8, 32);
