...
Driver* DriverManager::GetDriver(const std::string& name) {
  ...
  if (!SafeGet(name, driver)) return nullptr;
...
Device* DriverManager::RealGetDevice(const dev_id_t& dev_id) {
  ...
  while (ir != ir_end) {
    ...
    for (int i = 0; i < dev_num; i++) {
      ...
      if (dev->GetDeviceID() == dev_id) return dev;
    }
...
bool DriverManager::LoadDevice(Driver* driver, Device* device) {
  if (driver->GetDeviceStatus(device) == kDevNormal) return true;

  const dev_id_t& dev_id = device->GetDeviceID();

  DevExecutor* dev_executor =
      DevExecutorFactory::GetFactory()->Create(dev_id, dev_id);
...
bool DriverManager::UnloadDevice(Driver* driver, Device* device) {
  ...
  if (dev_executor) {
    if (!dev_executor->Stop() || !device->Stop()) return false;
...
bool DriverManager::LoadDevice(Driver* driver) {
  ...
  for (int i = 0; i < dev_num; i++) {
    ...
    if (!LoadDevice(driver, device)) return false;
  }
...
bool DriverManager::UnloadDevice(Driver* driver) {
  ...
  if (dev_num == 0) return true;

  for (int i = 0; i < dev_num; i++) {
    Device* device = driver->GetDevice(0);

    if (!UnloadDevice(driver, device)) return false;
  }
...
int DriverManager::ProbeDevice(void) {
  ...
  while (ir != ir_end) {
    ...
    if (driver->AutoProbe()) {
      ...
      LOG_INFO() << "Driver: " << driver->GetName() << " probed " << dev_num
                 << " devices\n";
    }
    ir++;
  }
...
Device* DriverManager::GetDefaultDevice(void) {
  ...
  if (GetDefaultDeviceName(dummy_name)) return manager->default_dev;
...
int load_device(const char* driver_name, const char* dev_name) {
  ...
  if (dev_name) {
    ...
    /* load special device */
    if (DriverManager::LoadDevice(driver, dev)) return 0;
  } else {
    if (DriverManager::LoadDevice(driver)) return 0;
  }
...
int unload_device(const char* driver_name, const char* dev_name) {
  ...
  if (dev_name) {
    ...
    if (DriverManager::UnloadDevice(driver, dev)) return 0;

  } else {
    if (DriverManager::UnloadDevice(driver)) return 0;
  }
...
int set_default_device(const char* device) {
  if (DriverManager::GetDevice(device) == nullptr) return -1;
