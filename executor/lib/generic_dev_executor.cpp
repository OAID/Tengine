...
bool GenericDevExecutor::GetQueueReference(
    GenericDevExecutor::QueueType queue_type, std::mutex *&p_mutex,
...
void GenericDevExecutor::InsertQueue(GenericDevExecutor::QueueType queue_type,
                                     SubgraphTask *task) {
  std::mutex *p_mutex = nullptr;
  task_queue_t *p_queue = nullptr;

  if (!GetQueueReference(queue_type, p_mutex, p_queue)) return;
...
bool GenericDevExecutor::RemoveQueue(QueueType queue_type, SubgraphTask *task) {
  ...
  if (!GetQueueReference(queue_type, p_mutex, p_queue)) return false;
...
SubgraphTask *GenericDevExecutor::PopQueue(QueueType queue_type) {
  ...
  if (!GetQueueReference(queue_type, p_mutex, p_queue)) return nullptr;
...
int GenericDevExecutor::GetElementNumber(QueueType queue_type) {
  ...
  if (!GetQueueReference(queue_type, p_mutex, p_queue)) return 0;
...
Subgraph *GenericDevExecutor::GetOptimizedGraph(SubgraphTask *task) {
  if (!task->graph_optimized) return nullptr;

  return DevGetOptimizedGraph(task->graph_handle);
}

bool GenericDevExecutor::PrerunTask(SubgraphTask *task) {
  if (DevGetStatus() != kDevNormal) return false;

  if (task->graph_handle == nullptr)
    task->graph_handle = DevCreateGraphHandle(task->sub_graph);

  if (task->graph_handle == nullptr || !OptimizeGraph(task)) return false;
  ...
  if (!DevPrerun(task->graph_handle)) return false;
...
bool GenericDevExecutor::SyncRunTask(SubgraphTask *task) {
  ...
  if (DevGetStatus() == kDevNormal) ret = DevSyncRun(task->graph_handle);
...
void GenericDevExecutor::OnSubgraphDone(Subgraph *sub_graph,
                                        bool exec_success) {
  SubgraphTask *task = SubgraphTask::GetSubgraphTask(sub_graph);

  // move the task from RUN QUEUE to WAIT QUEUE
  task->Lock();  // as this is an callback function, to protect the ops in
                 // SchedTask()/RunTask()
