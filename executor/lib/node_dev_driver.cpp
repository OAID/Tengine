...
bool NodeDriver::InitializeDevice(Device *device) {
  ...
  if (!InitDev(node_dev)) return false;
...
bool NodeDriver::StartDevice(Device *device) {
  ...
  if (!StartDev(node_dev)) return false;
...
bool NodeDriver::StopDevice(Device *device) {
  ...
  if (node_dev->dev_status == kDevStopped) return true;

  if (!StopDev(node_dev)) return false;
...
void NodeDriver::SetNodeDoneHook(Device *dev, void *node_handle,
                                 dev_node_cb_t func) {
...
int NodeDriver::GetDeviceNum(void) { return dev_table_.size(); }

Device *NodeDriver::GetDevice(int idx) {
  if ((unsigned int)idx >= dev_table_.size()) return nullptr;
...
bool NodeDriver::GetWorkload(Device *dev, DevWorkload &load) { return false; }

bool NodeDriver::GetPerf(Device *dev, Subgraph *graph, int policy,
                         GraphPerf &perf) {
  return false;
}
...
int NodeDriver::GetPolicyPriority(Device *dev, int policy) { return 10000; }
