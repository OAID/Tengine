...
static void AddConstNodeToSubGraph(Subgraph *graph, Tensor *tensor,
                                   Node *fused_node, int fused_port_index);

static bool Weight_Bn(Subgraph *graph, Node *ConvNode, float *mean, float *var,
  ...
  float *kernel_new =
      (float *)(malloc(kernel_size * channel_num * sizeof(float)));
  ...
  if (NULL == bias) {
    ...
    for (int c = 0; c < channel_num; c++) {
      scale_var_inv[c] = 1.f / sqrt(var[c] * rescale_factor_tmp + eps);
      scale_mean[c] =
          (bias[c] - mean[c] * rescale_factor_tmp) * scale_var_inv[c];
    }
  }
...
static void AddConstNodeToSubGraph(Subgraph *graph, Tensor *tensor,
  ...
  if (tensor->ExistAttr("free_mem")) tensor->RemoveAttr("free_mem");
...
static bool GraphFuseBNScale(Graph *graph, GraphOptimizer *opt) {
  ...
  for (int i = 0; i < node_number; i++) {
    ...
    if (op->GetName() != "BatchNormalization") continue;
  ...
  for (unsigned int i = 0; i < orig_sub.size(); i++) {
    ...
    std::string node_name =
        orig_input->GetName() + "-" + orig_output->GetName();
...
static bool GraphFuseRelu6(Graph *graph, GraphOptimizer *opt) {
  ...
  for (int i = 0; i < node_number; i++) {
    ...
    if (op->GetName() != "Eltwise") continue;

    Eltwise *eltwise_op = dynamic_cast<Eltwise *>(op);
    EltwiseParam *param = eltwise_op->GetParam();
    if (param->type != ELT_MIN_SCALAR) continue;  // todo:  verify 6
...
static bool GraphFuseConvBN(Graph *graph, GraphOptimizer *opt) {
  ...
  for (int i = 0; i < node_number; i++) {
    ...
    if (op->GetName() != "Convolution") continue;
  ...
  for (unsigned int i = 0; i < orig_sub.size(); i++) {
    ...
    *fused_param = *orig_param;
    ...
    Weight_Bn(&fused, fused_node, mean, var, gamma, beta, param_org->eps,
              param_org->rescale_factor, bias_tensor);
...
static bool NodeInGraph(Node *node, Graph *graph) {
  ...
  for (int i = 0; i < number; i++) {
    if (node == graph->seq_nodes[i]) return true;
  }
...
static bool GraphFuseConvReLuCommon(Graph *graph, GraphOptimizer *opt,
                                    bool relu6) {
  int node_number = graph->seq_nodes.size();
  std::vector<Subgraph *> orig_sub;
  ...
  for (int i = 0; i < node_number; i++) {
    ...
    if (relu6) {
      if (op->GetName() != "ReLu6") continue;
    } else {
      if (op->GetName() != "ReLu") continue;
      if (op->GetName() == "ReLu" &&
          dynamic_cast<ReLu *>(op)->GetParam()->negative_slope != 0.f) {
        continue;
      }
    }
    ...
    if (!NodeInGraph(conv_node, graph)) continue;
