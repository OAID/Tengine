...
int main(int argc, char * argv[])
{
   ...
   /* prepare input data */
   float  * input_data=caffe_process_image(image_file,mean_file,224,224);
  ...
  // DumpStaticGraph(graph);
   ...
   std::cout<<"Begin to OPT\n";

   GraphOptimizerManager::Init();
   GraphOptimizerManager::RunOpt("BNScaleRelu",runtime_graph);
   
   std::cout<<"OPT Done\n";

   runtime_graph->DumpGraph();

   std::cout<<"TRY TO RUN GRAPH\n";
   ...
   if(!p_engine->SetTensorBuffer(tensor,input_data,3*224*224*4))
   {
