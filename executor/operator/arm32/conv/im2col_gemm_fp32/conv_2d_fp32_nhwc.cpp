...
#include "conv_2d_fp32_nhwc_kernel/A17.inl"

namespace TEngine {

namespace conv_fast_nhwc {
typedef void (*sgemm_kernel_t)(float* biases, float* input, float* kernel, int kernel_size, float* output, int output_xy,
            ...
            int output_y, int col_start, int col_end)
{
    ...
    if(is_1x1)
    {
        ...
        for(col_i = (col_start & -4); col_i < (col_end & -4); col_i += 4)
        {
            ...
            {
                for(i = 0; i < 4; i++)
                    *cur_col++ = *(im + col_j + (col_i + i) * input_chan);
            }
        }
#else
        {
...
// interleave 0 ~ (output_chan & -12) kernels with 12 in form of k[0-12][0],k[0-12][1],k[0-12][2]..
...
void interleave_kernel(float* kernel, float* kernel_interleaved, int kernel_chan, int kernel_size)
{
    ...
    for(i = kernel_chan12; i < (kernel_chan & -4); i += 4)
    {
        for(k = 0; k < 4; k++)
            cur_kernel[k] = kernel + kernel_size * (i + k);
        for(j = 0; j < kernel_size; j++)
                       ...
                       sgemm_kernel_t kernel_func)
{
    ...
    for(kernel_num = kernel_start; kernel_num < kernel_end; kernel_num += 12)
    {
        ...
        for(col_line = (col_start & -4); col_line < (col_end & -4); col_line += 4)
        {
            ...
            float* cur_output_12 = cur_output + col_line * output_chan;

            kernel_func(cur_biases, cur_col, cur_kernel, kernel_size, cur_output_12, output_chan, activation, 1);
        ...
        if(col_end3)
        {
            ...
            kernel_func(cur_biases, cur_col, cur_kernel, kernel_size, result, 12, activation, 1);

            for(j = 0; j < (col_end3); j++)
                for(i = 0; i < 12; i++)
                    *(cur_output_12 + j * output_chan + i) = result[i + j * 12];
        }
    }
                      ...
                      sgemm_kernel_t kernel_func)
{
    ...
    for(kernel_num = kernel_start & -4; kernel_num < (kernel_end & -4); kernel_num += 4)
    {
        ...
        for(col_line = (col_start & -4); col_line < (col_end & -4); col_line += 4)
        {
            ...
            kernel_func(cur_biases, cur_col, cur_kernel, kernel_size, cur_output_4, output_chan, activation, 1);
        }
        if(col_end3)
        {
            cur_col = ( float* )(col + col_line * kernel_size);
            ...
            kernel_func(cur_biases, cur_col, cur_kernel, kernel_size, result, 4, activation, 1);
...
struct TimeLogger
{
    ...
    ~TimeLogger()
    {
        ...
        if(count && env)
        {
            ...
            printf("count [%d] total time: %llu im2col: %llu (%.2f) gemm: %llu (%.2f)\n", count, total, im2col_used,
...
bool ConvFastNHWC::Run(Node* node)
{
    ...
    // printf("input: %d,%d,%d -- %d,%d,%d  k: %d, s:%d\n",
    // input_chan,input_h,input_w,output_chan,output_y,output_x,kernel_x,stride_x);
...

        {
            for(int g = 0; g < group; g++)
            {
...

                {
                    ...
                    int chan_4_num = (output_chan != output_chan12) ? 1 : 0;
                    int l2_loop = (output_xy - 1) / col_cnt_l2 + 1;
                    int max_task_num = l2_loop * (chan_12_num + chan_4_num);
                    ...
                    for(int col_i = 0; col_i < output_xy; col_i += col_cnt_l2)
                    {
                        ...
                        if(cpu_number == 1)
                        {
                            sgemm4x12(col, kernel_g, bias_g, have_biases, output_g, kernel_size, col_start, col_end, 0,
                                      output_chan12, output_chan, activation, kernel_run_4x12);
                            if(output_chan != output_chan12)
                                sgemm4x4(col, kernel_g, bias_g, have_biases, output_g, kernel_size, col_start, col_end,
                                         output_chan12, output_chan, output_chan, activation, kernel_run_4x4);
                        }
                        else
                        {
...
{
    if(!NodeOpsRegistryManager::RegisterOPImplementor("arm32", "Convolution", conv_fast_nhwc::SelectFunc,
