...
bool Conv2dWinogradNHWC::sgemm_aider(int cpu, int seq, void* data)
{
    ...
    wino_sgemm_4x12_nhwc(param->ker, param->inp, param->output, param->bias, param->bias_term, param->input_c,
...
bool Conv2dWinogradNHWC::sgemm_aider_4(int cpu, int seq, void* data)
{
    wino_sgemm_param* param = ( wino_sgemm_param* )data;

    wino_sgemm_4x4_nhwc(param->ker, param->inp, param->output, param->bias, param->bias_term, param->input_c,
...
bool Conv2dWinogradNHWC::Prerun(Node* node)
{
    ...
    float* kernel_interleaved = ( float* )mem_alloc(trans_ker_size);
...
bool Conv2dWinogradNHWC::Run(Node* node)
{
    ...
    float* trans_inp = ( float* )mem_alloc(sizeof(float) * ELEM_SIZE * input_c * block_hw);
    ...
    for(int i = 0; i < output_n; i++)
    {
        ...
        if(cpu_number == 1)
        {
            wino_sgemm_4x12_nhwc(kernel_interleaved, trans_inp, output, bias, bias_term, input_c,
                                kernel_run_4x12, kernel_run_1x12, 0,
                                 nn_coutx12, 0,
                                 block_hw,    // block_start, block_end,
                                 block_h, block_w, output_c, output_w, resi_h, resi_w, activation);
            if(nn_coutx12 != output_c)
            {
                wino_sgemm_4x4_nhwc(kernel_interleaved, trans_inp, output, bias, bias_term, input_c, kernel_run_4x4,
                                    nn_coutx12, output_c, 0, block_hw, block_h, block_w, output_c, output_w, resi_h,
                                    resi_w, activation);
            }
        }
        else
        {
            ...
            auto f4 = std::bind(&Conv2dWinogradNHWC::sgemm_aider_4, this, std::placeholders::_1, std::placeholders::_2,
                                std::placeholders::_3);
            if(nn_coutx12 != output_c)
            {
...
{
    NodeOpsRegistryManager::RegisterOPImplementor("arm32", "Convolution", conv_2d_wino_nhwc::SelectFunc,
