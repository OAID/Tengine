...
bool Conv2dDepthK5::Run(Node* node)
{
    ...
    for(int i = 0; i < output_n; i++)
    {
        ...
        {
            if(stride_h == 1)
                depthwise_conv_k5s1(input_buf, weight_buf, bias, output_buf, input_h, input_w, input_c, output_h, output_w, pad0, pad1, activation);
            else if(stride_h == 2)
                depthwise_conv_k5s2(input_buf, weight_buf, bias, output_buf, input_h, input_w, input_c, output_h, output_w, activation);
            else
                return false;
        }
        else
...
static bool isDepthwiseSupported(const ConvParam* param, const TShape& input_shape)
{
    ...
    if(group == 1 || input_c != group || kernel_h != 5 || kernel_w != 5 || dilation_h != dilation_w ||
...
{
    if(!NodeOpsRegistryManager::RegisterOPImplementor("arm32", "Convolution", conv_2d_dw_k5::SelectFunc,
