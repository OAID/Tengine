...
static bool isDepthwiseSupported(const ConvParam* param, const TShape& input_shape)
{
    ...
    if(input_h < 8 || input_w < 8)
        return false;
    if(group == 1 || input_c != group || kernel_h != 7 || kernel_w != 7 || dilation_h != dilation_w ||
...
{
    if(!NodeOpsRegistryManager::RegisterOPImplementor("arm32", "Convolution", conv_2d_dw_k7::SelectFunc,
