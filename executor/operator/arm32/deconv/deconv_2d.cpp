...
#endif
#include "deconv_kernel/A17.inl"

#include "op_utils.hpp"
// extern "C" void sgemm_4x12_deconv_a17(float* input, float* kernel, long kernel_size, float* output, long weight_size);
// extern "C" void sgemm_4x4_deconv_a17(float* input, float* kernel, long kernel_size, float* output, long weight_size);
...
// transpose weight matrix and interleave with 12 and 4
void transpose_weight(float* weight, float* weightT, int weight_w, int weight_h)
{
    int i, j, k;
    int weight_w3 = weight_w & 0x3;
    int weight_w12 = (weight_w / 12) * 12;

    for(i = 0; i < weight_w12; i += 12)
...
static void sgemm4x12(float* inputT, float* weightT, float* col_buf, int input_ch, int input_wh, int weight_size,
                      int input_start, int input_end, int weight_start, int weight_end, int cpu_type)
{
    ...
    for(weight_num = weight_start; weight_num < weight_end; weight_num += 12)
    {
        ...
        for(input_line = (input_start & -4); input_line < (input_end & -4); input_line += 4)
        {
            ...
            if(cpu_type == TYPE_A17)
                sgemm_4x12_deconv_a17(cur_input, cur_weight, input_ch, cur_col, weight_size);
        }
        ...
        if(input_end3)
        {
            cur_input = inputT + input_line * input_ch;
            if(cpu_type == TYPE_A17)
                sgemm_4x12_deconv_a17(cur_input, cur_weight, input_ch, result, 12);
            for(j = 0; j < input_end3; j++)
                for(i = 0; i < 12; i++)
                    *(col_buf + (input_line + j) * weight_size + weight_num + i) = result[j * 12 + i];
        }
    }
                     ...
                     int input_start, int input_end, int weight_start, int weight_end, int cpu_type)
{
    ...
    for(weight_num = (weight_start & -4); weight_num < (weight_end & -4); weight_num += 4)
    {
        ...
        for(input_line = (input_start & -4); input_line < (input_end & -4); input_line += 4)
        {
            ...
            cur_col = col_buf + input_line * weight_size + weight_num;
            if(cpu_type == TYPE_A17)
                sgemm_4x4_deconv_a17(cur_input, cur_weight, input_ch, cur_col, weight_size);
        }
        ...
        if(input_end3)
        {
            cur_input = inputT + input_line * input_ch;
            if(cpu_type == TYPE_A17)
                sgemm_4x4_deconv_a17(cur_input, cur_weight, input_ch, result, 4);
            for(j = 0; j < input_end3; j++)
                for(i = 0; i < 4; i++)
...
struct DeconvolutionOps : public MTNodeOps
{
    ...
    bool sgemm_aider(int cpu, int seq, void* data)
    {
        ...
        if(cpu_info->GetCPUModel(cpu) == CPU_A17)
            cpu_type = TYPE_A17;
    ...
    bool Run(Node* node)
    {
        ...
        if(cpu_info->GetCPUModel(master_cpu) == TYPE_A17)
            cpu_type = TYPE_A17;
        ...
        for(int b = 0; b < batch; ++b)
        {
            ...
            for(int g = 0; g < group; g ++)
            {
                ...
                int ch12_num = (weight_size - 1) / 12 + 1;
                int max_task_num = ch12_num * l2_loop;
                int weight_size12 = (weight_size / 12) * 12;
                ...
                if(cpu_number == 1 || max_task_num < 4)
                {
                    for(int input_i = 0; input_i < input_wh; input_i += input_cnt_l2)
                    {
                        ...
                        sgemm4x12(inputT, cur_weight_g, col_buffer, input_ch, input_wh, weight_size, input_start, input_end, 0,
                                  weight_size12, cpu_type);
                        if(weight_size != weight_size12)
                        {
                            sgemm4x4(inputT, cur_weight_g, col_buffer, input_ch, input_wh, weight_size, input_start, input_end,
                                     weight_size12, weight_size, cpu_type);
                        }
                    }
                }
                else
                {
                    ...
                    for(int input_i = 0; input_i < input_wh; input_i += input_cnt_l2)
                    {
                        ...
                        input_end = input_end > input_wh ? input_wh : input_end;
                        int real_ch12_num = weight_size / 12;

                        for(int i = 0; i < real_ch12_num; i++)
                        {
                            ...
                            param->weight_start = i * 12;
                            param->weight_end = param->weight_start + 12;
                        ...
                        if(weight_size != weight_size12)
                        {
                            ...
                            param->input_start = input_start;
                            param->input_end = input_end;
                            param->weight_start = weight_size12;
                            param->weight_end = weight_size;
...
{
    NodeOpsRegistryManager::RegisterOPImplementor("arm32", "Deconvolution", DeconvolutionImpl::SelectFunc, 100);
}
