              ...
              int weight_stride, int start_channel, int end_channel, int cpu_type)
{
    ...
    for(ch = (start_channel & -8); ch < (end_channel & -8); ch += 8)
    {
        ...
        if(cpu_type == TYPE_A17)
            #ifdef ON_A17
            sgemv_1x8_a17(cur_biases, ( float* )input, cur_kernel, weight_stride, cur_result);
            #endif
    }
              ...
              int weight_stride, int start_channel, int end_channel, int cpu_type)
{
    ...
    for(ch = (start_channel & -2); ch < (end_channel & -2); ch += 2)
    {
        ...
        if(cpu_type == TYPE_A17)
            #ifdef ON_A17
            sgemv_1x2_a17(( float* )cur_biases, ( float* )input, cur_kernel, weight_stride, cur_result);
            #endif
    }
...
struct FMOps : public MTNodeOps
{
        ...
        for(int n = 0; n < batch; n++)
        {
            ...
            else
            {
                ...
                for(int i = 0; i < cpu_number; i++)
                {
                    ...
                    auto f = std::bind(&FMOps::SgemvAider, this, std::placeholders::_1, std::placeholders::_2,
                                       std::placeholders::_3);
    ...
    {
        if(node->ExistAttr("weight_interleaved"))
        {
...
using namespace FM_fast;

void RegisterFeatureMatchFast(void)
{
    if(!NodeOpsRegistryManager::RegisterOPImplementor("arm32", "FeatureMatch", FM_fast::SelectFunc,
                                                      FM_fast::default_prio))
        LOG_ERROR() << __FUNCTION__ << " :Regist OP failed for prio [" << FM_fast::default_prio << "]\n";
}
