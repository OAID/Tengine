static int ref_priorbox_uint8(uint8_t* output, const priorbox_ref_param* param, int elem_size)
{
    ...
    {
        int output_data = round(output_f32[i] / param->out_scale + param->out_zero);
        if(output_data > 255)
            output[i] = 255;
