...
int ref_rnn_fp32(float* input,float* output,rnn_param* param)
{
    ...
    for(int i = 0; i < param->seq_lens; i++)
    {
        ...
        if(!do_RNN_step(seq_input, init_h, param->kernel, param->bias, param->batch_size, param->input_size, param->hidden_size))
            return -1;
