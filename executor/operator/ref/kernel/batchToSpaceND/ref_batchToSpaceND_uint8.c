...
static int ref_batchToSpaceND_uint8(const uint8_t* in_data, uint8_t* out_data, struct batchToSpaceND_param* param)
{
    ...
    for(int in_batch = 0; in_batch < param->in_dims[0]; ++in_batch){
        ...
        uint8_t input_zero = param->in_zero;        
        for(int in_h = 0; in_h < param->in_dims[1]; ++in_h){
            ...
            for(int in_w = 0; in_w < param->in_dims[2]; ++in_w){
                ...
                const uint8_t* in = in_data + inOffset;
                for(int i = 0; i < param->out_dims[3]; ++i){
                    output_ptr[i] = (in[i] - input_zero) * scale;
                }
    ...
    float out_scale = 1.0f/param->out_scale;
    uint8_t out_zero = param->out_zero;
    for(int i = 0; i < o_size; i++){
        out_data[i] = round(output_ptr[i] * out_scale + out_zero);
    }
    free(output_ptr); 
