...
int ref_log_softmax_kernel_int8(int8_t* input, int8_t* output, float* max_array, float* sum_array, op_data* op_param)
{
    ...
    for(int i = 0; i < out_size; i++)
        for(int j = 0; j < on_in_size; j++)
            if(fmax < fabs(output_f[i * on_in_size + j]))
                fmax = fabs(output_f[i * on_in_size + j]);
