...
int ref_l2pool_uint8(uint8_t* data, uint8_t* out_data,l2pool_param* param)
{
            ...
            for(int ph = 0; ph < param->outh; ph++)
            {
                for(int pw = 0; pw < param->outw; pw++)
                {
                    ...
                    for(int h = h_start; h < h_end; h++)
                    {
                        ...
                        {
                            val = (data[i*param->inh*param->inc * param->inw +h * param->inc * param->inw + w * param->inc +c]-param->zero_point[0])*param->scale[0];
                            tmp += val * val;
                            pool_size++;
                        }
                    }
                    ...
                    {
                        out_data[index] = sqrt(tmp / pool_size)/param->scale[1]+param->zero_point[1];    
                    }
                }
            }
        }
