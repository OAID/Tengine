static int ref_roipooling_uint8(uint8_t* featmap, uint8_t* roi, uint8_t* output, roipooling_ref_param* param)
{
    ...
    {
        int output_data = round(out_f32[i] / param->out_scale) + param->out_zero;
        if(output_data > 255)
            output[i] = 255;
        else if(output_data < 0)
            output[i] = 0;
        else
            output[i] = ( uint8_t )output_data;
    }
