...
int ref_power_int8(int8_t* input, int8_t* output,ref_power_param* param)
{
        ...
        for(int c = 0; c < param->iDataC; c++)
        {
            for(int h = 0; h < param->iDataH; h++)
            {
                for(int w = 0; w < param->iDataW; w++)
                {
                    ...
                    output[size] = pow((param->shift + param->scale * input[size]* param->q_scale[0]), param->power)/ param->q_scale[1];
                }
            }
        }
    }
