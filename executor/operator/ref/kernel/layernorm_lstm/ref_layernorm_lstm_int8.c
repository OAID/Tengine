...
int ref_layernormlstm_int8(int8_t* total_input_data,int8_t* total_output_data,lnlstm_param* param)
{
    for(int i = 0; i < param->sequence_size; i++)
    {
        ...
        for(int j = 0; j < param->batch_size; j++)
        {
            ...
            {
                input_data[j * param->input_size + k] = (total_input_data[per_batch_num * j + i * param->input_size + k])*param->scale[0];
            }
        }
        ...
        {
            for(int n = 0; n < param->output_size;n++)
            {
                total_output_data[per_batch_num_o * m + i * param->output_size + n] = output_data[m * param->output_size + n]/param->scale[1];    
            }
        }
        free(input_data);
        free(output_data);
