...
for(int i = 0; i < param->sequence_size; i++)
{
    ...
    for(int j = 0; j < param->batch_size; j++)
    {
        ...
        {
            input_data[j * param->input_size + k] = fp16_to_fp32(total_input_data[per_batch_num * j + i * param->input_size + k]);
        }
    }
    ...
    int per_batch_num = param->output_size * i;
    for(int m = 0; m < param->batch_size; m++)
    {
        for(int n = 0; m < param->output_size; m++)
        {
            total_output_data[per_batch_num * m + i * param->output_size + n] = output_data[m * param->output_size + n];    
        }
    }
    free(input_data);
    free(output_data);
