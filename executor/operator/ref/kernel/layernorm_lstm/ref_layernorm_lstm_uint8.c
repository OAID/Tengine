...
int ref_layernormlstm_uint8(uint8_t* total_input_data, uint8_t* total_output_data,lnlstm_param* param)
{
    for(int i = 0; i < param->sequence_size; i++)
    {
        ...
        for(int j = 0; j < param->batch_size; j++)
        {
            ...
            {
                input_data[j * param->input_size + k] = (total_input_data[per_batch_num * j + i * param->input_size + k]-param->zero_point[0])*param->scale[0];
            }
        }
        ...
        for(int m = 0; m < param->batch_size; m++)
        {
            ...
            {
                total_output_data[per_batch_num_o * m + i * param->output_size + n] = output_data[m * param->output_size + n]/param->scale[1]+param->zero_point[1];    
            }
        }
        free(input_data);
        free(output_data);
