...
struct LRNOps : public NodeOps {
  ...
  inline float32x4_t vtaylor_polyq_f32(
      float32x4_t x, const std::array<float32x4_t, 8> &coeffs) {
  ...
  bool Run(Node *node) {
    ...
    for (int i = 0; i < n; i++) {
      ...
      for (int j = 0; j < img_size; j++) square[j] = img_base[j] * img_base[j];
        ...
        for (int j = 0; j < c; j++) {
          ...
          for (int m = 0; m < mod; m++) {
            ...
            for (int l = c_start; l <= c_end; l++) {
              accu =
                  vaddq_f32(accu, vld1q_f32(square + l * channel_size + m * 4));
            }
            const float32x4_t normalized =
                vpowq_f32(vmlaq_f32(bias_vec, alpha_vec, accu), beta_vec);
            const float32x4_t normalized_pixel =
                vmulq_f32(vld1q_f32(img_base + j * channel_size + m * 4),
                          vinvq_f32(normalized));
            vst1q_f32(out_base + j * channel_size + m * 4, normalized_pixel);
          }
          ...
          for (int m = 4 * mod; m < channel_size; m++) {
            ...
            out_base[j * channel_size + m] =
                img_base[j * channel_size + m] *
                std::pow(bias + alpha_over_size * sum, -beta);
          }
        }
      } else {
