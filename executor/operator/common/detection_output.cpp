...
struct DetectionOutputOps : public NodeOps {
  ...
  void get_boxes(std::vector<Box>& boxes, int num_prior, float* loc_ptr,
                 float* prior_ptr) {
  ...
  void nms_sorted_bboxes(const std::vector<Box>& bboxes,
  ...
  bool Run(Node* node) {
    ...
    for (int i = 0; i < num_classes; i++) {
      ...
      bbox_rects.insert(bbox_rects.end(), class_bbox_rects.begin(),
                        class_bbox_rects.end());
    }
...
void RegisterDetectionOutputNodeExec(void) {
  ...
  NodeOpsRegistryManager::RegisterOPImplementor("common", "DetectionOutput",
                                                ops);
}
