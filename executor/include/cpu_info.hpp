...
struct CPUInfo {
  ...
  CPUInfo(const struct cpu_info *cpu_dev) {
    ...
    dev.cluster = (struct cpu_cluster *)malloc(sizeof(struct cpu_cluster) *
                                               dev.cluster_number);
    ...
    if (dev.online_cpu_number > 0) {
      ...
      memcpy(dev.online_cpu_list, cpu_dev->online_cpu_list,
             sizeof(int) * dev.online_cpu_number);
    } else {
      int total_number = 0;
      ...
      for (int i = 0; i < total_number; i++) dev.online_cpu_list[i] = i;
    }
