...
enum exec_policy_t { kLatency, kPower, kBatch };
...
class Driver {
 public:
  ...
  /*!
   * @brief Create a graph handle on a device. Graph handle is a H/W context
   * inside device
   *
  ...
  virtual void SetGraphDoneHook(Device* dev, void* graph_handle,
                                dev_graph_cb_t func) = 0;

  /*!
   * @brief Set the callback hook, when a single node executed in graph_handle
   * context is done. It is used by driver to notify the upper layer node
   * execution is done.
   *
   * @param dev, the device to be set
   * @param graph_handle, the graph handle
  ...
  virtual void SetNodeDoneHook(Device* dev, void* graph_handle,
                               dev_node_cb_t func) = 0;

  /*!
   * @brief Optimized the upper layer passed graph, and save the optimized graph
   * in graph_handle
   *
  ...
  virtual bool OptimizeGraph(Device* dev, void* graph_handle,
                             Subgraph* graph) = 0;
  ...
  /*!
   * @brief Prepare the execution of the optimized subgraph, saved in
   * graph_handle This interface will only be called once for a subgraph, or
   * graph_handle
   *
   * @param dev, the device
   * @param graph_handle, the graph handle
  ...
  virtual bool GetRunMemorySize(Device* dev, void* graph_handle,
  ...
  virtual bool SetDevConfig(Device* dev, const char* config_name,
  ...
  virtual bool GetDevConfig(Device* dev, const char* config_name, void* val,
                            int size) {
    return false;
  }
  ...
  virtual bool DelDevConfig(Device* dev, const char* config_name) {
    return false;
  }
  ...
  /*!
   * @brief Get the performance vector of a sugraph with specific execution
   * policy
   *
   * @param dev, the device
   ...
   */
  virtual bool GetPerf(Device* dev, Subgraph* graph, int policy,
                       GraphPerf& perf) = 0;
  ...
  /*!
   * @brief Get the float ops rate ( Kfops/second) for the device to execution a
   * graph if the graph is nullptr, return the nominal fops of the device
   * @param dev, the device
  ...
  /*!
   * @brief Get the execution propsoal for subgraph on this dev with execution
   * policy the device should go throught the graph, and set or replace
   * DEV_PROPOSAL_ATTR of a node only when its level is greater than
   * presetted one
   *
   * @param dev, the device
   * @param graph, the graph
...
class Device {
 public:
  ...
  Device(const dev_id_t& dev_id) { dev_id_ = dev_id; }
  void SetName(const std::string& name) { name_ = name; }
  void BindDriver(Driver* driver) { driver_ = driver; }
  void SetDeviceType(const dev_type_t& dev_type) { dev_type_ = dev_type; }

  const std::string& GetName(void) { return name_; }
  const dev_id_t& GetDeviceID(void) { return dev_id_; }
  const dev_type_t& GetDeviceType(void) { return dev_type_; }
  Driver* GetDriver(void) { return driver_; }

  /* control interface */

  bool InitHW(void) { return driver_->InitializeDevice(this); }
  bool ReleaseHW(void) { return driver_->ReleaseDevice(this); }
  bool Stop(void) { return driver_->StopDevice(this); }
  bool Start(void) { return driver_->StartDevice(this); }

  /* data process interface */
  void* CreateGraphHandle(Subgraph* graph) {
    return driver_->CreateGraphHandle(this, graph);
  }
  void* CreateGraphHandle(void) { return driver_->CreateGraphHandle(this); }
  ...
  bool Run(void* graph_handle) { return driver_->Run(this, graph_handle); }
  ...
  bool GetProposal(Subgraph* graph, int policy) {
    return driver_->GetProposal(this, graph, policy);
  }
...
class DriverManager
    : public SimpleObjectManagerWithLock<DriverManager, Driver*> {
 public:
