...
struct DevExecutor {
  ...
  /*!
   * @brief Get the nominal performance to run a graph, in terms of Mfops per
   * second
   *
   * @param graph the graph to examine. if graph is nullptr, return the maximum
   * fops rate
   *
  ...
  virtual bool SetDevConfig(const char* config_name, const void* val,
                            int size) = 0;
  virtual bool GetDevConfig(const char* config_name, void* buffer,
                            int size) = 0;
