...
mtcnn::mtcnn(int minsize, float conf_p, float conf_r, float conf_o, float nms_p,
             float nms_r, float nms_o) {
  minsize_ = minsize;
...
int mtcnn::run_PNet(const cv::Mat& img, scale_window& win,
  ...
  generate_bounding_box(prob_data, reg_data, scale, conf_p_threshold_,
...
void mtcnn::run_RNet(const cv::Mat& img, std::vector<face_box>& pnet_boxes,
...
void mtcnn::run_ONet(const cv::Mat& img, std::vector<face_box>& rnet_boxes,
                     std::vector<face_box>& output_boxes) {
...
void mtcnn::detect(cv::Mat& img, std::vector<face_box>& face_list) {
  ...
  for (unsigned int i = 0; i < total_onet_boxes.size(); i++) {
    ...
    // std::cout<<"i="<<i<<"\t"<<box.x0<<" "<<box.y0<<" "<<box.x1<<"
    // "<<box.y1<<" "<<box.landmark.x[3]<<" "<<box.landmark.y[2]<<"\n";
  }
