...
void cal_scale_list(int height, int width, int minsize,
                    std::vector<scale_window>& list) {
  float factor = 0.709;
  int MIN_DET_SIZE = 12;
...
void set_cvMat_input_buffer(std::vector<cv::Mat>& input_channels,
...
void generate_bounding_box(const float* confidence_data, const float* reg_data,
                           float scale, float threshold, int feature_h,
                           int feature_w, std::vector<face_box>& output,
                           bool transposed) {
  int stride = 2;
  int cellSize = 12;
...
void nms_boxes(std::vector<face_box>& input, float threshold, int type,
               std::vector<face_box>& output) {
  output.clear();
  std::sort(
      input.begin(), input.end(),
      [](const face_box& a, const face_box& b) { return a.score > b.score; });
...
void process_boxes(std::vector<face_box>& input, int img_h, int img_w,
                   std::vector<face_box>& rects, float nms_r_thresh) {
...
void copy_one_patch(const cv::Mat& img, face_box& input_box, float* data_to,
