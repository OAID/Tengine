...
int main(int argc, char **argv) {
  ...
  if (argc == 1) {
    std::cout << "[usage]: " << argv[0]
              << " <test.jpg>  <model_dir> [save_result.jpg] \n";
  }
  ...
  std::cout << "repeat " << repeat_count << ", avg time is "
  ...
  for (unsigned int i = 0; i < face_info.size(); i++) {
    ...
    std::printf("BOX:( %g , %g ),( %g , %g )\n", box.x0, box.y0, box.x1,
                box.y1);
    cv::rectangle(image, cv::Point(box.x0, box.y0), cv::Point(box.x1, box.y1),
                  cv::Scalar(0, 255, 0), 1);
    for (int l = 0; l < 5; l++) {
      cv::circle(image, cv::Point(box.landmark.x[l], box.landmark.y[l]), 1,
