...
typedef struct abox {
  ...
  float score;
  int class_idx;
  bool operator<(const abox &tmp) const { return score < tmp.score; }
} abox;

void bbox_tranform_inv(float *local_anchors, float **boxs_delta, int num_roi,
                       int imgw, int imgh) {
  for (int i = 0; i < num_roi; i++) {
    ...
    if (boxs_delta[i][0] < 0) boxs_delta[i][0] = 0;
    if (boxs_delta[i][0] > imgw) boxs_delta[i][0] = imgw;

    boxs_delta[i][1] = pred_ctr_y - 0.5 * pred_h;
    if (boxs_delta[i][1] < 0) boxs_delta[i][1] = 0;
    if (boxs_delta[i][1] > imgh) boxs_delta[i][1] = imgh;

    boxs_delta[i][2] = pred_ctr_x + 0.5 * pred_w;
    if (boxs_delta[i][2] < 0) boxs_delta[i][2] = 0;
    if (boxs_delta[i][2] > imgw) boxs_delta[i][2] = imgw;

    boxs_delta[i][3] = pred_ctr_y + 0.5 * pred_h;
    if (boxs_delta[i][3] < 0) boxs_delta[i][3] = 0;
    if (boxs_delta[i][3] > imgh) boxs_delta[i][3] = imgh;
  }
}
...
void draw_detections(std::string &image_file, std::string &save_name,
  ...
  for (int b = 0; b < (int)boxes.size(); b++) {
    ...
    cv::putText(img, label, cv::Point(box.x1, box.y1), cv::FONT_HERSHEY_SIMPLEX,
                0.5, cv::Scalar(0, 0, 0));
  }
...
int main(int argc, char *argv[]) {
  ...
  const char *model_name = "faster_rcnn";
  ...
  // preprocess img
  const int INPUT_SIZE_LONG = 400;
  const int INPUT_SIZE_NARROW = 250;
  ...
  for (int i = 1; i < num_class; i++) {
    ...
    bbox_tranform_inv(rois_data, bbox_delt, num_roi, image.cols - 1,
                      image.rows - 1);
