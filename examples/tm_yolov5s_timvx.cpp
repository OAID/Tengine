    ...
    for (int h = 0; h <= feat_h - 1; h++)
    {
        for (int w = 0; w <= feat_w - 1; w++)
        {
            for (int a = 0; a <= anchor_num - 1; a++)
            {
                ...
                //process box score
                float box_score =
                    feat[a * feat_w * feat_h * (cls_num + 5) + (h * feat_w) * (cls_num + 5) + w * (cls_num + 5) + 4];
...
void get_input_data_focus_uint8(const char* image_file, uint8_t* input_data, int letterbox_rows, int letterbox_cols,
                                const float* mean, const float* scale, float input_scale, int zero_point)
{
                ...
                for (int h = 0; h < letterbox_rows / 2; h++)
                {
                    for (int w = 0; w < letterbox_cols / 2; w++)
                    {
                        ...
                        int out_index = i * 2 * 3 * (letterbox_cols / 2) * (letterbox_rows / 2)
                                        ...
                                        + c * (letterbox_cols / 2) * (letterbox_rows / 2) + h * (letterbox_cols / 2)
                                        + w;
...
int main(int argc, char* argv[])
{
    ...
    get_input_data_focus_uint8(image_file, input_data.data(), letterbox_rows, letterbox_cols, mean, scale, input_scale,
                               input_zero_point);
