...
bool run_tengine_library(const char* model_name, const char* proto_file, const char* model_file, const char* label_file,
                         const char* image_file, int img_h, int img_w, const float* mean, float scale, int repeat_count,
                         const std::string device)
{
    ...
    // create graph
    graph_t graph = create_graph(nullptr, "caffe", proto_file, model_file);
    if(graph == nullptr)
    {
    ...
    // set 'low_mem_mode' attribute for the graph
    int val = 1;
...
int main(int argc, char* argv[])
{
        ...
        switch(res)
        {
            ...
            case 'h':
                std::cout << "[Usage]: " << argv[0] << " [-h]\n"
                          ...
                          << "    [-g img_h,img_w] [-s scale] [-w mean[0],mean[1],mean[2]] [-r repeat_count] [-d "
                             "device_name]\n";
                return 0;
            default:
                break;
    ...
    if(proto_file.empty() || model_file.empty())
    {
        ...
        if(model_name.empty())
        {
            ...
            std::cout << "Model name and model files not specified, run " << model_name << " by default.\n";
        }
    ...
    // check input files
    if(!check_file_exist(proto_file) || !check_file_exist(model_file) || !check_file_exist(label_file) ||
       !check_file_exist(image_file))
        return -1;

    // start to run
    if(!run_tengine_library(model_name.c_str(), proto_file.c_str(), model_file.c_str(), label_file.c_str(),
                            image_file.c_str(), img_h, img_w, mean, scale, repeat_count, device))
        return -1;
