...
void draw_detections(std::string &image_file, std::string &save_name, int num,
                     float thresh, std::vector<Box> &boxes, float **probs,
                     int classes) {
  ...
  for (i = 0; i < num; ++i) {
    ...
    for (j = 0; j < classes; ++j) {
      if (probs[i][j] > thresh) {
        ...
        printf("BOX:( %d , %d ),( %d , %d )\n", left, top, right, bot);
        cv::rectangle(img, cv::Rect(left, top, (right - left), (bot - top)),
                      cv::Scalar(0, 255, 255), line_width);
        std::ostringstream score_str;
        score_str << probs[i][j];
        std::string label =
            std::string(class_names[class_id + 1]) + ": " + score_str.str();
        int baseLine = 0;
        cv::Size label_size =
        ...
        cv::putText(img, label, cv::Point(left, top), cv::FONT_HERSHEY_SIMPLEX,
                    0.5, cv::Scalar(0, 0, 0));
      }
    }
  }
...
int main(int argc, char **argv) {
  ...
  for (int i = 0; i < repeat_count; i++) {
    ...
    if (repeat_count == 1)
      draw_detections(image_file, save_name, total, thresh, boxes, probs,
                      num_class);
