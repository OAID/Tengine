    ...
    for (int h = 0; h < img_h; h++)
    {
        for (int w = 0; w < img_w; w++)
        {
            for (int c = 0; c < 3; c++)
            {
                ...
                /* quant to int8 */
                int idata = (round)(input_fp32 / input_scale + (float)zero_point);
                if (idata > 127)
                    idata = 127;
                else if (idata < -127)
                    idata = -127;
...
static void draw_objects(const cv::Mat& bgr, const std::vector<Object>& objects)
{
    ...
    cv::imwrite("yolov3_tiny_int8_out.jpg", image);
}
...
int main(int argc, char* argv[])
{
    ...
    get_input_data_yolov3_int8(image_file, input_data.data(), img_h, img_w, mean, scale, input_scale, input_zero_point);
    ...
    int p16_count = get_tensor_buffer_size(p16_output) / sizeof(int8_t);
    int p32_count = get_tensor_buffer_size(p32_output) / sizeof(int8_t);

    int8_t* p16_data_i8 = (int8_t*)get_tensor_buffer(p16_output);
    int8_t* p32_data_i8 = (int8_t*)get_tensor_buffer(p32_output);
    ...
    {
        p16_data[c] = ((float)p16_data_i8[c] - (float)p16_zero_point) * p16_scale;
    }

    for (int c = 0; c < p32_count; c++)
    {
        p32_data[c] = ((float)p32_data_i8[c] - (float)p32_zero_point) * p32_scale;
    }
