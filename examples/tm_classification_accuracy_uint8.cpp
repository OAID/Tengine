                     ...
                     const float* scale, int swapRB, int loop_count, int num_thread)
{
    ...
    float* input_data_fp32 = ( float* )malloc(img_size * sizeof(float));
    ...
    while(getline(f, line_str))
    {
        ...
        get_input_data_cv(m, input_data_fp32, img_h, img_w, mean, scale, swapRB);
        ...
        {
            int udata = (round)(input_data_fp32[i] / input_scale + input_zero_point);
            if (udata > 255)
                udata = 255;
            else if (udata < 0)
                udata = 0;

            input_data[i] = uint8_t(udata);
        }
        ...
        float scores_scale = 0.f;
        int scores_zp = 0;
        get_tensor_quant_param(output_tensor, &scores_scale, &scores_zp, 1);

        uint8_t* scores_data_u8 = ( uint8_t* )get_tensor_buffer(output_tensor);
        int scores_size_u8 = get_tensor_buffer_size(output_tensor);

        float* scores_data_fp32 = ( float* )malloc(scores_size_u8 * sizeof(float));
        for (int i = 0; i < scores_size_u8; i++)
            scores_data_fp32[i] = (( float )scores_data_u8[i] - (float)scores_zp) * scores_scale;

        // float* output_data = ( float* )get_tensor_buffer(output_tensor);
        // int output_size = get_tensor_buffer_size(output_tensor) / sizeof(float);

        accuracy_top5(scores_data_fp32, scores_size_u8, stoi(gt_id), total_num, top1_num, top5_num);
        fprintf(stderr, "--------------------------------------\n");
    }
...
int main(int argc, char* argv[])
{
    ...
    int res;
    while ((res = getopt(argc, argv, "m:i:v:g:s:w:r:t:c:d:h")) != -1)
    {
        switch (res)
        {
            case 'm':
