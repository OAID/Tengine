...
const Model_Config model_list[] = {{"squeezenet",
                                    227,
                                    227,
                                    1.f,
...
const Model_Config *get_model_config(const char *model_name) {
  ...
  for (unsigned int i = 0; i < name1.size(); i++) name1[i] = tolower(name1[i]);
...
bool run_tengine_library(const char *model_name, const char *tm_file,
  ...
  // load model
  if (load_model(model_name, "tengine", tm_file) < 0) return false;
...
int main(int argc, char *argv[]) {
    ...
    switch (res) {
      ...
      case 'h':
        std::cout << "[Usage]: " << argv[0] << " [-h]\n"
                  << "    [-n model_name] [-t tm_file] [-l label_file] [-i "
                     "image_file]\n"
  ...
  if (tm_file.empty()) {
    ...
    if (model_name.empty()) {
      ...
      std::cout << "Model name and tm file not specified, run " << model_name
                << " by default.\n";
    }
  ...
  // start to run
  if (run_tengine_library(model_name.c_str(), tm_file.c_str(),
                          label_file.c_str(), image_file.c_str(), img_h, img_w,
