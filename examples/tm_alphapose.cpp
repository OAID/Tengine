...
cv::Mat crop_box(const cv::Mat& org_img, const pt_t& up_left, const pt_t& bottom_right, const int& input_res_h,
...
float* pre_process_pose(cv::Mat& img, const std::vector<bbox_t>& boxes, std::vector<pt_t>& pt1, std::vector<pt_t>& pt2)
{
...
cv::Mat transform_box_invert_batch(cv::Mat& pt, const std::vector<pt_t>& ul, const std::vector<pt_t>& br,
                                   const int& input_res_h, const int& input_res_w, const int& output_res_h,
                                   const int& output_res_w)
{
...
predict_t get_predict(float* hm_data, const int hm_dims[4], const std::vector<pt_t>& pt1, const std::vector<pt_t>& pt2,
                      const int& input_res_h, const int& input_res_w)
{
    ...
    auto get_hm_data = [](float* data, const int data_dims[4], const std::array<int, 4> ele_dims) {
        return *(data + ele_dims[0] * data_dims[1] * data_dims[2] * data_dims[3]
                 + ele_dims[1] * data_dims[2] * data_dims[3] + ele_dims[2] * data_dims[3] + ele_dims[3]);
    };
    ...
    for (int i = 0; i < hm_dims[0]; i++)
    {
        for (int j = 0; j < hm_dims[1]; j++)
        {
            ...
            preds.ptr<cv::Vec2f>(i, j)->val[0] = preds.ptr<cv::Vec2f>(i, j)->val[1] =
                std::distance(start_iter, max_element) + 1;
    ...
    for (int i = 0; i < hm_dims[0]; i++)
    {
        for (int j = 0; j < hm_dims[1]; j++)
        {
            ...
            if ((0 < pX) && (pX < (hm_dims[2] - 1)) && (0 < pY) && (pY < (hm_dims[3] - 1)))
            {
                ...
                float x = get_hm_data(hm_data, hm_dims, { i, j, pY, pX + 1 })
                          - get_hm_data(hm_data, hm_dims, { i, j, pY, pX - 1 });
                float y = get_hm_data(hm_data, hm_dims, { i, j, pY + 1, pX })
                          - get_hm_data(hm_data, hm_dims, { i, j, pY - 1, pX });
...
void post_process_pose(const char* image_file, float* heatmap_data, int heatmap_dims[4], const std::vector<pt_t>& pt1,
                       const std::vector<pt_t>& pt2)
{
    cv::Mat preds_hm, preds_scores;
    std::tie(std::ignore, preds_hm, preds_scores) =
        get_predict(heatmap_data, heatmap_dims, pt1, pt2, DEFAULT_IMG_H, DEFAULT_IMG_W);
    auto preds_mean_scores = cv::mean(preds_scores.col(0));

    cv::Mat frame          = cv::imread(image_file);
    for (int i = 0; i < preds_hm.rows; i++)
    {
        if (cv::mean(preds_scores.row(i)).val[0] < s_keypoint_thresh)
            continue;
        for (int kp_i = 0; kp_i < preds_hm.cols; kp_i++)
        {
            cv::circle(frame, cv::Point((int)preds_hm.ptr<float>(i, kp_i)[0], (int)preds_hm.ptr<float>(i, kp_i)[1]), 4,
