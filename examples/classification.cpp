...
int main(int argc, char* argv[])
{
    ...
    // check input files
    if(!check_file_exist(tm_file.c_str()) ||
       !check_file_exist(label_file.c_str()) ||
       !check_file_exist(image_file.c_str()))
        return -1;
    ...
    std::array<float, 3> scales{scale, scale, scale};
    get_input_data(_image_file, (float* )input_tensor.data, img_h, img_w, _channel_mean, scales.data());
