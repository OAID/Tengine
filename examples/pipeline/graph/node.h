...
template <typename... Args> class Param {
public:
...
template <typename IN, typename OUT> class Node : public BaseNode {
...
public:
  template <size_t I> IN_Edge<I> input() { return std::get<I>(m_inputs); }

  template <size_t I> OUT_Edge<I> output() { return std::get<I>(m_outputs); }

  template <size_t I> void set_input(IN_Edge<I> edge) {
    std::get<I>(m_inputs) = edge;
    edge->set_out_node(this, m_name);
  }

  template <size_t I> void set_output(OUT_Edge<I> edge) {
