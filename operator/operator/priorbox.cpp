...
bool PriorBox::InferShape(const std::vector<TEngine::TShape>& ishape,
  ...
  if (param_.flip) len_aspect_ratio += 1;
  int len_max = 0;
  ...
  param_.num_priors_ =
      (param_.aspect_ratio.size() * len_aspect_ratio + 1 + len_max) *
      param_.min_size.size();
