...
void scale_enum(const Anchor anchor, const std::vector<float>& anchor_scales_,
                std::vector<Anchor>& result) {
  Box tmp_box;
  whctrs(anchor, &tmp_box);
  for (int i = 0; i < (int)anchor_scales_.size(); ++i) {
    Anchor tmp;
    mkanchor(tmp_box.w * anchor_scales_[i], tmp_box.h * anchor_scales_[i],
             tmp_box.cx, tmp_box.cy, &tmp);
    result.push_back(tmp);
  }
}

void ratio_enum(const Anchor anchor, const std::vector<float>& ratios_,
                std::vector<Anchor>& result) {
  Box tmp_box;
  whctrs(anchor, &tmp_box);
...
void generate_anchors(const int base_size, const std::vector<float>& ratios_,
  ...
  for (int i = 0; i < (int)ratio_anchors.size(); ++i) {
    ...
    gen_anchors_.insert(gen_anchors_.end(), scale_anchors.begin(),
                        scale_anchors.end());
  }
}
...
bool RPN::InferShape(const std::vector<TEngine::TShape>& ishape,
  ...
  generate_anchors(param_.basesize, param_.ratios, param_.anchor_scales,
                   param_.anchors_);
