...
bool Pooling::InferShape(const std::vector<TEngine::TShape>& ishape,
  ...
  if (param_.pad_h >= 0) {
    if (param_.caffe_flavor)
      output_h = std::ceil(((float)(input_h - param_.kernel_shape[0] +
                                    2 * param_.pads[0])) /
                           param_.strides[0]) +
                 1;
    else
      output_h = (input_h - param_.kernel_shape[0] + 2 * param_.pads[0]) /
                     param_.strides[0] +
                 1;
    ...
    output_h =
        (input_h - param_.kernel_shape[0] + param_.pads[0] + param_.pads[2]) /
            param_.strides[0] +
        1;
  }
  ...
  if (param_.pad_w >= 0) {
    if (param_.caffe_flavor)
      output_w = std::ceil(((float)(input_w - param_.kernel_shape[1] +
                                    2 * param_.pads[1])) /
                           param_.strides[1]) +
                 1;
    else
      output_w = (input_w - param_.kernel_shape[1] + 2 * param_.pads[1]) /
                     param_.strides[1] +
                 1;
  } else {
    ...
    output_w =
        (input_w - param_.kernel_shape[1] + param_.pads[1] + param_.pads[3]) /
            param_.strides[1] +
        1;
  }

  std::vector<int> dim = {input_shape.GetN(), input_shape.GetC(), output_h,
                          output_w};
...
float Pooling::GetFops(const std::vector<TShape>& inputs,
