...
bool Clip::InferShape(const std::vector<TEngine::TShape>& ishape, std::vector<TEngine::TShape>& oshape, int layout)
{
...
void Clip::SetSchema(void)
{
    Input({"input:float"}).Output({"output:float"}).SetAttr("max", 0).SetAttr("min", 0).SetDoc(R"DOC(Clip Layer)DOC");
}
