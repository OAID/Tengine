...

{
  ...
  if ((input_c != weight_c * param_.group) ||
      (output_c != param_.output_channel) || (param_.kernel_h != weight_h) ||
  ...
  int output_h = (input_h - dilation_h * (param_.kernel_h - 1) - 1 +
                  param_.pads[0] + param_.pads[2]) /
                     param_.stride_h +
                 1;
  int output_w = (input_w - dilation_w * (param_.kernel_w - 1) - 1 +
                  param_.pads[1] + param_.pads[3]) /
                     param_.stride_w +
                 1;
...
float Convolution::GetFops(const std::vector<TShape>& inputs,
                           const std::vector<TShape>& outputs) {
  const std::vector<int>& input_dims = inputs[0].GetDim();

  int per_input_c = input_dims[1] / param_.group;

  float ops = 1.0f * per_input_c * param_.kernel_h * param_.kernel_w *
              outputs[0].GetSize() * 2;

  if (ops < 0) {
    std::cout << "input_c: " << per_input_c << " kernel_h: " << param_.kernel_h
